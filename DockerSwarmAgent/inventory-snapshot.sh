...
#   {"cast-operations.dockerswarm.kind":"Node","data":{...flat fields...}}
#
...
LOG_PATH="${DOCKER_INVENTORY_LOG_PATH:-/var/log/cast-operations-docker-swarm-inventory.log}"
INTERVAL="${DOCKER_INVENTORY_INTERVAL_SECONDS:-300}"
DOCKER_API="http://localhost/v1.44"

TMP_DIR="$(dirname "${LOG_PATH}")"
SERVICES_JSON="${TMP_DIR}/.cast-operations-swarm-services.json"
NODES_JSON="${TMP_DIR}/.cast-operations-swarm-nodes.json"
...
snapshot() {
    ...
    # Nodes
    jq -c '.[] | {"cast-operations.dockerswarm.kind":"Node","data":{
        ID: .ID,
      ...
      || echo "cast-operations-swarm-inventory: failed to emit Node" >&2

    # Services
    jq -c '.[] | {"cast-operations.dockerswarm.kind":"Service","data":{
        ID: .ID,
        Name: .Spec.Name,
      ...
      || echo "cast-operations-swarm-inventory: failed to emit Service" >&2

    # Stacks — derived from the com.docker.stack.namespace service label.
    jq -c '[.[] | .Spec.Labels["com.docker.stack.namespace"] // empty]
            | group_by(.)
            | map({"cast-operations.dockerswarm.kind":"Stack","data":{Name: .[0], Services: (length|tostring)}})
            | .[]' "${SERVICES_JSON}" >> "${LOG_PATH}.tmp" 2>/dev/null \
      || echo "cast-operations-swarm-inventory: failed to emit Stack" >&2
    ...
    fetch "/tasks" | jq -c \
        ...
        | {"cast-operations.dockerswarm.kind":"Task","data":{
            ID: .ID,
      ...
      || echo "cast-operations-swarm-inventory: failed to emit Task" >&2

    # Networks — swarm-scoped (overlay/ingress) only.
    fetch "/networks" | jq -c '.[] | select(.Scope == "swarm")
        | {"cast-operations.dockerswarm.kind":"Network","data":{ID: .Id, Name: .Name, Driver: .Driver, Scope: .Scope}}' \
      >> "${LOG_PATH}.tmp" 2>/dev/null \
      || echo "cast-operations-swarm-inventory: failed to emit Network" >&2

    # Secrets
    fetch "/secrets" | jq -c '.[] | {"cast-operations.dockerswarm.kind":"Secret","data":{ID: .ID, Name: .Spec.Name, CreatedAt: .CreatedAt, UpdatedAt: .UpdatedAt}}' \
      >> "${LOG_PATH}.tmp" 2>/dev/null \
      || echo "cast-operations-swarm-inventory: failed to emit Secret" >&2

    # Configs
    fetch "/configs" | jq -c '.[] | {"cast-operations.dockerswarm.kind":"Config","data":{ID: .ID, Name: .Spec.Name, CreatedAt: .CreatedAt, UpdatedAt: .UpdatedAt}}' \
      >> "${LOG_PATH}.tmp" 2>/dev/null \
      || echo "cast-operations-swarm-inventory: failed to emit Config" >&2

    # Volumes (local to the manager node this agent runs on).
    fetch "/volumes" | jq -c --arg node "${NODE_HOSTNAME}" '.Volumes[]?
        | {"cast-operations.dockerswarm.kind":"Volume","data":{Name: .Name, Driver: .Driver, Scope: .Scope, Mountpoint: .Mountpoint, Node: $node}}' \
      >> "${LOG_PATH}.tmp" 2>/dev/null \
      || echo "cast-operations-swarm-inventory: failed to emit Volume" >&2

    mv "${LOG_PATH}.tmp" "${LOG_PATH}"
}

while :; do
    snapshot || echo "cast-operations-swarm-inventory: snapshot iteration failed" >&2
    sleep "${INTERVAL}"
done
