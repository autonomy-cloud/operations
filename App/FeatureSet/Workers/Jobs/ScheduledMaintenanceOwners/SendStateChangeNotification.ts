    ...
    for (const scheduledMaintenanceStateTimeline of scheduledMaintenanceStateTimelines) {
      ...
      for (const user of owners) {
        ...
        const sms: SMSMessage = {
          message: `This is a message from Cast Operations. Scheduled maintenance event - ${scheduledMaintenanceIdentifier}, state changed${previousState ? ` from ${previousState.name}` : ""} to ${scheduledMaintenanceState!
            .name!}. To view this event, go to Cast Operations Dashboard. To unsubscribe from this notification go to User Settings in Cast Operations Dashboard.`,
        };

        const callMessage: CallRequestMessage = {
          data: [
            {
              sayMessage: `This is a message from Cast Operations. Scheduled maintenance event ${scheduledMaintenanceIdentifier} state changed${previousState ? ` from ${previousState.name}` : ""} to ${scheduledMaintenanceState!
                .name!}. To view this event, go to Cast Operations Dashboard. To unsubscribe from this notification go to User Settings in Cast Operations Dashboard. Good bye.`,
            },
          ],
        };
