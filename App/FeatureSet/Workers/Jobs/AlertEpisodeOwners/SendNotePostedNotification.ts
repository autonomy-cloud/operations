    ...
    for (const noteObject of notes) {
      ...
      for (const user of owners) {
        const emailMessage: EmailEnvelope = {
          ...
          vars: vars,
          subject: `[Update Alert Episode ${episodeNumberStr}] - ${episode.title}`,
        };

        const sms: SMSMessage = {
          message: `This is a message from Cast Operations. New note posted on alert episode ${episodeIdentifier}. To unsubscribe from this notification go to User Settings in Cast Operations Dashboard.`,
        };

        const callMessage: CallRequestMessage = {
          data: [
            {
              sayMessage: `This is a message from Cast Operations. New note posted on alert episode ${episodeIdentifier}. To see the note, go to Cast Operations Dashboard. To unsubscribe from this notification go to User Settings in Cast Operations Dashboard. Good bye.`,
            },
          ],
        };
