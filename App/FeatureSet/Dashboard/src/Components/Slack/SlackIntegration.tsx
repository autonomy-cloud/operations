...
const SlackIntegration: FunctionComponent<ComponentProps> = (
  ...
  if (!isUserAccountConnected && isProjectAccountConnected) {
    cardTitle = `You are disconnected from Slack (but Cast Operations is already installed in ${slackTeamName} team)`;
    cardDescription = `Connect your account with Slack to make the most out of Cast Operations.`;
    cardButtons = [
      ...
      {
        title: `Uninstall Cast Operations from Slack`,
        isLoading: isButtonLoading,
        buttonStyle: ButtonStyleType.DANGER,
        onClick: async () => {
          try {
