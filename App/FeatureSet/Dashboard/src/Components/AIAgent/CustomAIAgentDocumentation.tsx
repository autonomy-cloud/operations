        ...
        description={
          <div className="space-y-2 w-full mt-5">
            <CodeBlock
              ...
              code={`
# Run with Docker
docker run --name cast-operations-ai-agent --network host -e AI_AGENT_KEY=${props.aiAgentKey.toString()} -e AI_AGENT_ID=${props.aiAgentId.toString()} -e CAST_OPERATIONS_URL=${host.toString()} -d cast-operations/ai-agent:release
`}
            />
          </div>
        }
