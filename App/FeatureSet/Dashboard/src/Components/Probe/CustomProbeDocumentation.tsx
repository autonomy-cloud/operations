        ...
        description={
          <div className="space-y-2 w-full mt-5">
            <CodeBlock
              ...
              code={`
# Run with Docker
docker run --name cast-operations-probe --network host -e PROBE_KEY=${props.probeKey.toString()} -e PROBE_ID=${props.probeId.toString()} -e CAST_OPERATIONS_URL=${host.toString()} -d cast-operations/probe:release
`}
            />
            <div className="mt-4">
...
docker run --name cast-operations-probe --network host \\
  ...
  -e CAST_OPERATIONS_URL=${host.toString()} \\
