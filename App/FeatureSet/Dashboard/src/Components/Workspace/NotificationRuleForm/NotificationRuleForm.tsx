...
const NotificationRuleForm: FunctionComponent<ComponentProps> = (
  ...
  formFields = formFields.concat([
    ...
    },
    {
      ...
      required: true,
      description: `If your new channel name is "cast-operations-${props.eventType.toLowerCase()}-", then we will append the ${props.eventType === NotificationRuleEventType.Monitor ? "monitor name" : `${props.eventType} number`} in the end so, it'll look like "cast-operations-${props.eventType.toLowerCase()}-${props.eventType === NotificationRuleEventType.Monitor ? "monitor-name" : "X"}".`,
      fieldType: FormFieldSchemaType.Text,
      placeholder: `cast-operations-${props.eventType.toLowerCase()}-`,
    },
    {
      field: {
