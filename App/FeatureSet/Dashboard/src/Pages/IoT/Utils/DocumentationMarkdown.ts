...
export const IOT_INGESTION_METHODS: Array<IoTIngestionMethodOption> = [
  ...
  },
  {
    ...
    label: "MQTT",
    description:
      "Publish JSON readings straight to Cast Operations’ built-in MQTT endpoint — no SDK or collector.",
  },
];
...
Emit your readings as metrics using the \`iot_*\` names below. Within a minute or so the device appears under the **IoT** section of the Cast Operations dashboard. Keep \`iot.fleet.name\` stable: changing it registers a brand-new fleet.

## Quick Start — OpenTelemetry Collector

When many devices report through a gateway, run an OpenTelemetry Collector on the gateway and export to Cast Operations. (If your devices speak MQTT, switch to the **MQTT** tab above — no collector needed.) The \`resource\` processor stamps the fleet attributes; receive readings from your devices (OTLP, file logs, etc.) and forward them on:
...
2. In the Cast Operations dashboard, open the **IoT** section — your fleet should appear within a minute or so.
