...
- A **Cast Operations Telemetry Ingestion Key** — create one from *Project Settings → Telemetry Ingestion Keys* and copy the value.
...
1. **Collector** — a stock \`otel/opentelemetry-collector-contrib\` with a tuned config. It scrapes \`docker_stats\` for per-container CPU/memory, tails container logs, and tails the inventory snapshot file — stamping everything with \`docker.swarm.cluster.name\` before shipping to Cast Operations over OTLP.
...
Replace \`my-swarm\` with a friendly name for this cluster — it is how the cluster will appear in Cast Operations. Keep it stable: changing it registers a new cluster.
...
The agent deliberately stamps **only** \`docker.swarm.cluster.name\` (not \`host.name\` / \`container.runtime\`) so Cast Operations attributes the telemetry to this swarm cluster rather than auto-registering each node as a standalone Host or Docker Host.
