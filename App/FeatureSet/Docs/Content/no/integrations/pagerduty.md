...
Utløs en [PagerDuty](https://www.pagerduty.com)-hendelse hver gang en Cast Operations-hendelse opprettes, og løs den når Cast Operations løser. Nyttig når PagerDuty eier eskalerings- og vaktordningsplanene dine og du vil at Cast Operations-overvåkingen skal mate inn i den.

Denne integrasjonen er **utgående**: Cast Operations kaller PagerDutys [Events API v2](https://developer.pagerduty.com/docs/events-api-v2/overview/). Den bruker en Cast Operations **[Arbeidsflyt](/docs/workflows/index)** med en **Incident → On Create**-trigger og en **API-komponent**.

> Cast Operations har sin egen vaktordning og eskalering innebygd — se [On Call](/docs/on-call/incoming-call-policy). Bruk denne integrasjonen bare hvis du spesifikt vil at hendelser skal lande i PagerDuty i tillegg.
...
3. Legg til en **API**-blokk koblet til triggeren:
   ...
   **`dedup_key`** knytter denne PagerDuty-hendelsen til Cast Operations-hendelsen slik at du kan løse den senere. Å bruke Cast Operations-hendelse-ID-en holder den unik og forutsigbar.

4. **Lagre**, aktiver, og opprett en testhendelse. Et `202`-svar i arbeidsflytloggene betyr at PagerDuty aksepterte hendelsen.

## Steg 3 — Løs ved Cast Operations-løsning (anbefalt)
...
PagerDutys `severity` godtar `critical`, `error`, `warning` eller `info`. For å mappe fra Cast Operations-alvorlighetsgrader, legg til **Conditions**-grener på `{{Incident.incidentSeverity.name}}` før API-blokken og send en annen body fra hver.

## Innkommende (valgfritt)

For å gå den andre veien — åpne en Cast Operations-hendelse fra en PagerDuty-hendelse — legg til en **Webhook**-trigger-arbeidsflyt og pek en PagerDuty [V3-webhook](https://developer.pagerduty.com/docs/webhooks/v3-overview/) (eller en Events Orchestration) mot URL-en, og bruk deretter **Create Incident**. Se [det innkommende mønsteret](/docs/integrations/index#inbound-another-tool-sends-data-into-cast-operations).
