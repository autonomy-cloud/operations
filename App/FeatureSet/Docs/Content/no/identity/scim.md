...
Cast Operations støtter SCIM v2.0-protokollen for automatisert brukerklargjøring og avklargjøring. SCIM gjør det mulig for identitetsleverandører (IdP-er) som Azure AD, Okta og andre identitetssystemer for bedrifter å automatisk administrere brukertilgang til Cast Operations-prosjekter og statussider.

## Oversikt

SCIM-integrasjon gir følgende fordeler:

- **Automatisert brukerklargjøring**: Opprett automatisk brukere i Cast Operations når de tildeles i IdP-en din
- **Automatisert brukeravklargjøring**: Fjern automatisk brukere fra Cast Operations når de fratildelses i IdP-en din
- **Synkronisering av brukerattributter**: Hold brukerinformasjonen synkronisert mellom IdP-en og Cast Operations
- **Sentralisert tilgangsstyring**: Administrer Cast Operations-tilgang fra det eksisterende identitetsstyringssystemet ditt

## SCIM for prosjekter

Prosjekt-SCIM lar identitetsleverandører administrere teammedlemmer innenfor Cast Operations-prosjekter.
...
3. **Konfigurer identitetsleverandøren din**
   - Bruk SCIM-basis-URL: `https://latticeruntime.com/scim/v2/{scimId}`
...
1. **Brukertildeling i IdP**: Når en bruker tildeles Cast Operations i IdP-en din
2. **SCIM-klargjøring**: IdP kaller Cast Operations SCIM API for å opprette brukeren
3. **Teammedlemskap**: Brukeren legges automatisk til i konfigurerte standardteam
4. **Tilgang gitt**: Brukeren kan nå få tilgang til Cast Operations-prosjektet
5. **Brukerfratildeling**: Når brukeren fratildelses i IdP
6. **SCIM-avklargjøring**: IdP kaller Cast Operations SCIM API for å fjerne brukeren
...
1. **Naviger til statussideinnstillinger**
   ...
   - Gå til Cast Operations-statussiden din
...
3. **Konfigurer identitetsleverandøren din**
   - Bruk SCIM-basis-URL: `https://latticeruntime.com/status-page-scim/v2/{scimId}`
   - Konfigurer bearer-tokenautentisering med det angitte tokenet
   - Kartlegg brukerattributter (e-post er påkrevd)
...
1. **Brukertildeling i IdP**: Når en bruker tildeles Cast Operations-statussiden i IdP-en din
2. **SCIM-klargjøring**: IdP kaller Cast Operations SCIM API for å opprette abonnenten
...
5. **SCIM-avklargjøring**: IdP kaller Cast Operations SCIM API for å fjerne abonnenten
...
Microsoft Entra ID gir identitetsstyring på bedriftsnivå med robuste SCIM-klargjøringsfunksjoner. Følg disse detaljerte trinnene for å konfigurere SCIM-klargjøring med Cast Operations.

#### Forutsetninger

- Microsoft Entra ID-leietaker med Premium P1 eller P2-lisens (påkrevd for automatisk klargjøring)
- Cast Operations-konto med Scale-plan eller høyere
- Administratortilgang til både Microsoft Entra ID og Cast Operations

#### Trinn 1: Hent SCIM-konfigurasjon fra Cast Operations
...
1. I Cast Operations-bedriftsapplikasjonen din, gå til **Klargjøring**
2. Klikk **Kom i gang**
3. Sett **Klargjøringsmodus** til **Automatisk**
4. Under **Administratorlegitimasjon**:
   - **Leietaker-URL**: Skriv inn SCIM-basis-URL fra Cast Operations (f.eks. `https://latticeruntime.com/api/identity/scim/v2/{your-scim-id}`)
   - **Hemmelig token**: Skriv inn Bearer-tokenet fra Cast Operations
...
| Azure AD-attributt                                            | Cast Operations SCIM-attributt       | Påkrevd   |
| ------------------------------------------------------------- | ------------------------------ | --------- |
...
Hvis du aktiverte **Push Groups** i Cast Operations:
...
| Azure AD-attributt | Cast Operations SCIM-attributt |
| ------------------ | ------------------------ |
...
1. I Cast Operations-bedriftsapplikasjonen din, gå til **Brukere og grupper**
2. Klikk **+ Legg til bruker/gruppe**
3. Velg brukerne og/eller gruppene du vil klargjøre til Cast Operations
4. Klikk **Tildel**
...
Okta gir fleksibel identitetsstyring med utmerket SCIM-støtte. Følg disse detaljerte trinnene for å konfigurere SCIM-klargjøring med Cast Operations.

#### Forutsetninger

- Okta-leietaker med klargjøringsfunksjoner (Lifecycle Management-funksjon)
- Cast Operations-konto med Scale-plan eller høyere
- Administratortilgang til både Okta og Cast Operations
...
3. Finn og velg den eksisterende Cast Operations-applikasjonen din
...
5. Skriv inn "Cast Operations" som appnavn
6. Fullfør SAML-konfigurasjonen (se SSO-dokumentasjonen)
7. Klikk **Fullfør**

#### Trinn 3: Aktiver SCIM-klargjøring

1. I Cast Operations-applikasjonen din, gå til fanen **Generelt**
...
5. Konfigurer følgende:
   - **SCIM-konnektorbasis-URL**: Skriv inn SCIM-basis-URL fra Cast Operations (f.eks. `https://latticeruntime.com/api/identity/scim/v2/{your-scim-id}`)
...
| Okta-attributt     | Cast Operations SCIM-attributt        | Retning      |
| ------------------ | ------------------------------- | ------------ |
...
2. Filtrer etter hendelser relatert til Cast Operations-applikasjonen din
3. Bekreft at klargjøringshendelser er vellykkede
4. Sjekk Cast Operations for å bekrefte at brukere er opprettet
...
- **Feil: 401 Uautorisert**: Generer Bearer-tokenet på nytt i Cast Operations og oppdater Okta

---

### Andre identitetsleverandører

Cast Operations SCIM-implementasjon følger SCIM v2.0-spesifikasjonen og skal fungere med alle kompatible identitetsleverandører. Generelle konfigurasjonstrinn:

1. **SCIM-basis-URL**: `https://latticeruntime.com/api/identity/scim/v2/{scim-id}` (for prosjekter) eller `https://latticeruntime.com/api/identity/status-page-scim/v2/{scim-id}` (for statussider)
...
Når en bruker avklargjøres (enten via DELETE-forespørsel eller ved å sette `active: false`), fjernes de fra teamene konfigurert i SCIM-innstillingene. Brukerkontoen forblir i Cast Operations, men mister tilgang til prosjektet.

### Kan jeg bruke SCIM uten SSO?

Ja, SCIM og SSO er uavhengige funksjoner. Du kan bruke SCIM for brukerklargjøring mens brukere logger inn med Cast Operations-passordene sine eller en annen autentiseringsmetode.

### Hvordan håndterer jeg brukere som allerede finnes i Cast Operations?

Når SCIM prøver å opprette en bruker som allerede finnes (matchet etter e-post), vil Cast Operations ganske enkelt legge dem til de konfigurerte standardteamene fremfor å opprette en duplikatbruker.
