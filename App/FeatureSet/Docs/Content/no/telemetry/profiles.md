# Send kontinuerlige profileringsdata til Cast Operations

## Oversikt

Kontinuerlig profilering er den fjerde søylen i observerbarhet ved siden av logger, metrikker og spor. Profiler fanger opp hvordan applikasjonen bruker CPU-tid, allokerer minne og bruker systemressurser på funksjonsnivå. Cast Operations henter inn profileringsdata via OpenTelemetry Protocol (OTLP) og lagrer det ved siden av de andre telemetrirammene for enhetlig analyse.

Med profileringsdata i Cast Operations kan du identifisere hete funksjoner som bruker CPU, oppdage minnelekkasjer, finne flaskehalser og korrelere ytelsesproblemer med spesifikke spor og spans.

## Støttede profiltyper

Cast Operations støtter følgende profiltyper:
...
Cast Operations aksepterer profileringsdata over både gRPC og HTTP ved hjelp av OTLP-profilprotokollen.

| Protokoll | Endepunkt                                            |
| --------- | ---------------------------------------------------- |
| gRPC      | `your-operations-host:4317` (standard OTLP gRPC-port) |
| HTTP      | `https://your-operations-host/otlp/v1/profiles`       |

**Miljøvariabler**

Sett følgende miljøvariabler for å peke profileringsverktøyet mot Cast Operations:
...
Hvis du selvhoster Cast Operations, erstatt endepunktet med din egen vert (f.eks. `http(s)://YOUR-OPERATIONS-HOST/otlp`). For gRPC, koble direkte til port 4317 på Cast Operations-verten din.

## Instrumenteringsguide

### Bruke Grafana Alloy (eBPF-basert profilering)

Grafana Alloy (tidligere Grafana Agent) kan samle inn CPU-profiler fra alle prosesser på en Linux-vert ved hjelp av eBPF, uten kodeendringer. Konfigurer den til å eksportere via OTLP til Cast Operations.
...
For Go-applikasjoner kan du bruke standardpakken `net/http/pprof` ved siden av en OTLP-eksporter. Konfigurer kontinuerlig profilering ved periodisk å samle inn pprof-data og videresende det til Cast Operations.
...
func collectProfile() {
    ...
    // Konverter pprof-utdata til OTLP-format og send til Cast Operations
}
```
...
For kontinuerlig profilering, kjør py-spy ved siden av applikasjonen din og konfigurer OpenTelemetry Collector til å hente inn og videresende profilene til Cast Operations.

## Bruke OpenTelemetry Collector

Du kan bruke OpenTelemetry Collector som en proxy for å motta profiler fra applikasjonene dine og videresende dem til Cast Operations.
...
Cast Operations rendrer profildata som interaktive flamegrafer. Hver stolpe representerer en funksjon i kallstakken, og bredden er proporsjonal med tiden eller ressursene som forbrukes. Du kan klikke på en hvilken som helst funksjon for å zoome inn og se dens kallere og kallede.
...
Profiler i Cast Operations kan korreleres med distribuerte spor. Når en profil inkluderer spor- og span-ID-er (via OTLP-lenketabellen), kan du navigere direkte fra et tregt spor-span til den tilsvarende CPU- eller minneprofilen for å forstå nøyaktig hvilken kode som ble kjørt.
...
Oppbevaring av profildata konfigureres per telemetritjeneste i Cast Operations-prosjektinnstillingene. Standard oppbevaringsperiode er 15 dager. Data slettes automatisk etter at oppbevaringsperioden utløper.
...
Ta kontakt med support@latticeruntime.com hvis du trenger hjelp med å sette opp profilering med Cast Operations.
