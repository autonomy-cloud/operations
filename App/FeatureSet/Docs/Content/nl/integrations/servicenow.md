...
Open automatisch een [ServiceNow](https://www.servicenow.com)-incident wanneer een Cast Operations-incident wordt aangemaakt — zodat ITSM en monitoring gelijke tred houden.

Deze integratie is **outbound**: Cast Operations roept de ServiceNow [Table API](https://docs.servicenow.com/bundle/utah-application-development/page/integrate/inbound-rest/concept/c_TableAPI.html) aan. Ze maakt gebruik van een Cast Operations **[Workflow](/docs/workflows/index)** met een **Incident → On Create**-trigger en een **API-component**.
...
2. Ga in Cast Operations naar **Workflows → Global Variables → Create**, geef het de naam `SERVICENOW_AUTH`, plak de base64-string, en zet **Is Secret** aan.
...
3. Voeg een **API**-blok toe verbonden met de trigger:
   ...
   `correlation_id` houdt een koppeling bij naar het Cast Operations-incident — handig als je later een oplossen-stap toevoegt. ServiceNow `urgency`/`impact` gebruiken `1` (hoog), `2` (gemiddeld), `3` (laag).
...
## Stap 3 — Oplossen bij Cast Operations-oplossing (optioneel)
...
2. Om het juiste ServiceNow-record bij te werken heb je de `sys_id` nodig. Sla hem op bij het Cast Operations-incident in Stap 2 (lees `{{CreateRecord.response-body.result.sys_id}}` en schrijf hem naar een label met **Update Incident**), of zoek het record eerst op met een `GET` op `/api/now/table/incident?sysparm_query=correlation_id=cast-operations-{{Incident._id}}`.
3. Voeg een **API**-blok toe: **Method** `PATCH`, **URL** `https://your-instance.service-now.com/api/now/table/incident/<sys_id>`, body `{ "state": "6", "close_code": "Resolved by monitoring", "close_notes": "Resolved in Cast Operations" }` (`state` `6` = Resolved in de standaard ITIL-workflow).
