# FluentBit gebruiken om telemetriegegevens naar Cast Operations te sturen

## Overzicht

U kunt de [FluentBit](https://docs.fluentbit.io/manual)-plugin gebruiken om logboeken en telemetriegegevens te verzamelen van uw applicaties en diensten. De plugin stuurt de telemetriegegevens naar de Cast Operations OpenTelemetry HTTP Collector. U kunt de opentelemetry-uitvoerplugin van FluentBit gebruiken om de telemetriegegevens naar de Cast Operations OpenTelemetry HTTP Collector te sturen. Deze plugin is hier te vinden: https://docs.fluentbit.io/manual/pipeline/outputs/opentelemetry

## Aan de slag

FluentBit ondersteunt honderden gegevensbronnen en u kunt logboeken en telemetrie van elk van deze bronnen verwerken in Cast Operations. Enkele populaire bronnen zijn:
...
U kunt de volgende configuratie gebruiken om de telemetriegegevens naar de Cast Operations OpenTelemetry HTTP Collector te sturen. U kunt deze configuratie toevoegen aan het FluentBit-configuratiebestand. Het configuratiebestand bevindt zich doorgaans op `/etc/fluent-bit/fluent-bit.yaml`. Zo ziet een uitvoersectie van het configuratiebestand eruit:
...
**Als u Cast Operations zelf host**: Als u Cast Operations zelf host, kunt u de `host` vervangen door de host van uw Cast Operations-instantie. Als u op een http-server host en niet https, kunt u de `port` vervangen door de poort van uw Cast Operations-instantie (waarschijnlijk poort 80).
