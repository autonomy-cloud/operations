...
O [Zabbix](https://www.zabbix.com) monitora seus servidores e rede; o Cast Operations gerencia sua resposta a incidentes, plantão e páginas de status. Conecte os dois e cada problema do Zabbix vira automaticamente um incidente no Cast Operations — para que as pessoas certas sejam acionadas e sua página de status se mantenha fiel.

Esta integração é de **entrada**: o Zabbix envia problemas para o Cast Operations. Ela usa um **tipo de mídia webhook** no Zabbix de um lado e um **[Workflow](/docs/workflows/index)** do Cast Operations do outro. Sem plugins, sem serviços extras.
...
2. Uma **ação** do Zabbix instrui o tipo de mídia **Cast Operations** a enviar o evento.
3. O script do tipo de mídia faz POST de um pequeno payload JSON para uma URL do workflow do Cast Operations.
...
- Seu servidor Zabbix deve conseguir alcançar sua instância do Cast Operations via HTTPS.
- Um projeto no Cast Operations onde você possa criar workflows.

## Parte 1 — Construa o workflow do Cast Operations
...
4. Arraste um bloco **Create Incident** e conecte-o à saída **Yes** do bloco Conditions. Preencha:
   ...
   - **Severity**: escolha a severidade de incidente do Cast Operations que desejar (você pode refiná-la depois com mais ramificações de Conditions que mapeiam severidades do Zabbix).
...
### Passo 1: Crie o tipo de mídia Cast Operations
...
1. Vá em **Users → Users → Create user**. Nomeie-o `Cast Operations Webhook`, dê a ele um papel que possa receber notificações (ex.: **User role**) e adicione-o a um grupo de usuários.
...
### Passo 3: Envie problemas para o Cast Operations com uma ação
...
4. Na aba **Operations**, adicione uma operação que envie para **User: Cast Operations Webhook** pelo tipo de mídia **Cast Operations**.
...
1. De volta ao workflow do Cast Operations, ative **Enabled**.
...
4. Verifique **Incidents** no Cast Operations — seu problema do Zabbix agora é um incidente.
...
Como a resolução depende de como você modela os estados de incidente no seu projeto, mantenha o caminho de **criação** como o núcleo confiável e adicione o caminho de resolução depois de confirmar que os eventos fluem corretamente. Veja [Componentes → Componentes de dados do Cast Operations](/docs/workflows/components#cast-operations-data-components).

## Mapeando severidades do Zabbix (opcional)

As severidades do Zabbix (`Not classified`, `Information`, `Warning`, `Average`, `High`, `Disaster`) chegam como `{{Zabbix.Request Body.severity}}`. Para mapeá-las às severidades de incidente do Cast Operations, adicione ramificações **Conditions** antes de **Create Incident** — por exemplo, roteie `Disaster` e `High` para um incidente "Crítico" e todo o resto para "Maior". Construa um bloco **Create Incident** por ramificação.
...
- Uma resposta não-2xx do Cast Operations é exposta pelo `throw` no script — verifique se a URL do workflow está exatamente correta.
...
- Se você executa o Cast Operations auto-hospedado atrás de um firewall, permita que o IP de saída do seu servidor Zabbix o alcance via HTTPS.
