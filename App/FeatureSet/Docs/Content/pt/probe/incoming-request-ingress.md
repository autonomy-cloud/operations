...
Quando `PROBE_INGRESS_PORT` está definido, a probe vincula um listener HTTP adicional nessa porta. O listener aceita os mesmos caminhos de URL de `secretkey` dos endpoints públicos do Cast Operations:
...
A probe então faz proxy da requisição para a sua instância do Cast Operations, preservando o método, o corpo e os cabeçalhos de requisição (menos cabeçalhos hop-by-hop como `Host`, `Connection`, `Content-Length`, etc.). A probe anexa automaticamente um cabeçalho `Cast Operations-Probe-Id` para que a requisição seja atribuída à probe de encaminhamento.
...
- **Resposta síncrona, encaminhamento assíncrono.** A probe reconhece a requisição de entrada imediatamente com um `200` e encaminha para o Cast Operations em segundo plano. Seu serviço não precisa aguardar a conclusão do encaminhamento.
- **Cabeçalhos são preservados.** Todos os cabeçalhos exceto os hop-by-hop (`Host`, `Connection`, `Content-Length`, `Transfer-Encoding`, `Keep-Alive`, `Proxy-Authenticate`, `Proxy-Authorization`, `TE`, `Trailer`, `Upgrade`) são passados. A probe adiciona um cabeçalho `Cast Operations-Probe-Id` se identificando.
