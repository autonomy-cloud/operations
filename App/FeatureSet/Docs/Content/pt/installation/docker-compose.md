# Implantar o Cast Operations completamente gratuito com Docker Compose

Se preferir hospedar o Cast Operations no seu próprio servidor, você pode usar o Docker Compose para implantar uma instância de servidor único do Cast Operations no Debian, Ubuntu ou RHEL. Esta opção oferece mais controle e personalização sobre sua instância, mas também requer mais habilidades técnicas e recursos para implantá-la e mantê-la.

#### Escolha Seus Requisitos de Sistema

Dependendo do seu uso e orçamento, você pode escolher entre diferentes requisitos de sistema para o seu servidor. Para desempenho ideal, sugerimos usar o Cast Operations com:
...
- **Homelab / Requisitos Mínimos**
  - Se você quiser executar o Cast Operations para uso pessoal ou experimental em um ambiente doméstico (alguns dos nossos usuários até têm instalado em RaspberryPi), você pode usar os requisitos de homelab:
    - 8 GB RAM
    - 4 Núcleos
    - 20 GB de Disco
    - Docker e Docker Compose instalados
...
Para instalar o Cast Operations:
...
### Acessando o Cast Operations

O Cast Operations deve executar em: http://localhost. Você precisa registrar uma nova conta para a sua instância começar a usá-lo.

### Configurando Certificados TLS/SSL

O Cast Operations **não** suporta a configuração de certificados SSL/TLS. Você precisa configurar certificados SSL/TLS por conta própria.
...
3. Aponte o proxy reverso para o servidor do Cast Operations.
...
Idealmente, não implante o Cast Operations em produção com docker-compose. Recomendamos fortemente o uso de Kubernetes. Há um helm chart disponível para o Cast Operations [aqui](https://artifacthub.io/packages/helm/autonomy-cloud/operations).

Se ainda quiser implantar o Cast Operations em produção com docker-compose, considere o seguinte:

- **SSL/TLS**: Configure certificados SSL/TLS. O Cast Operations não suporta a configuração de certificados SSL/TLS. Você precisa configurar os certificados SSL/TLS por conta própria. Consulte acima.
...
- **Atualizações**: Atualize o Cast Operations regularmente. Lançamos atualizações diariamente. Recomendamos que você atualize o software pelo menos uma vez por semana se estiver executando em produção.

### Atualizando o Cast Operations
...
- Na nossa configuração Docker, empregamos um driver de log local. O Cast Operations, particularmente dentro dos contêineres de probe e ingest, gera uma quantidade substancial de logs. Para evitar que seu armazenamento fique cheio, é crucial limitar o armazenamento de log no Docker. Para instruções detalhadas sobre como fazer isso, consulte a documentação oficial do Docker [aqui](https://docs.docker.com/config/containers/logging/local/).

### Desinstalando o Cast Operations

Para desinstalar o Cast Operations, execute o seguinte comando:

```
npm run down
```

Isso irá parar e remover todos os contêineres, redes e volumes criados pelo Cast Operations. Não removerá o arquivo `config.env` ou o repositório clonado.
