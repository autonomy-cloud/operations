...
O Cast Operations suporta Single Sign-On (SSO) baseado em SAML 2.0 para autenticação empresarial. O SSO permite que os membros da sua equipe façam login no Cast Operations usando as credenciais do provedor de identidade (IdP) da sua organização, fornecendo gerenciamento centralizado de acesso e segurança aprimorada.
...
- **Fadiga de Senhas Reduzida**: Os usuários não precisam lembrar uma senha separada do Cast Operations
...
3. **Obter Metadados SSO do Cast Operations**
...
O Keycloak é uma solução popular de código aberto para gerenciamento de identidade e acesso. Siga estas etapas para configurar o Keycloak como seu provedor de identidade SAML para o Cast Operations.

### Pré-requisitos

- Uma instância do Keycloak em execução com um realm configurado
- Acesso de administrador ao Keycloak e ao Cast Operations
- Conta do Cast Operations com suporte a SSO

### Passo 1: Configurar o SSO do Cast Operations
...
3. Clique em **Create SSO** e preencha o seguinte:
   - **Nome**: Um nome descritivo (ex.: `my-project-cast-operations`)
...
4. Copie o valor do certificado e cole-o no Cast Operations no seguinte formato:
...
4. Defina **Client ID** como o valor de **Identifier (Entity ID)** do **View SSO Config** do Cast Operations
5. Defina **Valid Redirect URIs** para a sua URL do Cast Operations
6. Defina **Root URL** para a URL base do Cast Operations
7. Cole o **Reply URL (Assertion Consumer Service URL)** do Cast Operations no campo **Assertion Consumer Service POST Binding URL**
...
1. Salve todas as configurações no Keycloak e no Cast Operations
2. Tente fazer login no Cast Operations usando SSO
3. Você deve ser redirecionado para a página de login do Keycloak e de volta ao Cast Operations após autenticação bem-sucedida
...
O Microsoft Entra ID é o serviço de gerenciamento de identidade e acesso baseado em nuvem da Microsoft. Siga estas etapas para configurar o Entra ID como seu provedor de identidade SAML para o Cast Operations.
...
3. Em **Basic SAML Configuration**, clique em **Edit** e defina:
   - **Identifier (Entity ID)**: Cole o **Identifier (Entity ID)** do **View SSO Config** do Cast Operations
   - **Reply URL (Assertion Consumer Service URL)**: Cole o **Reply URL** do **View SSO Config** do Cast Operations
4. Clique em **Save**
...
6. Na seção **Set up Cast Operations**, copie:
   - **Login URL** — cole isso como o **Sign On URL** no Cast Operations
   - **Azure AD Identifier** — cole isso como o **Issuer** no Cast Operations
7. Volte ao Cast Operations e cole o certificado e as URLs, depois salve
...
1. Salve todas as configurações no Entra ID e no Cast Operations
2. Tente fazer login no Cast Operations usando SSO
3. Você deve ser redirecionado para a página de login da Microsoft e de volta ao Cast Operations após autenticação bem-sucedida

### Solução de Problemas do Microsoft Entra ID

- **AADSTS700016 Error**: O Identifier (Entity ID) no Entra ID não corresponde ao Cast Operations — verifique se ambos os valores são idênticos
...
- **Name ID Mismatch**: Certifique-se de que a declaração Name ID está definida como um endereço de email que corresponde ao email do usuário no Cast Operations

---

## Configuração SAML do Okta

O Okta é uma plataforma de identidade amplamente utilizada que fornece capacidades robustas de SSO SAML. Siga estas etapas para configurar o Okta como seu provedor de identidade SAML para o Cast Operations.
...
3. Clique em **Create SSO** e preencha o seguinte:
   ...
   - **Sign On URL**: Você obterá isso do Okta no [Passo 3](#passo-3-copiar-metadados-saml-do-okta-para-o-cast-operations)
   - **Issuer**: Você obterá isso do Okta no [Passo 3](#passo-3-copiar-metadados-saml-do-okta-para-o-cast-operations)
   - **Certificate**: Você obterá isso do Okta no [Passo 3](#passo-3-copiar-metadados-saml-do-okta-para-o-cast-operations)
   - **Signature Algorithm**: `RSA-SHA-256`
   - **Digest Algorithm**: `SHA256`
...
5. Insira "Cast Operations" como o **App name** e clique em **Next**
6. Na seção **SAML Settings**, configure:
   - **Single sign-on URL**: Cole o **Reply URL (Assertion Consumer Service URL)** do **View SSO Config** do Cast Operations
   - **Audience URI (SP Entity ID)**: Cole o **Identifier (Entity ID)** do **View SSO Config** do Cast Operations
...
### Passo 3: Copiar Metadados SAML do Okta para o Cast Operations
...
3. A partir do XML de metadados ou dos detalhes da aba **Sign On**:
   - Copie a **Sign On URL** (também chamada de **Identity Provider Single Sign-On URL**) — cole isso como o **Sign On URL** no Cast Operations
   - Copie o **Issuer** (também chamado de **Identity Provider Issuer**) — cole isso como o **Issuer** no Cast Operations
4. Faça o download do certificado de assinatura:
   ...
   - Cole o certificado no Cast Operations (incluindo as linhas `BEGIN CERTIFICATE` e `END CERTIFICATE`)
5. Salve a configuração SSO do Cast Operations
...
1. Salve todas as configurações no Okta e no Cast Operations
2. Tente fazer login no Cast Operations usando SSO
3. Você deve ser redirecionado para a página de login do Okta e de volta ao Cast Operations após autenticação bem-sucedida

### Solução de Problemas do Okta

- **404 or Invalid SSO URL**: Verifique se a **Single sign-on URL** no Okta corresponde exatamente ao **Reply URL** do Cast Operations
- **Audience Mismatch**: Certifique-se de que o **Audience URI** no Okta corresponde exatamente ao **Identifier (Entity ID)** do Cast Operations
...
A implementação SSO do Cast Operations usa o protocolo SAML 2.0 e deve funcionar com qualquer provedor de identidade compatível. As etapas gerais de configuração são:

1. No Cast Operations, crie uma configuração SSO e anote o **Identifier (Entity ID)** e o **Reply URL (Assertion Consumer Service URL)** do botão **View SSO Config**
2. No seu provedor de identidade, crie um aplicativo SAML usando:
   - **Assertion Consumer Service URL / Reply URL**: Da configuração SSO do Cast Operations
   - **Entity ID / Audience URI**: Da configuração SSO do Cast Operations
   - **Name ID Format**: Endereço de email
3. Do seu provedor de identidade, copie o seguinte para o Cast Operations:
...
O Cast Operations atualmente não suporta o mapeamento de funções SAML do seu provedor de identidade. O acesso baseado em funções deve ser configurado separadamente nas **Project Settings** > **SSO** do Cast Operations, onde você pode atribuir funções padrão para usuários SSO.
