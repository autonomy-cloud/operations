...
Instale o aplicativo nativo **Cast Operations On-Call** para Android a partir da Google Play Store, ou faça o sideload do APK diretamente em dispositivos sem Google Play.
...
2. Pesquise por **"Cast Operations On-Call"**, ou abra este link no seu dispositivo:
   [https://github.com/autonomy-cloud/operations/releases](https://github.com/autonomy-cloud/operations/releases)
3. Toque em **Instalar**.
4. Após a instalação, toque em **Abrir** ou inicie o **Cast Operations On-Call** pela gaveta de aplicativos.
...
4. Inicie o **Cast Operations On-Call** pela gaveta de aplicativos.

O APK é compilado e assinado pelo Cast Operations a partir da mesma fonte da versão da Play Store. As atualizações do aplicativo não são automáticas quando feito sideload — baixe o APK mais recente pelo link acima sempre que uma nova versão for lançada.
...
1. Abra **Configurações → Apps → Cast Operations On-Call → Notificações** e confirme que todas as categorias estão habilitadas.
2. Abra **Configurações → Apps → Cast Operations On-Call → Bateria** e escolha **Sem restrições** (ou desative a otimização de bateria) para que o sistema operacional não atrase as notificações em segundo plano.
3. Permita que o aplicativo seja executado em segundo plano e desative quaisquer restrições do "Economia de Dados" para ele.
4. Se você usa dispositivos Samsung, também desative **Configurações → Cuidado com o dispositivo → Bateria → Limites de uso em segundo plano** para o Cast Operations On-Call.
5. Adicione o Cast Operations On-Call a quaisquer listas de exceção do **Do Not Disturb** (Não perturbar) para que os chamados continuem tocando durante seu turno de plantão.

## Atualizações

**Google Play:**

- As atualizações são instaladas automaticamente. Para acioná-las manualmente, abra **Play Store → Perfil → Gerenciar apps e dispositivo → Atualizações disponíveis → Cast Operations On-Call → Atualizar**.
...
1. **Mantenha pressionado** o ícone do **Cast Operations On-Call** e toque em **Desinstalar**.
2. Ou abra **Configurações → Apps → Cast Operations On-Call → Desinstalar**.
...
- Confirme que as notificações estão habilitadas em **Configurações → Apps → Cast Operations On-Call → Notificações**.
- Desative a otimização de bateria para o Cast Operations On-Call (veja Notificações Push acima).
- Certifique-se de que o Do Not Disturb está desativado, ou que o Cast Operations On-Call está na lista de exceções.
