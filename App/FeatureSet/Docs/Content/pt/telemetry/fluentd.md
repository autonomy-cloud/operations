# Usar o Fluentd para enviar dados de telemetria para o Cast Operations

## Visão Geral

Você pode usar o plugin [Fluentd](https://www.fluentd.org/) para coletar logs e dados de telemetria dos seus aplicativos e serviços. O plugin envia os dados de telemetria para a Fonte HTTP do Cast Operations. Você pode usar o plugin de saída http do fluentd para enviar os dados de telemetria para a Fonte HTTP do Cast Operations. Este plugin pode ser encontrado aqui: https://docs.fluentd.org/output/http

## Primeiros Passos

O Fluentd suporta centenas de fontes de dados e você pode ingerir logs de qualquer uma dessas fontes no Cast Operations. Algumas das fontes populares incluem:
...
Você pode usar a seguinte configuração para enviar os dados de telemetria para a Fonte HTTP do Cast Operations. Você pode adicionar esta configuração ao arquivo de configuração do fluentd. O arquivo de configuração geralmente está localizado em `/etc/fluentd/fluent.conf` ou `/etc/td-agent/td-agent.conf`.

Você precisa substituir `YOUR_SERVICE_TOKEN` pelo token que você criou na etapa anterior. Você também precisa substituir `YOUR_SERVICE_NAME` pelo nome do seu serviço. O nome do serviço pode ser qualquer nome que você quiser. Se o serviço não existir no Cast Operations, ele será criado automaticamente.
...
**Se você estiver auto-hospedando o Cast Operations**: Se você estiver auto-hospedando o Cast Operations, pode substituir o `endpoint_url` pela URL da sua instância do Cast Operations. `http(s)://SEU_HOST_CAST_OPERATIONS/fluentd/logs`
