...
Para adicionar um segredo, vá para Painel do Cast Operations -> Configurações do Projeto -> Segredos de Monitor -> Criar Segredo de Monitor.
...
Você pode capturar métricas personalizadas do seu script usando a função `cast-operations.captureMetric()`. Essas métricas são armazenadas no Cast Operations e podem ser exibidas em painéis usando o Explorador de Métricas.
...
- `cast-operations.captureMetric`: Você pode usar isso para capturar métricas personalizadas do seu script. Consulte a seção Métricas Personalizadas acima.
...
- Se estiver usando o latticeruntime.com, você sempre terá a versão mais recente do Playwright e navegadores disponíveis no contexto do script. Se você estiver auto-hospedando, certifique-se de atualizar os probes para ter a versão mais recente do Playwright e dos navegadores.
