...
O monitoramento manual permite criar monitores cujo status é gerenciado inteiramente à mão ou por meio da API. O Cast Operations não realiza nenhuma verificação automatizada — você controla o status do monitor diretamente.

## Visão Geral

Os monitores manuais são espaços reservados que você atualiza você mesmo. Isso é útil para:

- Integração com ferramentas de monitoramento externas que atualizam o status via API do Cast Operations
...
- **Painel** — Altere o status do monitor diretamente no Painel do Cast Operations
- **API** — Atualize o status do monitor programaticamente usando a API do Cast Operations
...
| Status orientado por API               | Permita que ferramentas externas atualizem o status do monitor via API do Cast Operations               |
| Espaços reservados da página de status | Mostre componentes na sua página de status que são gerenciados fora do Cast Operations                  |
