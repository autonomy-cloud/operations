...
`PROBE_INGRESS_PORT`가 설정되면 프로브는 해당 포트에 추가적인 HTTP 리스너를 바인딩합니다. 리스너는 공개 Cast Operations 엔드포인트와 동일한 `secretkey` URL 경로를 허용합니다:
...
그런 다음 프로브는 요청을 Cast Operations 인스턴스로 프록시하며, 메서드, 본문 및 요청 헤더를 보존합니다 (`Host`, `Connection`, `Content-Length` 등 홉바이홉 헤더 제외). 프로브는 요청이 전달 프로브에 귀속되도록 `Cast Operations-Probe-Id` 헤더를 자동으로 첨부합니다.
...
- **동기 응답, 비동기 전달.** 프로브는 인바운드 요청을 즉시 `200`으로 확인하고 백그라운드에서 Cast Operations으로 전달합니다. 서비스는 전달이 완료될 때까지 기다릴 필요가 없습니다.
- **헤더가 보존됩니다.** 홉바이홉 헤더 (`Host`, `Connection`, `Content-Length`, `Transfer-Encoding`, `Keep-Alive`, `Proxy-Authenticate`, `Proxy-Authorization`, `TE`, `Trailer`, `Upgrade`) 를 제외한 모든 헤더가 통과됩니다. 프로브는 자신을 식별하는 `Cast Operations-Probe-Id` 헤더를 추가합니다.
