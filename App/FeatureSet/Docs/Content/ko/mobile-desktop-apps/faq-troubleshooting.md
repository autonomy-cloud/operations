...
Cast Operations 모바일 및 데스크톱 앱에 대한 자주 묻는 질문과 해결 방법입니다.

## Cast Operations은 앱을 어떻게 배포하나요?

- **모바일 (iOS 및 Android):** Cast Operations은 **Cast Operations On-Call**이라는 네이티브 앱을 제공합니다. [Apple App Store](https://github.com/autonomy-cloud/operations/releases)와 [Google Play](https://github.com/autonomy-cloud/operations/releases)에 게시되어 있습니다. Google Play가 없는 Android 기기를 위해 서명된 [APK 다운로드](https://github.com/autonomy-cloud/operations/releases/latest/download/cast-operations-on-call-android-app.apk)도 제공됩니다.
- **데스크톱 (Windows, macOS, Linux):** Cast Operations 웹 대시보드는 프로그레시브 웹 앱(PWA)입니다. Chromium 기반 브라우저 또는 Safari에서 직접 데스크톱 애플리케이션으로 설치할 수 있으며, 스토어 계정이 필요하지 않습니다.
...
네. Cast Operations On-Call 앱은 무료로 설치할 수 있습니다. 기존 Cast Operations 계정으로 로그인하면 됩니다.

### 자체 호스팅 Cast Operations 인스턴스에서 앱을 사용할 수 있나요?

네. 첫 실행 시 앱이 **서버 URL**을 요청합니다. 자체 호스팅 인스턴스의 URL을 입력하세요(예: `https://operations.example.com`). 앱은 로그인을 허용하기 전에 서버에 연결 가능한지 확인합니다.
...
1. OS 수준에서 **Cast Operations On-Call**에 대한 알림이 활성화되어 있습니다.
...
5. **자체 호스팅 전용:** Cast Operations 인스턴스에 푸시 알림이 구성되어 있습니다. [Push Notifications](/docs/self-hosted/push-notifications) 가이드를 참조하세요.
...
네. 필요한 만큼 많은 기기에서 동일한 Cast Operations 계정으로 로그인하세요. 각 기기는 자체 푸시 알림을 받습니다.

### 어떻게 제거하나요?

- **iOS:** 아이콘을 길게 누른 후 → **앱 제거** → **앱 삭제**.
- **Android:** 아이콘을 길게 누른 후 → **제거**, 또는 **설정 → 앱 → Cast Operations On-Call → 제거**.

Cast Operations 계정과 데이터는 서버에 저장되어 있으며 앱을 제거해도 삭제되지 않습니다.
...
- **즉시 업데이트** — 배포하는 즉시 앱이 Cast Operations 인스턴스와 동기화됩니다.
...
2. Cast Operations 인스턴스가 유효한 인증서를 사용하여 HTTPS로 제공되는지 확인하세요.
...
- **Windows:** **설정 → 앱 → Cast Operations → 제거**, 또는 시작 메뉴 항목을 마우스 오른쪽 버튼으로 클릭하세요.
...
- 배터리 최적화를 비활성화하세요: **설정 → 앱 → Cast Operations On-Call → 배터리 → 제한 없음**.
- 앱에 대한 데이터 절약 모드를 비활성화하세요.
- Samsung 기기에서는 Cast Operations On-Call에 대한 **디바이스 케어 → 배터리 → 백그라운드 사용 제한**을 해제하세요.

**푸시 알림이 지연되거나 누락되는 경우 (iOS):**

- 앱을 강제 종료하지 마세요 — iOS가 백그라운드 전송을 일시 중지할 수 있습니다.
- 온콜 근무 중에는 저전력 모드를 비활성화하세요.
- 활성 집중 모드의 허용 목록에 Cast Operations On-Call을 추가하세요.

**Face ID / Touch ID / 지문이 작동하지 않는 경우:**

- OS 설정에서 생체 인식이 등록되어 있는지 확인하세요.
- Cast Operations On-Call 앱 내부의 **설정** 화면에서 생체 인증 잠금 해제를 다시 활성화하세요.
...
- Cast Operations 인스턴스가 HTTPS로 제공되는지 확인하세요.
...
- [Cast Operations GitHub 저장소](https://github.com/autonomy-cloud/operations)에 이슈를 등록하세요.
- Cast Operations 대시보드를 통해 지원팀에 문의하세요.
