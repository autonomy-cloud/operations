...
외부 상태 페이지 모니터링을 통해 의존하는 타사 상태 페이지를 모니터링하고, 사용하는 서비스가 장애 또는 성능 저하를 겪을 때 알림을 받을 수 있습니다. Cast Operations은 주기적으로 외부 상태 페이지(AWS, GCP, Azure, GitHub, OpenAI, Anthropic 등)를 확인하고 상태를 평가합니다.
...
Cast Operations은 다음 방법으로 상태 페이지 모니터링을 지원합니다:
...
**자동**으로 설정하면 Cast Operations은 상태 페이지 형식을 다음 순서로 자동 감지합니다:
...
상태 페이지의 공급자 유형을 선택합니다. Cast Operations이 형식을 자동으로 감지하도록 **자동** (기본값)을 사용하거나, 알고 있는 경우 **Atlassian Statuspage**, **incident.io**, **RSS** 또는 **Atom**을 지정합니다.
...
기본적으로 Cast Operations은 단순한 도달 가능성이 아니라 상태 페이지에 실제로 중요한 것, 즉 활성 인시던트와 구성 요소 상태를 기준으로 시드합니다:
