...
에이전트는 Docker Hub에서 `cast-operations/docker-agent:release`로 게시됩니다. 전체 `docker run` 및 `docker compose` 예시에 대한 [Docker 에이전트 설치 가이드](https://github.com/autonomy-cloud/operations/tree/master/DockerAgent)를 참조하십시오.
