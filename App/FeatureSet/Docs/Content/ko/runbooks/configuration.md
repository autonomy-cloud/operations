...
Bash와 JavaScript 단계는 **Cast Operations 워커에서 절대 실행되지 않습니다**. 이들은 특정 [Runbook 에이전트](/docs/runbooks/agents) — 자체 인프라 내부 호스트에 설치하는 작은 프로세스 — 에 작업으로 디스패치됩니다.
...
- **JavaScript와 Bash**는 Cast Operations 워커가 아닌, 여러분이 통제하는 Runbook 에이전트 호스트에서 실행됩니다. JavaScript는 평소의 프렐류드(`프로토타입 체인을 끊고, `Function`/`eval`을 제거하고, 내장 프로토타입을 동결)와 함께 `isolated-vm`샌드박스로 감쌉니다. Bash는 에이전트에서 타임아웃 강제와 함께`bash -c`로 실행됩니다.
