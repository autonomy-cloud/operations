...
[Grafana](https://grafana.com) 알림을 Cast Operations 인시던트로 변환합니다. Grafana가 대시보드의 알림 규칙을 평가하면, Cast Operations이 기록하고 에스컬레이션하고 추적합니다.

이 통합은 **인바운드**: Grafana의 알림이 Grafana **Webhook 연락처 포인트** 를 사용해 **Webhook 트리거** 로 시작하는 Cast Operations **[Workflow](/docs/workflows/index)** 로 POST합니다.
...
- Grafana가 HTTPS로 Cast Operations 인스턴스에 도달할 수 있어야 합니다.
...
4. 연락처 포인트를 저장합니다.
5. **Alerting → Notification policies** 로 이동해 원하는 알림(또는 기본 정책)을 **Cast Operations** 연락처 포인트로 라우팅합니다.
...
- Grafana의 알림을 완전히 건너뛰고 Cast Operations이 동일한 메트릭을 직접 모니터링하도록 할 수도 있습니다 — [메트릭 모니터](/docs/monitor/metrics-monitor)를 참조하세요.
...
- [메트릭 모니터](/docs/monitor/metrics-monitor) — Cast Operations에서 직접 메트릭 모니터링.
