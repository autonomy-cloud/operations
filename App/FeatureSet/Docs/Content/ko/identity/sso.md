...
Cast Operations는 엔터프라이즈 인증을 위한 SAML 2.0 기반 싱글 사인온(SSO)을 지원합니다. SSO를 통해 팀 구성원이 조직의 ID 공급자(IdP)를 사용하여 Cast Operations에 로그인할 수 있어 중앙화된 액세스 관리와 향상된 보안을 제공합니다.
...
- **비밀번호 피로 감소**: 사용자가 별도의 Cast Operations 비밀번호를 기억할 필요가 없습니다

## SSO 설정

1. **프로젝트 설정으로 이동**

   - Cast Operations 프로젝트로 이동합니다
...
3. **Cast Operations SSO 메타데이터 가져오기**
...
Keycloak은 널리 사용되는 오픈 소스 ID 및 액세스 관리 솔루션입니다. Cast Operations의 SAML ID 공급자로 Keycloak을 구성하는 단계를 따르십시오.

### 전제 조건

- 구성된 렐름이 있는 실행 중인 Keycloak 인스턴스
- Keycloak와 Cast Operations 모두에 대한 관리자 액세스
- SSO를 지원하는 Cast Operations 계정

### 1단계: Cast Operations SSO 구성
...
4. 인증서 값을 복사하여 다음 형식으로 Cast Operations에 붙여 넣습니다:
...
4. **클라이언트 ID**를 Cast Operations의 **SSO 구성 보기**에서 **식별자 (엔티티 ID)** 값으로 설정합니다
5. **유효한 리디렉션 URI**를 Cast Operations URL로 설정합니다
6. **루트 URL**을 Cast Operations 기본 URL로 설정합니다
7. Cast Operations의 **회신 URL (어설션 소비자 서비스 URL)**을 **어설션 소비자 서비스 POST 바인딩 URL** 필드에 붙여 넣습니다
...
1. Keycloak과 Cast Operations 모두에서 모든 설정을 저장합니다
2. SSO를 사용하여 Cast Operations에 로그인해 봅니다
3. Keycloak 로그인 페이지로 리디렉션된 후 인증 성공 시 Cast Operations으로 돌아와야 합니다
...
Microsoft Entra ID는 Microsoft의 클라우드 기반 ID 및 액세스 관리 서비스입니다. Cast Operations의 SAML ID 공급자로 Entra ID를 구성하는 단계를 따르십시오.

### 전제 조건

- Microsoft Entra ID 테넌트 (SAML SSO를 사용하는 엔터프라이즈 애플리케이션을 지원하는 모든 티어)
- Microsoft Entra ID와 Cast Operations 모두에 대한 관리자 액세스
- SSO를 지원하는 Cast Operations 계정
...
3. **기본 SAML 구성**에서 **편집**을 클릭하고 다음을 설정합니다:
   - **식별자 (엔티티 ID)**: Cast Operations의 **SSO 구성 보기**에서 **식별자 (엔티티 ID)** 붙여 넣기
   - **회신 URL (어설션 소비자 서비스 URL)**: Cast Operations의 **SSO 구성 보기**에서 **회신 URL** 붙여 넣기
4. **저장**을 클릭합니다
5. **SAML 인증서** 섹션에서:
   - **인증서 (Base64)** 다운로드
   - 다운로드된 인증서 파일을 텍스트 편집기로 열고 내용 복사
6. **Cast Operations 설정** 섹션에서 다음을 복사합니다:
   - **로그인 URL** — Cast Operations의 **로그온 URL**로 붙여 넣기
   - **Azure AD 식별자** — Cast Operations의 **발급자**로 붙여 넣기
7. Cast Operations으로 돌아가 인증서와 URL을 붙여 넣고 저장합니다
...
1. Entra ID와 Cast Operations 모두에서 모든 설정을 저장합니다
2. SSO를 사용하여 Cast Operations에 로그인해 봅니다
3. Microsoft 로그인 페이지로 리디렉션된 후 인증 성공 시 Cast Operations으로 돌아와야 합니다

### Microsoft Entra ID 문제 해결

- **AADSTS700016 오류**: Entra ID의 식별자 (엔티티 ID)가 Cast Operations과 일치하지 않음 — 두 값이 동일한지 확인합니다
...
- **이름 ID 불일치**: 이름 ID 클레임이 Cast Operations의 사용자 이메일과 일치하는 이메일 주소로 설정되어 있는지 확인합니다

---

## Okta SAML 구성

Okta는 강력한 SAML SSO 기능을 제공하는 널리 사용되는 ID 플랫폼입니다. Cast Operations의 SAML ID 공급자로 Okta를 구성하는 단계를 따르십시오.
...
5. **앱 이름**으로 "Cast Operations"을 입력하고 **다음**을 클릭합니다
6. **SAML 설정** 섹션에서 다음을 구성합니다:
   - **싱글 사인온 URL**: Cast Operations의 **SSO 구성 보기**에서 **회신 URL (어설션 소비자 서비스 URL)** 붙여 넣기
   - **대상 URI (SP 엔티티 ID)**: Cast Operations의 **SSO 구성 보기**에서 **식별자 (엔티티 ID)** 붙여 넣기
   - **이름 ID 형식**: `EmailAddress` 선택
   - **애플리케이션 사용자 이름**: `Email` 선택
7. **다음**을 클릭한 후 **나는 내 앱을 추가하는 Okta 고객입니다**를 선택하고 **마침**을 클릭합니다

### 3단계: Okta SAML 메타데이터를 Cast Operations에 복사

1. Okta 애플리케이션에서 **로그온** 탭으로 이동합니다
2. **SAML 서명 인증서** 섹션에서 활성 인증서를 찾고 **작업** > **IdP 메타데이터 보기**를 클릭합니다
3. 메타데이터 XML 또는 **로그온** 탭 세부 정보에서:
   - **로그온 URL** (Identity Provider Single Sign-On URL이라고도 함)을 복사하여 Cast Operations의 **로그온 URL**로 붙여 넣기
   - **발급자** (Identity Provider Issuer라고도 함)를 복사하여 Cast Operations의 **발급자**로 붙여 넣기
4. 서명 인증서 다운로드:
   - **SAML 서명 인증서** 섹션에서 활성 인증서에 대해 **작업** > **인증서 다운로드**를 클릭합니다
   - 다운로드된 `.cert` 파일을 텍스트 편집기로 열고 내용을 복사합니다
   - `BEGIN CERTIFICATE` 및 `END CERTIFICATE` 줄을 포함하여 Cast Operations에 인증서를 붙여 넣습니다
5. Cast Operations SSO 구성을 저장합니다
...
1. Okta와 Cast Operations 모두에서 모든 설정을 저장합니다
2. SSO를 사용하여 Cast Operations에 로그인해 봅니다
3. Okta 로그인 페이지로 리디렉션된 후 인증 성공 시 Cast Operations으로 돌아와야 합니다

### Okta 문제 해결

- **404 또는 잘못된 SSO URL**: Okta의 **싱글 사인온 URL**이 Cast Operations의 **회신 URL**과 정확히 일치하는지 확인합니다
- **대상 불일치**: Okta의 **대상 URI**가 Cast Operations의 **식별자 (엔티티 ID)**와 정확히 일치하는지 확인합니다
...
Cast Operations의 SSO 구현은 SAML 2.0 프로토콜을 사용하며 호환 가능한 모든 ID 공급자와 함께 작동해야 합니다. 일반 구성 단계:

1. Cast Operations에서 SSO 구성을 생성하고 **SSO 구성 보기** 버튼에서 **식별자 (엔티티 ID)**와 **회신 URL (어설션 소비자 서비스 URL)**을 기록합니다
2. ID 공급자에서 다음을 사용하여 SAML 애플리케이션을 생성합니다:
   - **어설션 소비자 서비스 URL / 회신 URL**: Cast Operations SSO 구성에서
   - **엔티티 ID / 대상 URI**: Cast Operations SSO 구성에서
   - **이름 ID 형식**: 이메일 주소
3. ID 공급자에서 다음을 Cast Operations에 복사합니다:
   - **로그온 URL** (SSO 엔드포인트)
   - **발급자** (IdP의 엔티티 ID)
   - **공개 인증서** (X.509 서명 인증서)
...
Cast Operations은 현재 ID 공급자의 SAML 역할 매핑을 지원하지 않습니다. 역할 기반 액세스는 **프로젝트 설정** > **SSO** 설정 내에서 Cast Operations에서 별도로 구성해야 하며, 여기서 SSO 사용자에게 기본 역할을 할당할 수 있습니다.
