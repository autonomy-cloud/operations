...
Cast Operations는 자동화된 사용자 프로비저닝 및 디프로비저닝을 위한 SCIM v2.0 프로토콜을 지원합니다. SCIM을 사용하면 Azure AD, Okta 및 기타 엔터프라이즈 ID 시스템과 같은 ID 공급자(IdP)가 Cast Operations 프로젝트 및 상태 페이지에 대한 사용자 액세스를 자동으로 관리할 수 있습니다.

## 개요

SCIM 통합은 다음과 같은 이점을 제공합니다:

- **자동화된 사용자 프로비저닝**: IdP에서 사용자가 할당될 때 Cast Operations에서 자동으로 사용자 생성
- **자동화된 사용자 디프로비저닝**: IdP에서 사용자가 할당 해제될 때 Cast Operations에서 자동으로 사용자 제거
- **사용자 속성 동기화**: IdP와 Cast Operations 간의 사용자 정보를 동기화 상태로 유지
- **중앙화된 액세스 관리**: 기존 ID 관리 시스템에서 Cast Operations 액세스 관리

## 프로젝트용 SCIM

프로젝트 SCIM은 ID 공급자가 Cast Operations 프로젝트 내의 팀 구성원을 관리할 수 있게 합니다.
...
1. **IdP에서 사용자 할당**: IdP에서 사용자가 Cast Operations에 할당될 때
2. **SCIM 프로비저닝**: IdP가 Cast Operations SCIM API를 호출하여 사용자 생성
3. **팀 구성원 자격**: 사용자가 구성된 기본 팀에 자동으로 추가됨
4. **액세스 부여**: 사용자가 이제 Cast Operations 프로젝트에 액세스 가능
5. **사용자 할당 해제**: IdP에서 사용자가 할당 해제될 때
6. **SCIM 디프로비저닝**: IdP가 Cast Operations SCIM API를 호출하여 사용자 제거
...
1. **IdP에서 사용자 할당**: IdP에서 사용자가 Cast Operations 상태 페이지에 할당될 때
2. **SCIM 프로비저닝**: IdP가 Cast Operations SCIM API를 호출하여 구독자 생성
3. **액세스 부여**: 사용자가 이제 비공개 상태 페이지에 액세스 가능
4. **사용자 할당 해제**: IdP에서 사용자가 할당 해제될 때
5. **SCIM 디프로비저닝**: IdP가 Cast Operations SCIM API를 호출하여 구독자 제거
...
Microsoft Entra ID는 강력한 SCIM 프로비저닝 기능을 갖춘 엔터프라이즈급 ID 관리를 제공합니다. Cast Operations으로 SCIM 프로비저닝을 구성하는 상세한 단계를 따르십시오.

#### 전제 조건

- Premium P1 또는 P2 라이선스가 있는 Microsoft Entra ID 테넌트 (자동 프로비저닝에 필요)
- Scale 플랜 이상의 Cast Operations 계정
- Microsoft Entra ID와 Cast Operations 모두에 대한 관리자 액세스

#### 1단계: Cast Operations에서 SCIM 구성 가져오기
...
1. Cast Operations 엔터프라이즈 애플리케이션에서 **프로비저닝**으로 이동합니다
2. **시작하기**를 클릭합니다
...
| Azure AD 속성                                                 | Cast Operations SCIM 속성            | 필수 여부 |
| ------------------------------------------------------------- | ------------------------------ | --------- |
...
Cast Operations에서 **그룹 푸시**를 활성화한 경우:
...
| Azure AD 속성 | Cast Operations SCIM 속성 |
| ------------- | ------------------- |
...
1. Cast Operations 엔터프라이즈 애플리케이션에서 **사용자 및 그룹**으로 이동합니다
2. **+ 사용자/그룹 추가**를 클릭합니다
3. Cast Operations에 프로비저닝하려는 사용자 및/또는 그룹을 선택합니다
4. **할당**을 클릭합니다
...
Okta는 훌륭한 SCIM 지원을 갖춘 유연한 ID 관리를 제공합니다. Cast Operations으로 SCIM 프로비저닝을 구성하는 상세한 단계를 따르십시오.

#### 전제 조건

- 프로비저닝 기능이 있는 Okta 테넌트 (라이프사이클 관리 기능)
- Scale 플랜 이상의 Cast Operations 계정
- Okta와 Cast Operations 모두에 대한 관리자 액세스
...
3. 기존 Cast Operations 애플리케이션을 찾아 선택합니다
...
5. 앱 이름으로 "Cast Operations"을 입력합니다
6. SAML 구성을 완료합니다 (SSO 문서 참조)
7. **마침**을 클릭합니다

#### 3단계: SCIM 프로비저닝 활성화

1. Cast Operations 애플리케이션에서 **일반** 탭으로 이동합니다
...
| Okta 속성          | Cast Operations SCIM 속성             | 방향            |
| ------------------ | ------------------------------- | --------------- |
...
2. Cast Operations 애플리케이션과 관련된 이벤트를 필터링합니다
3. 프로비저닝 이벤트가 성공했는지 확인합니다
4. Cast Operations을 확인하여 사용자가 생성되었는지 확인합니다
...
- **오류: 401 Unauthorized**: Cast Operations에서 Bearer 토큰을 재생성하고 Okta를 업데이트합니다
...
사용자가 디프로비저닝되면 (DELETE 요청 또는 `active: false` 설정에 의해) SCIM 설정에서 구성된 팀에서 제거됩니다. 사용자 계정 자체는 Cast Operations에 남아 있지만 프로젝트에 대한 액세스 권한을 잃습니다.

### SSO 없이 SCIM을 사용할 수 있나요?

예, SCIM과 SSO는 독립적인 기능입니다. SSO로 로그인을 허용하면서 사용자 프로비저닝에 SCIM을 사용할 수 있습니다.

### Cast Operations에 이미 존재하는 사용자를 어떻게 처리하나요?

SCIM이 이미 존재하는 사용자(이메일로 일치)를 생성하려고 할 때 Cast Operations은 중복 사용자를 생성하지 않고 단순히 구성된 기본 팀에 추가합니다.
