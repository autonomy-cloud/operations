# Cast Operations Terraform 공급자

Cast Operations Terraform 공급자를 통해 인프라 코드(IaC)를 사용하여 Cast Operations 리소스를 관리할 수 있습니다. 이 공급자를 통해 Terraform을 통해 모니터링, 인시던트 관리, 상태 페이지 및 기타 Cast Operations 기능을 구성할 수 있습니다.
...
Cast Operations Terraform 공급자는 [Terraform 레지스트리](https://registry.terraform.io/providers/autonomy-cloud/operations)에서 사용할 수 있습니다.
...
⚠️ **자체 호스팅 고객에게 중요**: API 호환성을 보장하기 위해 Terraform 공급자 버전을 항상 Cast Operations 설치 버전과 일치하도록 고정합니다.
...
여러 가지 방법으로 Cast Operations 버전을 찾을 수 있습니다:

1. **대시보드**: Cast Operations 대시보드에서 설정 → 정보로 이동합니다
...
먼저 Cast Operations 대시보드에서 API 키를 생성합니다:
...
Cast Operations 클라우드 고객의 경우 최신 공급자 버전을 사용합니다:
...
**중요**: 자체 호스팅 고객은 Cast Operations 설치와 일치하도록 공급자 버전을 고정해야 합니다:

| Cast Operations 버전 | 공급자 버전 | 구성                   |
| -------------- | ----------- | ---------------------- |
| 7.0.x          | 7.0.x       | `version = "~> 7.0.0"` |
| 7.1.x          | 7.1.x       | `version = "~> 7.1.0"` |
| 7.2.x          | 7.2.x       | `version = "~> 7.2.0"` |

Cast Operations 7.0.123의 예시:
...
Cast Operations Terraform 공급자는 다음 리소스를 지원합니다:
...
variable "project_id" {
  description = "Cast Operations 프로젝트 ID (대시보드에서 수동으로 생성)"
  type        = string
}
...
- Cast Operations을 업그레이드할 때 공급자 버전을 업데이트합니다
...
1. Cast Operations 대시보드에서 **기존 리소스 감사**
...
Cast Operations을 업그레이드할 때 (자체 호스팅):
...
1. **버전 불일치 (자체 호스팅)**
   ...
   **해결책**: 공급자 버전이 Cast Operations 설치와 일치하는지 확인합니다
