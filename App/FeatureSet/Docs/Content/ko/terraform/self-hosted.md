# 자체 호스팅 Cast Operations Terraform 구성 가이드

이 가이드는 자체 호스팅 Cast Operations 인스턴스를 실행하는 고객을 위한 것입니다. Terraform 공급자와 함께 자체 Cast Operations 배포를 사용하기 위한 버전 관리, 구성 및 모범 사례를 다룹니다.

## 중요 참고 사항

⚠️ **프로젝트는 Terraform을 통해 생성할 수 없습니다** - 프로젝트는 먼저 Cast Operations 대시보드에서 수동으로 생성해야 합니다. Terraform 구성에서는 프로젝트 ID를 사용합니다.

⚠️ **자체 호스팅 고객에게 가장 중요한 규칙**: Terraform 공급자 버전을 항상 Cast Operations 설치 버전과 정확히 일치하도록 고정합니다.

## 리소스 구조

모든 Cast Operations Terraform 리소스는 단순화된 구조를 따릅니다:
...
- Terraform 공급자는 Cast Operations API에서 자동 생성됩니다
- 각 Cast Operations 버전에는 다른 API 엔드포인트와 스키마가 있을 수 있습니다
...
다음은 자체 호스팅 Cast Operations 인스턴스에 대한 완전한 예시입니다:
...
Cast Operations 인스턴스를 업그레이드할 때:
...
표준 Cast Operations 업그레이드 프로세스를 따릅니다 (Docker, Helm 등).
...
- Cast Operations API 엔드포인트 (일반적으로 포트 443/HTTPS)
- 모니터링되는 내부 리소스
...
1. Cast Operations 인스턴스가 실행 중인지 확인합니다
...
1. Cast Operations 버전 확인: `curl https://your-instance/api/status`
