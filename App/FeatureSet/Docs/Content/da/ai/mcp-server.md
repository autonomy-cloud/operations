...
Cast Operations Model Context Protocol (MCP) Serveren giver LLM'er direkte adgang til din Cast Operations-instans, hvilket muliggør AI-drevet overvågning, incident management og observabilitetsoperationer.

## Hvad er Cast Operations MCP Serveren?

Cast Operations MCP Serveren er en bro mellem Large Language Models (LLM'er) og din Cast Operations-instans. Den implementerer Model Context Protocol (MCP), som giver AI-assistenter som Claude mulighed for at interagere direkte med din overvågningsinfrastruktur.

## Sådan fungerer det

MCP-serveren hostes sammen med din Cast Operations-instans og er tilgængelig via Streamable HTTP-transporten. Der kræves ingen lokal installation.

**Skybrugere**: `https://latticeruntime.com/mcp`
**Selvhostede brugere**: `https://your-operations-domain.com/mcp`
...
Med Cast Operations MCP Serveren kan AI-assistenter hjælpe dig med:
...
- Cast Operations-instans (sky eller selvhostet)
- MCP-kompatibel klient (Claude Desktop, VS Code med GitHub Copilot osv.)
- Gyldig Cast Operations API-nøgle (kræves kun til autentificerede operationer – offentlige værktøjer fungerer uden den)

## Hentning af din API-nøgle

1. Log ind på din Cast Operations-instans
...
Erstat `latticeruntime.com` med dit Cast Operations-domæne:
...
VS Code understøtter MCP-servere nativt med GitHub Copilot (version 1.99+). Dette giver Copilot mulighed for at få adgang til Cast Operations-data direkte.
...
3. Klik på "cast-operations" for at starte serveren
4. Når du bliver bedt om det, skal du indtaste din Cast Operations API-nøgle
...
- **`cast_operations_help`**: Få hjælp og vejledning om Cast Operations MCP-kapaciteter
- **`cast_operations_list_resources`**: Liste over tilgængelige ressourcer og deres operationer
...
- `x-api-key`: Din Cast Operations API-nøgle
...
Værktøjet **`cast_operations_whoami`** returnerer det projekt, din API-nøgle tilhører (ID og navn). Det er et nyttigt første kald, så en agent kan orientere sig — og da oprettelsesværktøjer udleder `projectId` fra API-nøglen, behøver agenten aldrig at angive et projekt-ID.
...
- Overvåg brugen: Hold styr på API-nøglebrug i Cast Operations
...
1. Bekræft, at din Cast Operations URL er korrekt
2. Kontroller, at din API-nøgle er gyldig
3. Sørg for, at din Cast Operations-instans er tilgængelig
4. Test health-endpointet

### Ugyldig API-nøgle

- Bekræft API-nøglen i dine Cast Operations-indstillinger
