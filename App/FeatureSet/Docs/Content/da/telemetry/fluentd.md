# Brug Fluentd til at sende telemetridata til Cast Operations

## Oversigt

Du kan bruge [Fluentd](https://www.fluentd.org/)-pluginnet til at indsamle logs og telemetridata fra dine applikationer og tjenester. Pluginnet sender telemetridataene til Cast Operations HTTP Source. Du kan bruge http-outputpluginnet til fluentd til at sende telemetridataene til Cast Operations HTTP Source. Dette plugin kan findes her: https://docs.fluentd.org/output/http

## Kom i gang

Fluentd understøtter hundredvis af datakilder, og du kan indsamle logs fra enhver af disse kilder til Cast Operations. Nogle af de populære kilder inkluderer:
...
Du kan bruge følgende konfiguration til at sende telemetridataene til Cast Operations HTTP Source. Du kan tilføje denne konfiguration til fluentd-konfigurationsfilen. Konfigurationsfilen er normalt placeret på `/etc/fluentd/fluent.conf` eller `/etc/td-agent/td-agent.conf`.

Du skal erstatte `YOUR_SERVICE_TOKEN` med det token, du oprettede i det forrige trin. Du skal også erstatte `YOUR_SERVICE_NAME` med navnet på din tjeneste. Tjenestenavnet kan være et hvilket som helst navn du ønsker. Hvis tjenesten ikke eksisterer i Cast Operations, oprettes den automatisk.
...
**Hvis du selvhoster Cast Operations**: Hvis du selvhoster Cast Operations, kan du erstatte `endpoint_url` med URL'en til din Cast Operations-instans. `http(s)://YOUR_ONEUPTIME_HOST/fluentd/logs`
