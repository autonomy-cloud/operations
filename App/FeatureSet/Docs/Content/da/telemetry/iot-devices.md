# Cast Operations IoT-enheder

## Oversigt

Cast Operations overvåger flåder af IoT-enheder — sensorer, gateways, controllere og edge-bokse — ved at indlæse et lille sæt `iot_*` metrikker, mærket med hvilken **flåde** hver måling tilhører og dens eget **enheds-id**. Cast Operations grupperer disse metrikker i en flåde, opbygger en live enhedsoversigt og sporer batteri, forbindelse, temperatur, CPU, hukommelse og tilgængelighed pr. enhed.
...
- **MQTT** — forbind direkte til Cast Operations indbyggede MQTT-endpoint (MQTT-over-WebSocket på `wss://<your-host>/mqtt` eller rå MQTT TCP på selv-hostede deployments) og publicer JSON-målinger. Ingen collector påkrævet, og understøttelse af Last Will giver dig øjeblikkelig offline-detektering.
...
Cast Operations kortlægger dine enheder på to begreber ved hjælp af OpenTelemetry-ressourceattributter:

- **Flåde** — en logisk gruppe af enheder (for eksempel `building-a-sensors` eller `field-gateways`). Flåden udledes af ressourceattributten `iot.fleet.name` og vises i Cast Operations som telemetritjenesten `iot/<fleet>`. Sæt `service.name=iot/<fleet>`, så logs og metrikker placeres under den samme tjeneste.
- **Enhed** — en individuel enhed inden for en flåde, identificeret ved attributten `device.id`. Cast Operations opbygger og vedligeholder en enhedsoversigt pr. flåde nøglet på `device.id`.

Valgfrie attributter forfiner, hvordan hver enhed klassificeres og afgrænses i monitorer:

| Attribut             | Påkrævet | Beskrivelse                                                                      |
| -------------------- | -------- | -------------------------------------------------------------------------------- |
| `iot.fleet.name`     | Ja       | Den flåde, denne enhed tilhører. Bliver til Cast Operations-tjenesten `iot/<fleet>`    |
...
Hvis din enhed kører en OpenTelemetry SDK direkte, så peg den mod Cast Operations og stempl IoT-ressourceattributterne via standard `OTEL_*` miljøvariablerne. Erstat token, endpoint, flådenavn og enheds-id med værdier for dit miljø.
...
| ----------------------------- | -------- | ---------------------------------------------------------------------------------------------------- |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | Ja       | Cast Operations OTLP-endpoint (`https://latticeruntime.com/otlp`, eller `http(s)://YOUR-OPERATIONS-HOST/otlp` selv-hostet) |
...
Når mange enheder rapporterer gennem en gateway, så kør en OpenTelemetry Collector på gatewayen og eksporter til Cast Operations. `resource`-processoren stempler flådeattributterne; modtag målinger fra dine enheder (OTLP, MQTT-bro, fil-logs osv.) og videresend dem:
...
- Behold `device.id` (og valgfrit `iot.device.kind` / `iot.device.type` / `iot.device.firmware`) på hvert datapunkt, så Cast Operations kan slå den individuelle enhed op inde i flåden.
- **`otlphttp`** sender til Cast Operations over HTTPS med indlæsningstokenet vedhæftet. Både standard protobuf-encodingen og `encoding: json` accepteres.

## Afsendelse af metrikker via MQTT

Cast Operations leveres med et indbygget MQTT-endpoint, så enheder, der allerede taler MQTT, kan sende målinger direkte — ingen OpenTelemetry SDK, collector eller bro påkrævet. Alt, der publiceres over MQTT, havner i den samme pipeline som OTLP: flåder oprettes automatisk, enhedsoversigten opdateres, og hver IoT-monitor og alarmskabelon fungerer uændret.

**Endpoints**

| Transport             | Adresse                                | Bemærkninger                                                                              |
| --------------------- | -------------------------------------- | ----------------------------------------------------------------------------------------- |
| MQTT over WebSocket   | `wss://<your-host>/mqtt`               | Fungerer på alle deployments — kører på den normale HTTPS-port gennem Cast Operations-ingressen |
...
Eksempel med Node.js `mqtt` over WebSocket (fungerer mod latticeruntime.com og enhver selv-hostet instans):
...
- Publiceringer uden for emnekontrakten eller med misdannede payloads accepteres og **droppes** (MQTT 3.1.1 har ingen fejlrespons pr. besked) — serveren logger en advarsel med årsagen, så tjek Cast Operations-app-logs, hvis der ikke ankommer data.
- På WebSocket-endpointet skal du holde MQTT-keepalive **under 5 minutter** — Cast Operations-ingressen lukker inaktive WebSocket-forbindelser efter 300 sekunder, hvilket ville udløse din Last Will og en falsk Device Offline-alarm. Klientbibliotekernes standardværdier (60 s for `mqtt` og `paho-mqtt`) er fine. Det rå TCP-endpoint har ikke et sådant loft.
- Payloads er begrænset til 128 KB og 100 metrikker pr. publicering; for store pakker afbryder forbindelsen.

## Metrikkonventioner

Cast Operations genkender følgende `iot_*` metriknavne. Hvert datapunkt bør bære `device.id`-labelen, så målingen tilskrives den rigtige enhed. Du behøver kun at sende de metrikker, der giver mening for din enhed — manglende metrikker bliver blot ikke vist i grafer.
...
| `iot_cpu_usage_ratio`       | CPU-udnyttelse som et forhold `0`–`1` (Cast Operations gemmer det som en procentdel)  |
...
2. I Cast Operations-dashboardet skal du åbne **IoT**-sektionen — din flåde bør vises som `iot/<fleet>` i løbet af cirka et minut.
...
2. Bekræft, at eksportør-endpointet er `https://latticeruntime.com/otlp` (eller dit selv-hostede `…/otlp`), og at `x-cast-operations-token`-headeren bærer et gyldigt token.
...
2. Husk, at `iot_cpu_usage_ratio` er et `0`–`1`-forhold; send det rå forhold, og Cast Operations gengiver det som en procentdel.
3. Tillad op til et minut, før de første datapunkter dukker op, efter en enhed begynder at rapportere.

## Selv-hostet Cast Operations

Hvis du selv-hoster Cast Operations, så peg endpointet mod din egen instans:
...
- For at lære den underliggende OTLP-integration i dybden, se [Integrer OpenTelemetry med Cast Operations](/docs/telemetry/open-telemetry).
