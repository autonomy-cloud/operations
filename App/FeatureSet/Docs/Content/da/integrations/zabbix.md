...
[Zabbix](https://www.zabbix.com) overvåger dine servere og dit netværk; Cast Operations håndterer din hændelsesrespons, vagtplan og statussider. Forbind de to, og hvert Zabbix-problem bliver automatisk en Cast Operations-hændelse — så de rette personer kontaktes, og din statusside holder sig præcis.

Denne integration er **indgående**: Zabbix sender problemer til Cast Operations. Den bruger en Zabbix **webhook-medietype** på den ene side og et Cast Operations **[Workflow](/docs/workflows/index)** på den anden. Ingen plugins, ingen ekstra tjenester.
...
2. En Zabbix **action** beder **Cast Operations**-medietypen om at sende eventet.
3. Medietypens script POSTer en lille JSON-payload til en Cast Operations workflow-URL.
...
- Din Zabbix-server skal kunne nå din Cast Operations-instans via HTTPS.
- Et Cast Operations-projekt, hvor du kan oprette workflows.

## Del 1 — Byg Cast Operations-workflowet
...
4. Træk en **Create Incident**-blok og forbind den til **Conditions**-blokkens **Yes**-output. Udfyld:
   ...
   - **Severity**: vælg den Cast Operations-hændelsesalvorlighed, du ønsker (du kan forfine dette senere med flere Conditions-grene, der afbilder Zabbix-alvorligheder).
...
### Trin 1: Opret Cast Operations-medietypen
...
1. Gå til **Users → Users → Create user**. Navngiv den `Cast Operations Webhook`, giv den en rolle, der kan modtage notifikationer (f.eks. **User role**), og tilføj den til en brugergruppe.
...
### Trin 3: Send problemer til Cast Operations med en action
...
4. På fanen **Operations** tilføjer du en operation, der sender til **User: Cast Operations Webhook** via **Cast Operations**-medietypen.
...
1. Tilbage i Cast Operations-workflowet, slå **Enabled** til.
...
4. Tjek **Incidents** i Cast Operations — dit Zabbix-problem er nu en hændelse.
...
Zabbix-alvorligheder (`Not classified`, `Information`, `Warning`, `Average`, `High`, `Disaster`) ankommer som `{{Zabbix.Request Body.severity}}`. For at afbilde dem til Cast Operations-hændelsesalvorligheder tilføjer du **Conditions**-grene før **Create Incident** — for eksempel ruter du `Disaster` og `High` til en "Critical"-hændelse og alt andet til "Major". Byg én **Create Incident**-blok per gren.
...
- Et ikke-2xx-svar fra Cast Operations afspejles af `throw` i scriptet — tjek at workflow-URL'en er helt korrekt.
...
- Hvis du kører Cast Operations selvhostet bag en firewall, tillad din Zabbix-servers udgående IP adgang til den via HTTPS.
