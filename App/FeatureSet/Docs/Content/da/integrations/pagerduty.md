...
Udløs en [PagerDuty](https://www.pagerduty.com)-hændelse, når en Cast Operations-hændelse oprettes, og løs den, når Cast Operations løser den. Nyttigt, når PagerDuty ejer din eskalering og vagtplaner, og du ønsker, at Cast Operations overvågning føder den.

Denne integration er **udgående**: Cast Operations kalder PagerDutys [Events API v2](https://developer.pagerduty.com/docs/events-api-v2/overview/). Den bruger et Cast Operations **[Workflow](/docs/workflows/index)** med en **Incident → On Create**-trigger og en **API-komponent**.

> Cast Operations har sin egen vagtplan og eskalering indbygget — se [On Call](/docs/on-call/incoming-call-policy). Brug kun denne integration, hvis du specifikt ønsker, at events også lander i PagerDuty.
...
3. Tilføj en **API**-blok forbundet til triggeren:
   ...
   **`dedup_key`** knytter denne PagerDuty-hændelse til Cast Operations-hændelsen, så du kan løse den senere. Brug af Cast Operations-hændelsens id holder den unik og forudsigelig.

4. **Gem**, aktivér, og opret en testhændelse. Et `202`-svar i workflowets logfiler betyder, at PagerDuty accepterede eventet.

## Trin 3 — Løs ved Cast Operations-løsning (anbefalet)
...
PagerDutys `severity` accepterer `critical`, `error`, `warning` eller `info`. For at afbilde fra Cast Operations-alvorligheder tilføjer du **Conditions**-grene på `{{Incident.incidentSeverity.name}}` før API-blokken og sender en anden body fra hver.

## Indgående (valgfrit)

For at gå den anden vej — åbn en Cast Operations-hændelse fra et PagerDuty-event — tilføj et **Webhook**-trigger-workflow og peg et PagerDuty [V3 webhook](https://developer.pagerduty.com/docs/webhooks/v3-overview/) (eller en Events Orchestration) mod dens URL, og brug derefter **Create Incident**. Se det [indgående mønster](/docs/integrations/index#inbound-another-tool-sends-data-into-cast-operations).
...
- [On Call](/docs/on-call/incoming-call-policy) — Cast Operations indbyggede eskalering.
