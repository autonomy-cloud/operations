...
Cast Operations forbinder sig med de værktøjer, dit team allerede bruger — Zabbix, Jira, PagerDuty, Slack og mange flere — via **[Workflows](/docs/workflows/index)**, det indbyggede automatiseringsmotor. Der er ingen separat plugin at installere. Du sætter en integration sammen på et træk-og-slip-lærred, og den kører, når der sker noget.

Denne side forklarer de to mønstre, som alle integrationer bruger. Når du forstår dem, kan du forbinde Cast Operations med næsten alt — selv værktøjer, der ikke har deres egen side her.

## De to mønstre

Enhver integration bevæger data i én af to retninger (og mange bruger begge).

### Indgående — et andet værktøj sender data til Cast Operations

Brug dette, når et eksternt system har brug for at _oprette eller opdatere noget i Cast Operations_ — typisk åbne en hændelse eller en alarm, når det opdager et problem.

1. Byg et workflow, der starter med en **[Webhook-trigger](/docs/workflows/triggers#webhook)**. Cast Operations giver dig en unik URL.
...
### Udgående — Cast Operations sender data til et andet værktøj

Brug dette, når _noget i Cast Operations skal vises i et andet værktøj_ — åbn en Jira-ticket, kontakt nogen i PagerDuty, post til Slack.

1. Byg et workflow, der starter med en **[Cast Operations event-trigger](/docs/workflows/triggers#cast-operations-event-triggers)** — for eksempel **Incident → On Create**.
...
| --------------------------------------------------------------------- | ---------------------- | ----------------------------------------------------------------------------- |
| [Zabbix](/docs/integrations/zabbix)                                   | Indgående              | Gør Zabbix-problemer til Cast Operations-hændelser (og løser dem ved genopretning). |
| [Jira](/docs/integrations/jira)                                       | Udgående (+ indgående) | Åbn en Jira-sag for hver hændelse; synkronisér status tilbage.                |
| [PagerDuty](/docs/integrations/pagerduty)                             | Udgående (+ indgående) | Udløs og løs PagerDuty-events fra Cast Operations-hændelser.                        |
| [Opsgenie](/docs/integrations/opsgenie)                               | Udgående (+ indgående) | Opret og luk Opsgenie-alarmer.                                                |
| [ServiceNow](/docs/integrations/servicenow)                           | Udgående (+ indgående) | Åbn ServiceNow-hændelser fra Cast Operations.                                       |
...
- Hvis værktøjet kan **sende en webhook**, når noget sker, brug det **indgående** mønster — peg dets webhook mod en Cast Operations Webhook-trigger.
...
- [Triggere](/docs/workflows/triggers) — Webhook- og Cast Operations event-triggere i detaljer.
