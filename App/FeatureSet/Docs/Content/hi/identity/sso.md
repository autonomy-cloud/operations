...
Cast Operations enterprise authentication के लिए SAML 2.0 आधारित Single Sign-On (SSO) का समर्थन करता है। SSO आपके team members को Cast Operations में आपके organization के identity provider (IdP) का उपयोग करके login करने की अनुमति देता है, जिससे centralized access management और बेहतर सुरक्षा मिलती है।
...
- **कम Password Fatigue**: Users को एक अलग Cast Operations password याद नहीं रखना होता

## SSO सेट अप करना

1. **Project Settings पर जाएं**

   - अपने Cast Operations project पर जाएं
...
3. **Cast Operations SSO Metadata प्राप्त करें**
...
Keycloak एक लोकप्रिय open-source identity और access management solution है। Cast Operations के लिए SAML identity provider के रूप में Keycloak configure करने के लिए इन steps का पालन करें।

### पूर्व आवश्यकताएं

- एक configured realm के साथ एक चलता हुआ Keycloak instance
- Keycloak और Cast Operations दोनों तक Admin access
- SSO support के साथ Cast Operations account

### चरण 1: Cast Operations SSO Configure करें
...
4. certificate value copy करें और Cast Operations में निम्नलिखित format में paste करें:
...
4. **Client ID** को Cast Operations के **View SSO Config** से **Identifier (Entity ID)** value पर सेट करें
5. **Valid Redirect URIs** को अपने Cast Operations URL पर सेट करें
6. **Root URL** को अपने Cast Operations base URL पर सेट करें
7. Cast Operations से **Reply URL (Assertion Consumer Service URL)** को **Assertion Consumer Service POST Binding URL** field में paste करें
...
1. Keycloak और Cast Operations दोनों में सभी settings सहेजें
2. SSO का उपयोग करके Cast Operations में login करने की कोशिश करें
3. आपको अपने Keycloak login page पर redirect होना चाहिए और सफल authentication के बाद Cast Operations पर वापस
...
Microsoft Entra ID, Microsoft की cloud-based identity और access management service है। Cast Operations के लिए SAML identity provider के रूप में Entra ID configure करने के लिए इन steps का पालन करें।
...
5. एक नाम दर्ज करें (जैसे "Cast Operations")
...
3. **Basic SAML Configuration** में, **Edit** पर क्लिक करें और सेट करें:
   - **Identifier (Entity ID)**: Cast Operations के **View SSO Config** से **Identifier (Entity ID)** paste करें
   - **Reply URL (Assertion Consumer Service URL)**: Cast Operations के **View SSO Config** से **Reply URL** paste करें
...
6. **Set up Cast Operations** section में, copy करें:
   - **Login URL** — इसे Cast Operations में **Sign On URL** के रूप में paste करें
   - **Azure AD Identifier** — इसे Cast Operations में **Issuer** के रूप में paste करें
7. Cast Operations पर वापस जाएं और certificate और URLs paste करें, फिर सहेजें
...
1. Entra ID और Cast Operations दोनों में सभी settings सहेजें
2. SSO का उपयोग करके Cast Operations में login करने की कोशिश करें
3. आपको Microsoft login page पर redirect होना चाहिए और सफल authentication के बाद Cast Operations पर वापस

### Microsoft Entra ID के लिए समस्या निवारण

- **AADSTS700016 Error**: Entra ID में Identifier (Entity ID) Cast Operations से मेल नहीं खाती — दोनों values समान हैं इसे सत्यापित करें
...
- **Name ID Mismatch**: सुनिश्चित करें कि Name ID claim एक email address पर सेट है जो Cast Operations में user के email से मेल खाती है

---

## Okta SAML Configuration

Okta एक व्यापक रूप से उपयोग किया जाने वाला identity platform है जो robust SAML SSO capabilities प्रदान करता है। Cast Operations के लिए SAML identity provider के रूप में Okta configure करने के लिए इन steps का पालन करें।
...
5. **App name** के रूप में "Cast Operations" दर्ज करें और **Next** पर क्लिक करें
6. **SAML Settings** section में, configure करें:
   - **Single sign-on URL**: Cast Operations के **View SSO Config** से **Reply URL (Assertion Consumer Service URL)** paste करें
   - **Audience URI (SP Entity ID)**: Cast Operations के **View SSO Config** से **Identifier (Entity ID)** paste करें
...
### चरण 3: Okta SAML Metadata Cast Operations में Copy करें
...
3. metadata XML से, या **Sign On** tab details से:
   - **Sign On URL** copy करें (जिसे **Identity Provider Single Sign-On URL** भी कहते हैं) — इसे Cast Operations में **Sign On URL** के रूप में paste करें
   - **Issuer** copy करें (जिसे **Identity Provider Issuer** भी कहते हैं) — इसे Cast Operations में **Issuer** के रूप में paste करें
4. signing certificate download करें:
   ...
   - Cast Operations में certificate paste करें (`BEGIN CERTIFICATE` और `END CERTIFICATE` lines सहित)
5. Cast Operations SSO configuration सहेजें
...
1. Okta और Cast Operations दोनों में सभी settings सहेजें
2. SSO का उपयोग करके Cast Operations में login करने की कोशिश करें
3. आपको Okta login page पर redirect होना चाहिए और सफल authentication के बाद Cast Operations पर वापस

### Okta के लिए समस्या निवारण

- **404 या Invalid SSO URL**: सत्यापित करें कि Okta में **Single sign-on URL**, Cast Operations के **Reply URL** से बिल्कुल मेल खाती है
- **Audience Mismatch**: सुनिश्चित करें कि Okta में **Audience URI**, Cast Operations के **Identifier (Entity ID)** से बिल्कुल मेल खाती है
...
Cast Operations का SSO implementation SAML 2.0 protocol उपयोग करता है और किसी भी compliant identity provider के साथ काम करना चाहिए। सामान्य configuration steps हैं:

1. Cast Operations में, एक SSO configuration बनाएं और **View SSO Config** button से **Identifier (Entity ID)** और **Reply URL (Assertion Consumer Service URL)** नोट करें
2. अपने identity provider में, इन्हें उपयोग करके एक SAML application बनाएं:
   - **Assertion Consumer Service URL / Reply URL**: Cast Operations SSO config से
   - **Entity ID / Audience URI**: Cast Operations SSO config से
   - **Name ID Format**: Email address
3. अपने identity provider से निम्नलिखित Cast Operations में copy करें:
...
Cast Operations वर्तमान में अपने identity provider से SAML roles mapping का समर्थन नहीं करता। Role-based access को Cast Operations के **Project Settings** > **SSO** settings के भीतर अलग से configure किया जाना चाहिए, जहाँ आप SSO users के लिए default roles assign कर सकते हैं।
