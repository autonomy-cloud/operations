...
每當建立 Cast Operations 事件時觸發一個 [PagerDuty](https://www.pagerduty.com) 事件，並在 Cast Operations 解決時一併解決。當 PagerDuty 掌管你的升級流程與待命排程，而你希望讓 Cast Operations 的監控資料餵入其中時，這項整合非常實用。

這項整合是**對外（outbound）**的：Cast Operations 呼叫 PagerDuty 的 [Events API v2](https://developer.pagerduty.com/docs/events-api-v2/overview/)。它使用一個 Cast Operations **[Workflow](/docs/workflows/index)**，搭配 **Incident → On Create** 觸發器與 **API 元件**。

> Cast Operations 內建自己的待命與升級功能 — 請參閱 [On Call](/docs/on-call/incoming-call-policy)。只有在你特別希望事件也送入 PagerDuty 時，才使用這項整合。
...
- 一個具有 **Events API v2** 整合的 PagerDuty 服務。在 PagerDuty 中：**Service → Integrations → Add integration → Events API v2**。複製 **Integration Key**（也稱為 _routing key_）。
...
3. 新增一個連接到觸發器的 **API** 區塊：
   ...
   **`dedup_key`** 將這個 PagerDuty 事件與 Cast Operations 事件綁定在一起，讓你之後可以解決它。使用 Cast Operations 事件 id 可以讓它保持唯一且可預測。

4. **Save**、啟用，並建立一個測試事件。工作流程記錄中出現 `202` 回應，代表 PagerDuty 已接受該事件。

## 步驟 3 — 在 Cast Operations 解決時一併解決（建議）
...
PagerDuty 的 `severity` 接受 `critical`、`error`、`warning` 或 `info`。若要從 Cast Operations 的嚴重性進行對應，請在 API 區塊之前依 `{{Incident.incidentSeverity.name}}` 新增 **Conditions** 分支，並從每個分支送出不同的 body。
...
- [On Call](/docs/on-call/incoming-call-policy) — Cast Operations 內建的升級功能。
