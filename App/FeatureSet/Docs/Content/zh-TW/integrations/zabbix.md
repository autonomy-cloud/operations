...
[Zabbix](https://www.zabbix.com) 監看您的伺服器與網路；Cast Operations 負責執行您的事件應變、待命排班與狀態頁。將兩者連接起來，每個 Zabbix 問題都會自動成為一個 Cast Operations 事件——讓對的人收到呼叫，並讓您的狀態頁保持真實。

此整合為**入站（inbound）**：Zabbix 將問題傳送至 Cast Operations。它一端使用 Zabbix 的 **webhook 媒體類型（media type）**，另一端使用 Cast Operations 的 **[Workflow](/docs/workflows/index)**。不需要外掛、不需要額外服務。
...
2. 一個 Zabbix **action** 告知 **Cast Operations** 媒體類型傳送該事件。
3. 媒體類型的腳本將一個小型的 JSON 酬載 POST 至某個 Cast Operations workflow URL。
...
- 您的 Zabbix 伺服器必須能透過 HTTPS 連接到您的 Cast Operations 執行個體。
- 一個您可以建立 workflow 的 Cast Operations 專案。

## 第 1 部分 — 建置 Cast Operations workflow
...
4. 將一個 **Create Incident** 區塊拖曳進來，並將它連接至 Conditions 區塊的 **Yes** 輸出。填入：
   ...
   - **Severity**：挑選您想要的 Cast Operations 事件嚴重性（您稍後可以透過更多 Conditions 分支來細化此設定，將 Zabbix 嚴重性對應過來）。
...
1. 前往 **Users → Users → Create user**。將它命名為 `Cast Operations Webhook`，給它一個可以接收通知的角色（例如 **User role**），並將它加入某個使用者群組。
...
### 步驟 3：透過 action 將問題傳送至 Cast Operations
...
4. 在 **Operations** 分頁中，新增一個透過 **Cast Operations** 媒體類型傳送至 **User: Cast Operations Webhook** 的 operation。
5. 若要在稍後於復原時解決事件，也請在 **Recovery operations** 中以相同的使用者／媒體填入。
6. **Add** 以儲存，並確認此 action 為 **Enabled**。

## 第 3 部分 — 測試它

1. 回到 Cast Operations workflow，將 **Enabled** 開啟。
...
4. 在 Cast Operations 中查看 **Incidents**——您的 Zabbix 問題現在已成為一個事件。
...
Zabbix 嚴重性（`Not classified`、`Information`、`Warning`、`Average`、`High`、`Disaster`）會以 `{{Zabbix.Request Body.severity}}` 抵達。若要將它們對應至 Cast Operations 事件嚴重性，請在 **Create Incident** 之前新增 **Conditions** 分支——例如，將 `Disaster` 與 `High` 導向「Critical」事件，並將其餘所有內容導向「Major」。每個分支建置一個 **Create Incident** 區塊。
...
- 來自 Cast Operations 的非 2xx 回應會由腳本中的 `throw` 呈現出來——請檢查 workflow URL 是否完全正確。
...
- 如果您在防火牆後方自架運行 Cast Operations，請允許您 Zabbix 伺服器的出口 IP 透過 HTTPS 連接到它。
