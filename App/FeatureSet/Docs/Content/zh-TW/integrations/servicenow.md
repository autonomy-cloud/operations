...
每當建立 Cast Operations 事件時，自動開啟一則 [ServiceNow](https://www.servicenow.com) 事件——讓 ITSM 與監控保持同步。

此整合為**對外（outbound）**：Cast Operations 會呼叫 ServiceNow 的 [Table API](https://docs.servicenow.com/bundle/utah-application-development/page/integrate/inbound-rest/concept/c_TableAPI.html)。它使用一個帶有 **Incident → On Create** 觸發器與 **API 元件**的 Cast Operations **[Workflow](/docs/workflows/index)**。
...
- 一個你可以建立 workflow 的 Cast Operations 專案。
...
2. 在 Cast Operations 中，前往 **Workflows → Global Variables → Create**，將其命名為 `SERVICENOW_AUTH`，貼上該 base64 字串，並開啟 **Is Secret**。
...
3. 加入一個連接至觸發器的 **API** 區塊：
   ...
   `correlation_id` 會保留回連至 Cast Operations 事件的連結——如果你之後要加入解決步驟會很方便。ServiceNow 的 `urgency`/`impact` 使用 `1`（高）、`2`（中）、`3`（低）。
