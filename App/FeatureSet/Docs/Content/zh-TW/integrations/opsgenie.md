...
每當建立 Cast Operations 事件時，就建立一個 [Opsgenie](https://www.atlassian.com/software/opsgenie) 警報，並在 Cast Operations 解決時將其關閉。

此整合屬於**對外（outbound）**：Cast Operations 會呼叫 [Opsgenie Alert API](https://docs.opsgenie.com/docs/alert-api)。它使用具備 **Incident → On Create** 觸發器與 **API component** 的 Cast Operations **[Workflow](/docs/workflows/index)**。
...
3. 新增一個連接到該觸發器的 **API** 區塊：
   ...
   - **Method**：`POST`
   - **URL**：`https://api.opsgenie.com/v2/alerts` _(歐盟請使用 `api.eu.opsgenie.com`)_
   - **Headers**：
   ...
   **`alias`** 會將此 Opsgenie 警報與該 Cast Operations 事件繫結，讓你之後可以依 alias 將其關閉。請注意，Opsgenie 的驗證機制是字面字串 `GenieKey`，後接一個空格與你的金鑰。

4. **Save**、啟用，並建立一個測試事件。工作流程記錄中出現 `202 Accepted` 回應即表示 Opsgenie 已將警報排入佇列。

## 步驟 3 — 在 Cast Operations 解決時關閉（建議）
...
Opsgenie 的優先順序為 `P1`–`P5`。在 API 區塊之前，以 **Conditions** 分支依 `{{Incident.incidentSeverity.name}}` 從 Cast Operations 的嚴重性對應。
...
- [On Call](/docs/on-call/incoming-call-policy) — Cast Operations 內建的升級機制。
