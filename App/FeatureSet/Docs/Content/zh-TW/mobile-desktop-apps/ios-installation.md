...
在您的 iPhone 或 iPad 上，從 Apple App Store 安裝 **Cast Operations On-Call** 原生 iOS 應用程式。

## 需求

- 執行 **iOS 15.0 或更新版本**的 iPhone 或 iPad
- 一個有效的 Cast Operations 帳號（或您自架 Cast Operations 執行個體的 URL）
- 用於登入及接收推播通知的網際網路連線

## 從 App Store 安裝

1. 在您的 iPhone 或 iPad 上**開啟 App Store**。
2. 點按 **Search**（搜尋）分頁並搜尋 **"Cast Operations On-Call"**，或在您的裝置上開啟此連結：
...
4. 安裝完成後，點按 **Open**（開啟）或從主畫面啟動 **Cast Operations On-Call**。
...
2. **登入**
   - 輸入您 Cast Operations 帳號的電子郵件與密碼。
...
1. 前往 **Settings → Notifications → Cast Operations On-Call**。
...
5. 將 **Cast Operations On-Call** 加入任何您仍想接收呼叫的 **Focus**（專注模式）。

## 更新

應用程式透過 App Store 進行更新：

- 開啟 **App Store**，點按您的個人檔案圖片，捲動至 **Cast Operations On-Call**，然後點按 **Update**（更新）。
- 或啟用 **Settings → App Store → App Updates** 以自動安裝更新。

## 解除安裝

1. 在主畫面上**長按** **Cast Operations On-Call** 圖示。
2. 點按 **Remove App → Delete App**。
3. 點按 **Delete** 進行確認。

您的 Cast Operations 帳號與值班排程儲存在伺服器端，解除安裝應用程式時不會被移除。
...
- 如果您位於公司網路或 VPN，請確保可存取該 Cast Operations 執行個體。
- 確認伺服器是透過 HTTPS 並使用有效憑證提供服務。

**未收到推播通知：**

- 開啟 **Settings → Notifications → Cast Operations On-Call** 並確認已允許通知。
- 停用 **Do Not Disturb**（勿擾模式），或將 Cast Operations On-Call 加入您目前作用中專注模式的允許清單。
- 登出後再重新登入，以重新整理向伺服器註冊的推播權杖。
- 自架使用者：請確認您的 Cast Operations 執行個體已設定推播通知（請參閱自架的[推播通知](/docs/self-hosted/push-notifications)指南）。

**Face ID / Touch ID 無法運作：**

- 請確保已在 **Settings → Face ID & Passcode** 或 **Settings → Touch ID & Passcode** 中註冊生物辨識。
- 從 Cast Operations On-Call 應用程式內的 **Settings** 畫面重新啟用生物辨識解鎖。
...
如果您仍需協助，請透過您的 Cast Operations 儀表板聯絡我們，或在我們的 [GitHub repository](https://github.com/autonomy-cloud/operations) 上開立 issue。
