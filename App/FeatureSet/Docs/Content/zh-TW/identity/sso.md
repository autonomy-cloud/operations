...
Cast Operations 支援以 SAML 2.0 為基礎的單一登入（SSO），以進行企業驗證。SSO 讓您的團隊成員能夠使用貴組織的身分提供者（IdP）登入 Cast Operations，提供集中式的存取管理與更高的安全性。
...
- **降低密碼疲勞**：使用者不需要再記住另一組 Cast Operations 密碼
...
Keycloak 是熱門的開源身分與存取管理解決方案。請依照下列步驟，將 Keycloak 設定為 Cast Operations 的 SAML 身分提供者。

### 先決條件

- 一個已設定 realm 且正在執行的 Keycloak 執行個體
- 同時具有 Keycloak 與 Cast Operations 的管理員存取權
- 支援 SSO 的 Cast Operations 帳號

### 步驟 1：設定 Cast Operations SSO
...
4. 複製憑證值並以下列格式貼入 Cast Operations：
...
4. 將 **Client ID** 設為 Cast Operations **View SSO Config** 中的 **Identifier (Entity ID)** 值
5. 將 **Valid Redirect URIs** 設為您的 Cast Operations URL
6. 將 **Root URL** 設為您的 Cast Operations 基礎 URL
7. 將 Cast Operations 的 **Reply URL (Assertion Consumer Service URL)** 貼入 **Assertion Consumer Service POST Binding URL** 欄位
...
1. 在 Keycloak 與 Cast Operations 中儲存所有設定
2. 嘗試使用 SSO 登入 Cast Operations
3. 您應會被重新導向至您的 Keycloak 登入頁面，並在成功驗證後返回 Cast Operations
...
Microsoft Entra ID 是 Microsoft 以雲端為基礎的身分與存取管理服務。請依照下列步驟，將 Entra ID 設定為 Cast Operations 的 SAML 身分提供者。

### 先決條件

- Microsoft Entra ID 租用戶（任何支援具 SAML SSO 之企業應用程式的層級）
- 同時具有 Microsoft Entra ID 與 Cast Operations 的管理員存取權
- 支援 SSO 的 Cast Operations 帳號
...
3. 在 **Basic SAML Configuration** 中，點選 **Edit** 並設定：
   - **Identifier (Entity ID)**：貼上 Cast Operations **View SSO Config** 中的 **Identifier (Entity ID)**
   - **Reply URL (Assertion Consumer Service URL)**：貼上 Cast Operations **View SSO Config** 中的 **Reply URL**
4. 點選 **Save**
5. 在 **SAML Certificates** 區段中：
   - 下載 **Certificate (Base64)**
   - 在文字編輯器中開啟下載的憑證檔案並複製其內容
6. 在 **Set up Cast Operations** 區段中，複製：
   - **Login URL** — 將此貼為 Cast Operations 中的 **Sign On URL**
   - **Azure AD Identifier** — 將此貼為 Cast Operations 中的 **Issuer**
7. 返回 Cast Operations 並貼上憑證與 URL，然後儲存
...
1. 在 Entra ID 與 Cast Operations 中儲存所有設定
2. 嘗試使用 SSO 登入 Cast Operations
3. 您應會被重新導向至 Microsoft 登入頁面，並在成功驗證後返回 Cast Operations

### Microsoft Entra ID 疑難排解

- **AADSTS700016 錯誤**：Entra ID 中的 Identifier (Entity ID) 與 Cast Operations 不相符 — 請確認兩個值完全相同
- **憑證錯誤**：確認您下載的是 **Base64** 憑證（而非原始／二進位格式），並包含 `BEGIN CERTIFICATE` / `END CERTIFICATE` 兩行
- **使用者未指派**：使用者必須先明確指派至企業應用程式，才能透過 SSO 登入
- **Name ID 不相符**：確認 Name ID 宣告所設定的電子郵件地址，與該使用者在 Cast Operations 中的電子郵件相符

---

## Okta SAML 設定

Okta 是廣為使用的身分平台，提供強大的 SAML SSO 能力。請依照下列步驟，將 Okta 設定為 Cast Operations 的 SAML 身分提供者。
...
5. 在 **App name** 輸入「Cast Operations」並點選 **Next**
6. 在 **SAML Settings** 區段中，設定：
   - **Single sign-on URL**：貼上 Cast Operations **View SSO Config** 中的 **Reply URL (Assertion Consumer Service URL)**
   - **Audience URI (SP Entity ID)**：貼上 Cast Operations **View SSO Config** 中的 **Identifier (Entity ID)**
...
### 步驟 3：將 Okta SAML 中繼資料複製到 Cast Operations
...
3. 從中繼資料 XML 中，或從 **Sign On** 分頁的詳細資訊中：
   - 複製 **Sign On URL**（也稱為 **Identity Provider Single Sign-On URL**）— 將此貼為 Cast Operations 中的 **Sign On URL**
   - 複製 **Issuer**（也稱為 **Identity Provider Issuer**）— 將此貼為 Cast Operations 中的 **Issuer**
4. 下載簽署憑證：
   - 在 **SAML Signing Certificates** 區段中，針對使用中的憑證點選 **Actions** > **Download certificate**
   - 在文字編輯器中開啟下載的 `.cert` 檔案並複製其內容
   - 將憑證貼入 Cast Operations（包含 `BEGIN CERTIFICATE` 與 `END CERTIFICATE` 兩行）
5. 儲存 Cast Operations SSO 設定
...
1. 在 Okta 與 Cast Operations 中儲存所有設定
2. 嘗試使用 SSO 登入 Cast Operations
3. 您應會被重新導向至 Okta 登入頁面，並在成功驗證後返回 Cast Operations

### Okta 疑難排解

- **404 或無效的 SSO URL**：確認 Okta 中的 **Single sign-on URL** 與 Cast Operations 的 **Reply URL** 完全相符
- **Audience 不相符**：確認 Okta 中的 **Audience URI** 與 Cast Operations 的 **Identifier (Entity ID)** 完全相符
...
Cast Operations 的 SSO 實作採用 SAML 2.0 協定，應可與任何相容的身分提供者搭配運作。一般設定步驟如下：

1. 在 Cast Operations 中建立一個 SSO 設定，並從 **View SSO Config** 按鈕記下 **Identifier (Entity ID)** 與 **Reply URL (Assertion Consumer Service URL)**
2. 在您的身分提供者中，使用下列項目建立一個 SAML 應用程式：
   - **Assertion Consumer Service URL / Reply URL**：來自 Cast Operations SSO 設定
   - **Entity ID / Audience URI**：來自 Cast Operations SSO 設定
   - **Name ID Format**：電子郵件地址
3. 從您的身分提供者中，將下列項目複製到 Cast Operations：
   - **Sign On URL**（SSO 端點）
...
Cast Operations 目前不支援從您的身分提供者對應 SAML 角色。以角色為基礎的存取必須在 Cast Operations 的 **Project Settings** > **SSO** 設定中個別設定，您可以在該處為 SSO 使用者指派預設角色。
