...
## Cast Operations 資料元件

對於 Cast Operations 中每一種記錄（監視器、事件、警示、狀態頁面、待命政策，以及更多），調色盤都有這些元件——以類型名稱搜尋：
...
這就是工作流程讀取與變更 Cast Operations 資料的方式。例如：來自您 CI 工具的 webhook 可以使用 **Create Incident**，以失敗的詳細資訊開啟一個事件。

## 我應該使用哪個元件？

幾個快速原則：

- 如果有專屬區塊可以滿足您的需求（Slack、Email、某個 Cast Operations 記錄），就使用它——您會獲得更好的錯誤處理與更清楚的記錄。
