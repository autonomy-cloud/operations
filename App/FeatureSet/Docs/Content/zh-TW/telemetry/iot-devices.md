# Cast Operations IoT 裝置

## 概觀

Cast Operations 透過擷取標準的 OpenTelemetry (OTLP) 指標來監控 IoT 裝置機群 — 感測器、閘道器、控制器與邊緣裝置。每個裝置（或代表它的閘道器）會透過 OTLP HTTP 推送一小組 `iot_*` 指標，並標註它所屬的**機群（fleet）**以及自身的**裝置 id（device id）**。Cast Operations 會將這些指標歸入一個機群，建立即時的裝置清單，並追蹤每個裝置的電池、連線狀態、溫度、CPU、記憶體與可用性。
...
Cast Operations 使用 OpenTelemetry 資源屬性將你的裝置對應到兩個概念：

- **機群（Fleet）** — 裝置的邏輯群組（例如 `building-a-sensors` 或 `field-gateways`）。機群是從 `iot.fleet.name` 資源屬性衍生而來，並在 Cast Operations 中以遙測服務 `iot/<fleet>` 的形式出現。請設定 `service.name=iot/<fleet>`，讓記錄與指標能對齊到同一個服務之下。
- **裝置（Device）** — 機群內的個別裝置，由 `device.id` 屬性識別。Cast Operations 會以 `device.id` 為鍵，建立並維護每個機群的裝置清單。

選用屬性可進一步精細化每個裝置在監控器中的分類與範圍界定：

| 屬性                 | 必填     | 說明                                                                             |
| -------------------- | -------- | -------------------------------------------------------------------------------- |
| `iot.fleet.name`     | 是       | 此裝置所屬的機群。會成為 Cast Operations 服務 `iot/<fleet>`                            |
...
當許多裝置透過閘道器回報時，請在閘道器上執行一個 OpenTelemetry Collector 並匯出到 Cast Operations。`resource` 處理器會標記機群屬性；從你的裝置接收讀數（OTLP、MQTT 橋接、檔案記錄等）並將它們轉送出去：
...
- 請在每個資料點上保留 `device.id`（以及選用的 `iot.device.kind` / `iot.device.type` / `iot.device.firmware`），讓 Cast Operations 能解析出機群內的個別裝置。
- **`otlphttp`** 會透過 HTTPS 並附帶擷取權杖傳送到 Cast Operations。請注意 `encoding: json` 與 `Content-Type: application/json` 標頭為必填。

## 透過 MQTT 傳送指標

Cast Operations 內建了 MQTT 端點，因此已經會使用 MQTT 的裝置可以直接推送讀數 — 不需要 OpenTelemetry SDK、collector 或橋接。透過 MQTT 發布的一切都會進入與 OTLP 相同的管線：機群會自動建立、裝置清單會更新，而且每個 IoT 監控器與警示範本都能原封不動地運作。

**端點**

| 傳輸方式              | 位址                                    | 備註                                                                                     |
| --------------------- | --------------------------------------- | ---------------------------------------------------------------------------------------- |
| MQTT over WebSocket   | `wss://<your-host>/mqtt`                | 適用於所有部署 — 透過 Cast Operations ingress 走一般的 HTTPS 連接埠                            |
...
- 在主題約定之外發布，或是酬載格式錯誤的發布，會被接受並**丟棄**（MQTT 3.1.1 沒有針對單一訊息的錯誤回覆） — 伺服器會記錄一則含有原因的警告，因此若資料未送達，請檢查 Cast Operations 應用程式記錄。
- 在 WebSocket 端點上，請將 MQTT keepalive 保持在 **5 分鐘以內** — Cast Operations ingress 會在 300 秒後關閉閒置的 WebSocket 連線，這會觸發你的 Last Will 以及一則錯誤的 Device Offline 警示。用戶端函式庫的預設值（`mqtt` 與 `paho-mqtt` 皆為 60 秒）沒有問題。原始 TCP 端點則沒有這樣的上限。
- 酬載上限為每次發布 128 KB 與 100 個指標；過大的封包會中斷連線。

## 指標慣例

Cast Operations 可辨識下列 `iot_*` 指標名稱。每個資料點都應帶有 `device.id` 標籤，讓讀數歸屬於正確的裝置。你只需要傳送對你的裝置有意義的指標 — 缺少的指標單純不會繪製成圖表。
...
| `iot_cpu_usage_ratio`       | CPU 使用率，以 `0`–`1` 的比值表示（Cast Operations 會以百分比儲存）                   |
...
2. 在 Cast Operations 儀表板中，開啟 **IoT** 區段 — 你的機群應在大約一分鐘內以 `iot/<fleet>` 的形式出現。
...
2. 請記得 `iot_cpu_usage_ratio` 是 `0`–`1` 的比值；傳送原始比值，Cast Operations 會將它以百分比呈現。
3. 裝置開始回報後，請預留最多一分鐘讓第一批資料點浮現。

## 自我託管的 Cast Operations

如果你正在自我託管 Cast Operations，請將端點指向你自己的執行個體：
...
- 若要深入了解底層的 OTLP 整合，請參閱[將 OpenTelemetry 與 Cast Operations 整合](/docs/telemetry/open-telemetry)。
