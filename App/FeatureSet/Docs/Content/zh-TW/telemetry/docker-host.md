...
請將 `YOUR_CAST_OPERATIONS_URL`、`YOUR_TELEMETRY_INGESTION_TOKEN` 以及主機名稱替換為您環境的對應值。主機名稱即為此 Docker 主機在 Cast Operations 中顯示的名稱 — 請挑選類似 `prod-docker-01` 的名稱。
...
如果您是自架 Cast Operations，請將 `CAST_OPERATIONS_URL` 設定為您自己的執行個體：
...
1. 檢查 Agent 是否正在執行：`docker ps --filter name=cast-operations-docker-agent`
2. 檢查 Agent 日誌：`docker logs cast-operations-docker-agent | grep -i error`
3. 確認您的 Cast Operations URL 與服務權杖正確無誤
4. 確認您的 Docker 主機可透過網路連線至 Cast Operations 執行個體

### 沒有出現任何指標

1. 確認 Agent 內部可存取 Docker socket：`docker exec cast-operations-docker-agent ls -la /var/run/docker.sock`
2. 檢查 collector 日誌是否有匯出錯誤：`docker logs cast-operations-docker-agent | tail -100`
3. 確認您的服務權杖有效且尚未過期
