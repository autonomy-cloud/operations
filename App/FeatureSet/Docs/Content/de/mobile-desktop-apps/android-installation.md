...
Installieren Sie die native Android-App **Cast Operations On-Call** aus dem Google Play Store, oder installieren Sie die APK direkt auf Geräten ohne Google Play per Sideloading.

## Voraussetzungen

- Android-Telefon oder -Tablet mit **Android 8.0 (Oreo) oder neuer**
- Ein aktives Cast Operations-Konto (oder die URL Ihrer selbst gehosteten Cast Operations-Instanz)
...
2. Suchen Sie nach **„Cast Operations On-Call"**, oder öffnen Sie diesen Link auf Ihrem Gerät:
...
4. Sobald die Installation abgeschlossen ist, tippen Sie auf **Öffnen** oder starten Sie **Cast Operations On-Call** über Ihre App-Übersicht.
...
4. Starten Sie **Cast Operations On-Call** über Ihre App-Übersicht.

Die APK wird von Cast Operations aus derselben Quelle wie die Play-Store-Veröffentlichung erstellt und signiert. App-Updates erfolgen beim Sideloading nicht automatisch — laden Sie die neueste APK über den obigen Link herunter, wenn eine neue Version veröffentlicht wird.
...
2. **Anmelden**
   - Geben Sie die E-Mail-Adresse und das Passwort Ihres Cast Operations-Kontos ein.
...
1. Öffnen Sie **Einstellungen → Apps → Cast Operations On-Call → Benachrichtigungen** und bestätigen Sie, dass alle Kategorien aktiviert sind.
2. Öffnen Sie **Einstellungen → Apps → Cast Operations On-Call → Akku** und wählen Sie **Unbeschränkt** (oder deaktivieren Sie die Akkuoptimierung), damit das Betriebssystem Hintergrund-Pushes nicht verzögert.
3. Erlauben Sie der App, im Hintergrund zu laufen, und deaktivieren Sie alle „Datensparmodus"-Einschränkungen für sie.
4. Wenn Sie Samsung-Geräte verwenden, schalten Sie zusätzlich **Einstellungen → Gerätewartung → Akku → Begrenzungen für Hintergrundaktivität** für Cast Operations On-Call aus.
5. Fügen Sie Cast Operations On-Call zu allen Ausnahmelisten für **Do Not Disturb** (Nicht stören) hinzu, damit Benachrichtigungen während Ihrer Bereitschaftsschicht weiterhin klingeln.

## Updates

**Google Play:**

- Updates werden automatisch installiert. Um ein Update manuell auszulösen, öffnen Sie **Play Store → Profil → Apps und Gerät verwalten → Verfügbare Updates → Cast Operations On-Call → Aktualisieren**.
...
1. **Halten Sie das Symbol von Cast Operations On-Call** gedrückt und tippen Sie dann auf **Deinstallieren**.
2. Oder öffnen Sie **Einstellungen → Apps → Cast Operations On-Call → Deinstallieren**.
3. Bestätigen Sie, um die App zu entfernen.

Ihr Cast Operations-Konto und Ihre Bereitschaftspläne werden serverseitig gespeichert und nicht entfernt, wenn Sie die App deinstallieren.
...
- Wenn Sie sich in einem Unternehmensnetzwerk oder über ein VPN verbinden, stellen Sie sicher, dass die Cast Operations-Instanz erreichbar ist.
...
- Vergewissern Sie sich, dass Benachrichtigungen unter **Einstellungen → Apps → Cast Operations On-Call → Benachrichtigungen** aktiviert sind.
- Deaktivieren Sie die Akkuoptimierung für Cast Operations On-Call (siehe Push-Benachrichtigungen oben).
- Stellen Sie sicher, dass Do Not Disturb (Nicht stören) ausgeschaltet ist oder dass Cast Operations On-Call auf der Ausnahmeliste steht.
- Melden Sie sich ab und wieder an, um das beim Server registrierte Push-Token zu aktualisieren.
- Selbst gehostete Benutzer: Vergewissern Sie sich, dass Push-Benachrichtigungen auf Ihrer Cast Operations-Instanz konfiguriert sind (siehe die Anleitung [Push Notifications](/docs/self-hosted/push-notifications) für selbst gehostete Instanzen).
...
- Aktivieren Sie die biometrische Entsperrung erneut im **Einstellungen**-Bildschirm innerhalb der Cast Operations On-Call-App.
...
Wenn Sie weiterhin Hilfe benötigen, wenden Sie sich über Ihr Cast Operations-Dashboard an uns oder eröffnen Sie ein Issue in unserem [GitHub-Repository](https://github.com/autonomy-cloud/operations).
