# Cast Operations kostenlos mit Docker Compose bereitstellen

Wenn Sie Cast Operations auf Ihrem eigenen Server hosten möchten, können Sie Docker Compose verwenden, um eine Einzelserver-Instanz von Cast Operations auf Debian, Ubuntu oder RHEL bereitzustellen. Diese Option gibt Ihnen mehr Kontrolle und Anpassungsmöglichkeiten für Ihre Instanz, erfordert jedoch auch mehr technische Kenntnisse und Ressourcen für die Bereitstellung und Wartung.

#### Systemanforderungen wählen

Je nach Nutzung und Budget können Sie verschiedene Systemanforderungen für Ihren Server wählen. Für optimale Leistung empfehlen wir die Verwendung von Cast Operations mit:
...
- **Homelab / Minimalanforderungen**
  - Wenn Sie Cast Operations für den persönlichen oder experimentellen Einsatz in einer Heimumgebung betreiben möchten (einige unserer Benutzer haben es sogar auf einem RaspberryPi installiert), können Sie die Homelab-Anforderungen verwenden:
    - 8 GB RAM
    - 4 Kerne
    - 20 GB Festplatte
    - Docker und Docker Compose installiert
...
Um Cast Operations zu installieren:
...
### Zugriff auf Cast Operations

Cast Operations sollte unter http://localhost erreichbar sein. Sie müssen ein neues Konto für Ihre Instanz registrieren, um es zu verwenden.

### TLS/SSL-Zertifikate einrichten

Cast Operations unterstützt **nicht** das Einrichten von SSL/TLS-Zertifikaten. Sie müssen SSL/TLS-Zertifikate selbst einrichten.
...
3. Richten Sie den Reverse-Proxy auf den Cast Operations-Server.
...
Idealerweise sollten Sie Cast Operations in der Produktion nicht mit docker-compose bereitstellen. Wir empfehlen dringend die Verwendung von Kubernetes. Es gibt ein Helm-Chart für Cast Operations [hier](https://artifacthub.io/packages/helm/autonomy-cloud/operations).

Wenn Sie Cast Operations trotzdem in der Produktion mit docker-compose bereitstellen möchten, beachten Sie bitte Folgendes:

- **SSL/TLS**: Richten Sie SSL/TLS-Zertifikate ein. Cast Operations unterstützt das Einrichten von SSL/TLS-Zertifikaten nicht. Sie müssen SSL/TLS-Zertifikate selbst einrichten. Siehe oben.
...
- **Updates**: Bitte aktualisieren Sie Cast Operations regelmäßig. Wir veröffentlichen täglich Updates. Wir empfehlen, die Software mindestens einmal pro Woche zu aktualisieren, wenn Sie sie in der Produktion betreiben.

### Cast Operations aktualisieren
...
- In unserem Docker-Setup verwenden wir einen lokalen Logging-Treiber. Cast Operations, insbesondere in den Probe- und Ingest-Containern, generiert eine erhebliche Menge an Logs. Um zu verhindern, dass Ihr Speicher voll wird, ist es entscheidend, den Logging-Speicher in Docker zu begrenzen. Detaillierte Anweisungen hierzu finden Sie in der offiziellen Docker-Dokumentation [hier](https://docs.docker.com/config/containers/logging/local/).

### Cast Operations deinstallieren

Um Cast Operations zu deinstallieren, führen Sie den folgenden Befehl aus:

```
npm run down
```

Dadurch werden alle von Cast Operations erstellten Container, Netzwerke und Volumes gestoppt und entfernt. Die `config.env`-Datei oder das geklonte Repository werden nicht entfernt.
