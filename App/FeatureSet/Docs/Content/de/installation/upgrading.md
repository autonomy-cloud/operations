...
Diese Anleitung beschreibt, wie Sie Ihre selbst gehostete Cast Operations-Installation sicher aktualisieren können.
...
In jeder Telemetrie-Tabelle werden zwei Spalten umbenannt: `serviceId` → `primaryEntityId` und `serviceType` → `primaryEntityType`. Dies ist eine harte Umbenennung – **wenn Sie die Cast Operations-Analytics-API direkt mit `serviceId`-/`serviceType`-Filtern abfragen, stellen Sie diese auf die neuen Namen um.** Dashboards, Monitore und Alerts innerhalb von Cast Operations werden automatisch migriert.
...
- Die Kopie kann sicher ausgeführt werden, während Cast Operations live ist. Neue Telemetriedaten werden unabhängig davon in die neuen Tabellen geschrieben; die kopierte Historie füllt sich dahinter auf.
...
Führen Sie anschließend das Upgrade durch und lassen Sie Cast Operations vollständig hochfahren, bevor Sie fortfahren.
...
## Upgrade von Cast Operations 7 → 8
