...
Cast Operations unterstützt SAML 2.0-basiertes Single Sign-On (SSO) für die Enterprise-Authentifizierung. SSO ermöglicht Ihren Teammitgliedern, sich bei Cast Operations mit den Anmeldedaten Ihrer Organisation anzumelden und bietet zentralisiertes Zugriffsmanagement und erhöhte Sicherheit.
...
- **Reduzierte Passwortmüdigkeit**: Benutzer müssen sich kein separates Cast Operations-Passwort merken
...
3. **Cast Operations SSO-Metadaten abrufen**
...
### Schritt 1: Cast Operations SSO konfigurieren
...
3. In **Grundlegende SAML-Konfiguration** klicken Sie auf **Bearbeiten** und setzen Sie:
   - **Bezeichner (Entity ID)**: Den **Bezeichner (Entity ID)** aus der Cast Operations **SSO-Konfiguration anzeigen** einfügen
   - **Antwort-URL (Assertion Consumer Service URL)**: Die **Antwort-URL** aus der Cast Operations **SSO-Konfiguration anzeigen** einfügen
4. Klicken Sie auf **Speichern**

### Fehlerbehebung Microsoft Entra ID

- **AADSTS700016-Fehler**: Der Bezeichner (Entity ID) in Entra ID stimmt nicht mit Cast Operations überein
...
5. Geben Sie "Cast Operations" als **App-Name** ein und klicken Sie auf **Weiter**
6. Im Abschnitt **SAML-Einstellungen** konfigurieren Sie:
   - **Single Sign-On URL**: Die **Antwort-URL (Assertion Consumer Service URL)** aus der Cast Operations **SSO-Konfiguration anzeigen** einfügen
   - **Audience URI (SP Entity ID)**: Den **Bezeichner (Entity ID)** aus der Cast Operations **SSO-Konfiguration anzeigen** einfügen
   - **Name-ID-Format**: `EmailAddress` auswählen
   - **Anwendungsbenutzername**: `E-Mail` auswählen

### Fehlerbehebung Okta

- **404 oder ungültige SSO-URL**: Sicherstellen, dass die **Single Sign-On URL** in Okta exakt mit der **Antwort-URL** aus Cast Operations übereinstimmt
- **Audience Mismatch**: Sicherstellen, dass die **Audience URI** in Okta exakt mit dem **Bezeichner (Entity ID)** aus Cast Operations übereinstimmt
...
Cast Operations’ SSO-Implementierung verwendet das SAML 2.0-Protokoll und sollte mit jedem kompatiblen Identity Provider funktionieren. Die allgemeinen Konfigurationsschritte sind:

1. In Cast Operations eine SSO-Konfiguration erstellen und den **Bezeichner (Entity ID)** und die **Antwort-URL** notieren
2. In Ihrem Identity Provider eine SAML-Anwendung erstellen mit:
   - **Assertion Consumer Service URL / Antwort-URL**: Aus der Cast Operations SSO-Konfiguration
   - **Entity ID / Audience URI**: Aus der Cast Operations SSO-Konfiguration
   - **Name-ID-Format**: E-Mail-Adresse
3. Von Ihrem Identity Provider folgendes in Cast Operations einfügen:
...
Cast Operations unterstützt derzeit keine Zuordnung von SAML-Rollen aus Ihrem Identity Provider. Die rollenbasierte Zugriffssteuerung muss separat innerhalb von Cast Operations’ **Projekteinstellungen** > **SSO** konfiguriert werden.
