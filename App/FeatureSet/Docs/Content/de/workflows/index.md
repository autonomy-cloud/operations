...
Mit Workflows automatisieren Sie Aufgaben in Cast Operations, ohne Code schreiben zu müssen. Ziehen Sie ein paar Bausteine auf eine Arbeitsfläche, verbinden Sie sie miteinander, und schon haben Sie eine Automatisierung, die ausgeführt wird, sobald etwas passiert – ein Vorfall wird geöffnet, ein Zeitplan löst aus, oder ein anderes Tool sendet Daten an Cast Operations.
...
- **Cast Operations mit Ihren anderen Tools verbinden** – Vorfälle an Slack senden, Jira-Tickets erstellen, Webhooks in Ihrem Stack auslösen.
- **Auf Ereignisse in Cast Operations reagieren** – wenn ein kritischer Vorfall erstellt wird, das Bereitschaftsteam benachrichtigen und automatisch ein Ticket öffnen.
- **Aufgaben zeitgesteuert ausführen** – alle fünf Minuten, jede Nacht, jeden Montagmorgen.
- **Daten von außen empfangen** – andere Systeme können Daten über eine eindeutige URL an Cast Operations senden.
...
1. **Ein Auslöser** – startet den Workflow. Das kann ein manueller Knopf, ein Zeitplan, ein eingehender Webhook oder ein Ereignis in Cast Operations sein (zum Beispiel ein neuer Vorfall).
...
## Wo Sie Workflows in Cast Operations finden
...
2. **Auslöser auswählen** – manuell, zeitgesteuert, Webhook oder ein Ereignis aus Cast Operations.
...
## Wie Workflows in Cast Operations hineinpassen
