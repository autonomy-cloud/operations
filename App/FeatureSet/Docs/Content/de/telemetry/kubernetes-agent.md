...
Der Cast Operations Kubernetes Agent ist ein vorgefertigtes Helm-Chart, das eine OpenTelemetry-basierte Collector-Pipeline auf Ihrem Cluster installiert. Er liefert Node-, Pod-, Container- und Cluster-Metriken; Kubernetes-Events; Pod-Logs; und — mit standardmäßig aktiviertem eBPF — Anwendungs-Traces, HTTP-RED-Metriken, Service-Graph-Daten sowie Pod-zu-Pod-Netzwerkfluss-Metriken. Keine Codeänderungen, keine SDKs, ein einziges `helm install`.
...
- `helm` v3 installiert
- Ein **Cast Operations-API-Schlüssel** — erstellen Sie einen unter _Project Settings → API Keys_
...
Ersetzen Sie `YOUR_CAST_OPERATIONS_URL`, `YOUR_CAST_OPERATIONS_API_KEY` und den Cluster-Namen durch Werte für Ihre Umgebung. Der Cluster-Name ist die Art und Weise, wie das Cluster in Cast Operations erscheint — wählen Sie etwas Stabiles wie `prod-us-east-1`.
...
Sobald sich der Agent verbindet, erscheint Ihr Cluster automatisch im Abschnitt **Kubernetes** des Cast Operations-Dashboards.

## Konfigurationsoptionen

### Namespace-Filterung

`namespaceFilters.rules` wendet Namespace-Muster unabhängig auf vier Bereiche an:

- `podLogs`: filtert Pod-stdout/stderr am hostPath-filelog-Receiver oder API-Log-Tailer; Kubernetes-Ereignisse und Audit-Logs bleiben unverändert.
- `ebpfDiscovery`: filtert die OBI-Prozesserkennung und damit sowohl eBPF-Traces als auch eBPF-Metriken.
- `metrics`: filtert Namespace-bezogene Metrikreihen nach der Metadatenanreicherung; Node- und Cluster-Reihen ohne Namespace bleiben erhalten.
- `traces`: filtert nach der Metadatenanreicherung sowohl eBPF-Spans als auch von Anwendungen gesendete OTLP-Spans.

Namespace-Muster gleichen den vollständigen Namen ab und unterstützen * als Platzhalter, zum Beispiel team-*. Sobald ein Bereich eine include-Regel besitzt, werden dort nur passende Namespaces beibehalten. exclude-Regeln haben immer Vorrang. Die Standardregel schließt kube-system nur für podLogs und ebpfDiscovery aus.

So beschränken Sie Pod-Logs und die eBPF-Erkennung auf bestimmte Namespaces:
...
Um Logs eines lauten Namespace zu stoppen, aber dessen eBPF-Traces, Service Map und Metriken zu behalten, beschränken Sie den Ausschluss auf podLogs:
...
Regeln für podLogs und ebpfDiscovery filtern an der Quelle: ausgeschlossene Logdateien werden nie geöffnet und ausgeschlossene Workloads nie instrumentiert. Regeln für metrics und traces laufen später im Collector, nachdem die Namespace-Metadaten hinzugefügt wurden.

#### Metriken und Traces nach Namespace filtern

Fügen Sie diese Bereiche direkt zur Regel hinzu, wenn Sie auch Namespace-bezogene Metriken oder Spans filtern möchten:

```bash
  --set-json 'namespaceFilters.rules=[{"action":"exclude","namespaces":["kube-system","noisy-*"],"scopes":["podLogs","ebpfDiscovery","metrics","traces"]}]'
```

> **Metriken auf Node- und Cluster-Ebene bleiben immer erhalten. Ein Namespace gehört zu einem Pod, nicht zu einem Node; Reihen ohne Namespace entsprechen daher keiner Regel und werden nicht gelöscht.**

Kubernetes-Ereignisse können im Agent nicht nach Namespace gefiltert werden. Sie kommen vom k8sobjects-Receiver ohne das Attribut k8s.namespace.name; der Namespace steht im Ereignistext. Filtern Sie sie stattdessen serverseitig.
...
**Ihre span-basierten Monitore schon.** Alles, was Cast Operations aus den Spans selbst ableitet, skaliert mit der Rate herunter — siehe die Warnung weiter unten, bevor Sie dies aktivieren.
...
Jedes Ressourcenattribut mit dem Präfix `cast-operations.label.` wird zu einem Projekt-Label hochgestuft und an das Cluster, die Services und Hosts angehängt, die von diesem Agent ausgegeben werden. Muster: `cast-operations.label.<dimension>=<value>` wird zu einem Label mit dem Namen `<dimension>:<value>`.

Übergeben Sie Labels zur Installationszeit mit `--set cast-operations.labels.<key>=<value>`:
...
Labels werden ohne Beachtung der Groß-/Kleinschreibung abgeglichen, sodass ein vorhandenes, manuell erstelltes `Production`-Label wiederverwendet statt dupliziert wird. In der Cast Operations-Benutzeroberfläche manuell hinzugefügte Labels werden vom Agent niemals entfernt.
...
Das Chart führt ein DaemonSet mit [OpenTelemetry eBPF Instrumentation (OBI)](https://opentelemetry.io/docs/zero-code/obi/) auf jedem Node aus. Es lädt eBPF-Programme in den Kernel und erfasst automatisch HTTP/HTTPS-, gRPC- und SQL/Redis-Verkehr aus jeder unterstützten Laufzeitumgebung (Go, .NET, Java, Node.js, Python, Ruby, Rust) — kein SDK und kein Sidecar erforderlich. Traces und Request-Metriken fließen dann über den clusterinternen Collector zu Cast Operations.
...
Standardmäßig ist der Agent auf **Abdeckung** ausgelegt — er liefert Metriken, Pod-Logs und eBPF-Traces aus dem gesamten Cluster, sodass jedes Dashboard und jeder Monitor vom ersten Tag an funktioniert. Auf großen oder ausgelasteten Clustern kann das mehr Telemetrie sein, als Sie benötigen, was sich als höheres Ingest-Volumen niederschlägt (und, auf Cast Operations Cloud, als höhere Kosten). Nichts davon ist erforderlich, aber wenn ein Cluster mehr sendet, als Sie möchten, sind dies die Stellschrauben, an denen Sie drehen können — ungefähr in der Reihenfolge ihrer Auswirkung.
...
| **Metrik-Kardinalität**         | Anzahl der eindeutigen Serien (pro Container, pro PVC, …) | `filters.metrics.exclude`, `namespaceFilters.rules` (`metrics`), `cadvisor.metricsAllowlist`, `kubeletstats.volumeMetrics` |
...
Alle drei sind **unumkehrbar**: Was Sie hier verwerfen, erreicht Cast Operations nie, und alle drei können einen Monitor verstummen lassen. Die ersten beiden lassen einen Monitor verstummen, indem sie das Signal entfernen, das er beobachtet. Sampling ist enger gefasst: Die eBPF-RED-Metriken werden berechnet, bevor der Sampler läuft, sodass metrikbasierte Monitore exakt bleiben — aber Monitore, die *Spans* zählen (**Traces** auf `Span Count`, **Exceptions** auf `Exception Count`), sehen proportional weniger und benötigen ihre Schwellenwerte um denselben Faktor neu justiert. Wenn Sie lieber später entscheiden möchten, kann Cast Operations Daten stattdessen serverseitig verwerfen (**Logs → Settings → Drop Filters**, **Metrics → Settings → Pipeline Rules**) — das kostet weiterhin Egress, ist aber eine Einstellung, die Sie ohne erneutes Deployment ändern können.
...
- **Sie möchten nur Logs aus bestimmten Namespaces? Verwenden Sie eine include-Regel mit dem Bereich podLogs. Der Abgleich erfolgt an der Logquelle, sodass gefilterte Namespaces nie gelesen werden und die eBPF-Telemetrie unabhängig bleibt.**
  ...
  Um alle Namespaces außer einer lauten Gruppe zu behalten, verwenden Sie eine exclude-Regel mit namespaces: [noisy-*] und scopes: [podLogs].
...
- **Benötigen Sie überhaupt keine Pod-Logs von Cast Operations?** Schalten Sie sie aus:
...
- **Sie möchten die Metriken eines ganzen Namespace verwerfen? Fügen Sie eine exclude-Regel mit dem Bereich metrics hinzu. Reihen pro Pod und Container werden gefiltert; Node- und Cluster-Reihen ohne Namespace bleiben erhalten.**
...
> **Schnellster Weg — das Diagnoseskript ausführen.** Es prüft die Pod-Gesundheit, dekodiert und validiert den Ingestion-Schlüssel, kontrolliert, ob Ihr Cluster Cast Operations erreichen kann, und fragt Cast Operations, ob Ihr Token tatsächlich akzeptiert wird — und gibt dann eine einzige Ursachendiagnose aus:
>
> ```bash
...
1. Prüfen Sie, ob die Agent-Pods laufen: `kubectl get pods -n cast-operations-agent`
2. Prüfen Sie die Metrics-Collector-Logs: `kubectl logs -n cast-operations-agent -l component=metrics-collector -c otel-collector` (keine Fehler hier bedeutet **nicht**, dass Daten ankommen — siehe oben)
...
4. Stellen Sie sicher, dass Ihre Cast Operations-URL korrekt ist und Ihr Cluster sie über das Netzwerk erreichen kann.
...
1. Bestätigen Sie, dass der Log-Tailer-Pod Ready ist: `kubectl get pods -n cast-operations-agent -l component=log-collector`
2. Prüfen Sie dessen `/healthz` — es meldet die Anzahl aktiver Streams und den letzten Export-Fehler
3. Prüfen Sie die Logs: `kubectl logs -n cast-operations-agent deployment/kubernetes-agent-logs`
4. Bei sehr großen Clustern kann eine Replik zum Engpass werden — teilen Sie separate Releases mit podLogs-bezogenen include-Regeln in namespaceFilters.rules auf.
...
1. Bestätigen Sie, dass das eBPF-DaemonSet gesund ist: `kubectl get pods -n cast-operations-agent -l component=ebpf-instrument`
2. Aktivieren Sie den Debug-Trace-Printer, um zu bestätigen, dass OBI Verkehr erfasst: `--set ebpf.printTraces=true --set ebpf.logLevel=debug`, prüfen Sie dann `kubectl logs -n cast-operations-agent -l component=ebpf-instrument --tail=200`
3. Wenn Sie Spans in OBIs stdout sehen, aber nicht im Dashboard, liegt das Problem beim Export Collector → Cast Operations — prüfen Sie die Logs des Metrics-Collector-Pods.
