...
Cast Operations поддерживает протокол SCIM v2.0 для автоматизированного создания и отзыва прав доступа пользователей. SCIM позволяет поставщикам удостоверений (IdP), таким как Azure AD, Okta и другим корпоративным системам управления удостоверениями, автоматически управлять доступом пользователей к проектам и страницам статуса Cast Operations.

## Обзор

Интеграция SCIM предоставляет следующие преимущества:

- **Автоматическое создание пользователей**: Автоматически создавать пользователей в Cast Operations при их назначении в вашем IdP
- **Автоматический отзыв прав доступа**: Автоматически удалять пользователей из Cast Operations при снятии с них назначения в вашем IdP
- **Синхронизация атрибутов пользователей**: Поддерживать актуальность информации о пользователях между вашим IdP и Cast Operations
- **Централизованное управление доступом**: Управлять доступом к Cast Operations из существующей системы управления удостоверениями

## SCIM для проектов

Проектный SCIM позволяет поставщикам удостоверений управлять членами команды в проектах Cast Operations.
...
1. **Назначение пользователя в IdP**: Когда пользователь назначается в Cast Operations в вашем IdP
2. **Создание через SCIM**: IdP обращается к SCIM API Cast Operations для создания пользователя
3. **Членство в команде**: Пользователь автоматически добавляется в настроенные команды по умолчанию
4. **Предоставление доступа**: Пользователь теперь может получить доступ к проекту Cast Operations
5. **Снятие назначения**: Когда назначение пользователя снимается в IdP
6. **Отзыв прав через SCIM**: IdP обращается к SCIM API Cast Operations для удаления пользователя
...
1. **Перейдите в настройки страницы статуса**
   ...
   - Откройте вашу страницу статуса Cast Operations
...
1. **Назначение пользователя в IdP**: Когда пользователь назначается на страницу статуса Cast Operations в вашем IdP
2. **Создание через SCIM**: IdP обращается к SCIM API Cast Operations для создания подписчика
...
5. **Отзыв прав через SCIM**: IdP обращается к SCIM API Cast Operations для удаления подписчика
...
Microsoft Entra ID предоставляет корпоративное управление удостоверениями с надёжными возможностями SCIM-provisioning. Следуйте этим подробным шагам для настройки SCIM-provisioning с Cast Operations.

#### Предварительные требования

- Клиент Microsoft Entra ID с лицензией Premium P1 или P2 (требуется для автоматического provisioning)
- Аккаунт Cast Operations с планом Scale или выше
- Права администратора в Microsoft Entra ID и Cast Operations

#### Шаг 1: Получение конфигурации SCIM из Cast Operations
...
1. В вашем корпоративном приложении Cast Operations перейдите в **Provisioning**
...
| Атрибут Azure AD                                              | Атрибут SCIM Cast Operations         | Обязательный  |
| ------------------------------------------------------------- | ------------------------------ | ------------- |
...
Если вы включили **Push Groups** в Cast Operations:
...
| Атрибут Azure AD | Атрибут SCIM Cast Operations |
| ---------------- | ---------------------- |
...
1. В вашем корпоративном приложении Cast Operations перейдите в **Users and groups**
2. Нажмите **+ Add user/group**
3. Выберите пользователей и/или группы, которых хотите предоставить в Cast Operations
4. Нажмите **Assign**
...
Okta предоставляет гибкое управление удостоверениями с отличной поддержкой SCIM. Следуйте этим подробным шагам для настройки SCIM-provisioning с Cast Operations.

#### Предварительные требования

- Клиент Okta с возможностями provisioning (функция Lifecycle Management)
- Аккаунт Cast Operations с планом Scale или выше
- Права администратора в Okta и Cast Operations
...
3. Найдите и выберите существующее приложение Cast Operations
...
5. Введите "Cast Operations" в качестве имени приложения
6. Завершите конфигурацию SAML (см. документацию по SSO)
7. Нажмите **Finish**

#### Шаг 3: Включение SCIM-provisioning

1. В вашем приложении Cast Operations перейдите на вкладку **General**
...
| Атрибут Okta       | Атрибут SCIM Cast Operations          | Направление |
| ------------------ | ------------------------------- | ----------- |
...
2. Фильтруйте события, связанные с вашим приложением Cast Operations
3. Убедитесь, что события provisioning успешны
4. Проверьте в Cast Operations, что пользователи созданы
...
- **Ошибка: 401 Unauthorized**: Перегенерируйте Bearer Token в Cast Operations и обновите его в Okta
...
При отзыве прав пользователя (либо через DELETE-запрос, либо установкой `active: false`) он удаляется из команд, настроенных в параметрах SCIM. Сам аккаунт пользователя остаётся в Cast Operations, но теряет доступ к проекту.

### Можно ли использовать SCIM без SSO?

Да, SCIM и SSO — это независимые функции. Вы можете использовать SCIM для provisioning пользователей, позволяя им входить с паролем Cast Operations или любым другим методом аутентификации.

### Как обрабатываются пользователи, уже существующие в Cast Operations?

Когда SCIM пытается создать пользователя, который уже существует (совпадение по email), Cast Operations просто добавит его в настроенные команды по умолчанию вместо создания дублирующего пользователя.
