...
Cast Operations поддерживает отправку электронной почты через пользовательские SMTP-серверы с тремя методами аутентификации:
...
OAuth 2.0 обеспечивает более безопасный способ аутентификации на почтовых серверах, особенно для корпоративных сред, где отключена базовая аутентификация. Cast Operations поддерживает два типа предоставления OAuth:
...
При настройке SMTP с аутентификацией OAuth в Cast Operations вам потребуется:
...
4. Введите имя приложения (например, "Cast Operations SMTP")
...
### Шаг 5: Настройка в Cast Operations

В Cast Operations создайте или отредактируйте конфигурацию SMTP со следующими настройками:
...
### Шаг 7: Настройка в Cast Operations
...
- **Тестирование конфигурации**: Используйте кнопку "Send Test Email" в Cast Operations для проверки настройки
- **Проверка логов**: Просмотрите логи Cast Operations для получения подробных сообщений об ошибках
- **Кэширование токенов**: Cast Operations кэширует OAuth-токены и автоматически обновляет их до истечения срока действия
...
2. **Используйте выделенные сервисные аккаунты**: Создайте отдельные учётные данные для Cast Operations вместо совместного использования с другими приложениями
