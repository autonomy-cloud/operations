# Архитектура Cast Operations при самостоятельном хостинге

Данная диаграмма показывает типичный вид Cast Operations при самостоятельном хостинге в вашей среде (например, в кластере Kubernetes), включая то, как зонды отслеживают как внутренние, так и внешние ресурсы.
...
flowchart TB
  ...
  subgraph C["Сеть клиента (самостоятельный хостинг Kubernetes)"]
    ...
    subgraph Probes["Зонды Cast Operations"]
      direction TB
...
- Конечные пользователи получают доступ к Cast Operations через Ingress вашего кластера (NGINX), который маршрутизирует трафик на UI и API.
