...
Политики входящих звонков позволяют внешним абонентам связываться с дежурными инженерами, набирая выделенный номер телефона. При поступлении звонка Cast Operations маршрутизирует его через настроенные правила эскалации до тех пор, пока инженер не ответит.
...
flowchart TD
    ...
    B --> C[Twilio отправляет вебхук<br/>в Cast Operations]
    C --> D[Cast Operations воспроизводит<br/>приветственное сообщение]
...
sequenceDiagram
    ...
    Cast Operations->>Twilio: TwiML: Воспроизвести приветствие
    Twilio->>Абонент: «Пожалуйста, подождите, мы соединяем вас...»

    loop Правила эскалации
        Cast Operations->>Cast Operations: Получить следующее правило эскалации
        Cast Operations->>Twilio: TwiML: Набрать дежурного
        Twilio->>ДежурныйИнженер: Звонок на телефон
        alt Инженер ответил
            ДежурныйИнженер->>Twilio: Принимает звонок
            Twilio->>Cast Operations: Статус набора: completed
            Twilio->>Абонент: Соединить с инженером
            Note over Абонент,ДежурныйИнженер: Разговор в процессе
        else Нет ответа (тайм-аут)
            Twilio->>Cast Operations: Статус набора: no-answer
            Cast Operations->>Cast Operations: Попробовать следующее правило
        end
    end

    alt Все правила исчерпаны
        Cast Operations->>Twilio: TwiML: Воспроизвести сообщение «Нет ответа»
...
- Доступ к вашему самостоятельно размещённому экземпляру Cast Operations
...
Поскольку вы самостоятельно размещаете Cast Operations, необходимо настроить собственную учётную запись Twilio. Это даёт вам полный контроль над телефонными номерами и выставлением счетов.
...
## Шаг 2: Настройка конфигурации Call/SMS в Cast Operations
...
2. Cast Operations получит все номера телефонов из вашей учётной записи Twilio
3. Выберите нужный номер
4. Нажмите **Использовать этот номер** для привязки к политике

> **Примечание**: Если у номера телефона уже настроен вебхук, он будет обновлён для указания на Cast Operations.

### Вариант B: Покупка нового номера телефона

Для покупки нового номера непосредственно из Cast Operations:
...
- Убедитесь, что ваш экземпляр Cast Operations доступен из интернета
...
- Используйте HTTPS для вашего экземпляра Cast Operations
- Cast Operations проверяет подписи вебхуков для подтверждения того, что запросы приходят от Twilio
