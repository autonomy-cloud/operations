...
Поскольку разрешение зависит от того, как вы моделируете состояния инцидентов в проекте, сначала убедитесь, что путь _создания_ работает надёжно, и только потом добавляйте путь разрешения. См. [Компоненты → Компоненты данных Cast Operations](/docs/workflows/components#cast-operations-data-components).
