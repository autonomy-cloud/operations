...
В контексте скрипта доступен предварительно объявленный объект `screenshots`. Скриншоты можно присваивать ему в любой точке скрипта — они сохраняются **даже при исключении скрипта** (включая ошибки утверждений, тайм-ауты или неожиданные ошибки), что позволяет видеть состояние страницы в момент сбоя. Снятые скриншоты отображаются на панели управления Cast Operations для конкретного выполнения монитора.
...
Чтобы добавить секрет, перейдите в Cast Operations Dashboard -> Настройки проекта -> Секреты монитора -> Создать секрет монитора.
...
- При использовании latticeruntime.com всегда доступна последняя версия Playwright и браузеров. При самостоятельном хостинге убедитесь, что зонды обновлены до последней версии Playwright и браузеров.
