...
Deployment с одной репликой (образ `cast-operations/kubernetes-log-tailer`) использует Kubernetes API для стриминга логов контейнеров — тот же endpoint, который использует `kubectl logs -f`. Никаких hostPath, никакого доступа к хосту, никакого DaemonSet.
...
- Привилегированный Pod с hostPID — те же ограничения, что и у DaemonSet eBPF auto-instrumentation. Не может работать на GKE Autopilot, EKS Fargate или в других закрытых окружениях.

Тонкая настройка:

| Опция                         | По умолчанию         | Описание                                                                                                                                                                     |
| ----------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `profiling.enabled`           | `false`              | Главный переключатель. Выключен по умолчанию; подключите для непрерывных flame graph CPU.                                                                                    |
...
| `profiling.tracers`           | `""` _(все runtime)_ | Список языковых трассировщиков через запятую для загрузки.                                                                                                                   |
...
| `cast-operations.url`                           | _(обязательно)_                         | URL вашего инстанса Cast Operations.                                                                                                                                                                  |
| `cast-operations.apiKey`                        | _(обязательно)_                         | API-ключ проекта (Settings → API Keys).                                                                                                                                                         |
