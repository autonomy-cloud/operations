...
Мониторинг Kubernetes позволяет отслеживать работоспособность и производительность ваших кластеров Kubernetes, включая узлы, Pod, рабочие нагрузки и компоненты плоскости управления. Cast Operations собирает метрики из вашего кластера и оценивает их по заданным критериям.
...
1. Перейдите в раздел **Monitors** на дашборде Cast Operations
...
Выберите кластер Kubernetes для мониторинга. Кластеры должны быть интегрированы с Cast Operations через OpenTelemetry.
...
Cast Operations предоставляет шаблоны для распространённых сценариев мониторинга Kubernetes:
...
Чтобы использовать мониторинг Kubernetes, вам необходимо установить агент Cast Operations Kubernetes в вашем кластере. Агент отправляет метрики кластера, события, логи Pod и — по умолчанию — **трассировки приложений и HTTP RED-метрики, перехваченные через eBPF** в Cast Operations по OTLP. Никаких изменений в коде или SDK для каждого приложения для просмотра трафика уровня сервисов не требуется.
