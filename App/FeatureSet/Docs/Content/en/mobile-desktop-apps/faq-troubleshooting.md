...
Frequently asked questions and solutions for the Cast Operations mobile and desktop apps.

## How does Cast Operations distribute its apps?

- **Mobile (iOS and Android):** Cast Operations ships a native app called **Cast Operations On-Call**. It is published to the [Apple App Store](https://github.com/autonomy-cloud/operations/releases) and [Google Play](https://github.com/autonomy-cloud/operations/releases). A signed [APK download](https://github.com/autonomy-cloud/operations/releases/latest/download/cast-operations-on-call-android-app.apk) is also available for Android devices without Google Play.
- **Desktop (Windows, macOS, Linux):** The Cast Operations web dashboard is a Progressive Web App (PWA). You can install it as a desktop application directly from a Chromium-based browser or Safari — no store account required.
...
Yes. The Cast Operations On-Call app is free to install. You sign in with your existing Cast Operations account.

### Can I use the app with a self-hosted Cast Operations instance?

Yes. On first launch, the app asks for a **Server URL**. Enter the URL of your self-hosted instance (for example, `https://operations.example.com`). The app validates that the server is reachable before letting you sign in.
...
1. Notifications are enabled at the OS level for **Cast Operations On-Call**.
...
5. **Self-hosted only:** Push notifications are configured on your Cast Operations instance. See the [Push Notifications](/docs/self-hosted/push-notifications) guide.
...
Yes. Sign in with the same Cast Operations account on as many devices as you need. Each device receives its own push notifications.

### How do I uninstall?

- **iOS:** Long-press the icon → **Remove App** → **Delete App**.
- **Android:** Long-press the icon → **Uninstall**, or **Settings → Apps → Cast Operations On-Call → Uninstall**.

Your Cast Operations account and data are stored on the server and are not removed when you uninstall the app.
...
- **Instant updates** — the app stays in sync with your Cast Operations instance the moment you deploy.
...
2. Confirm your Cast Operations instance is served over HTTPS with a valid certificate.
...
- **Windows:** **Settings → Apps → Cast Operations → Uninstall**, or right-click the Start Menu entry.
...
- Disable battery optimisation: **Settings → Apps → Cast Operations On-Call → Battery → Unrestricted**.
- Disable Data Saver for the app.
- On Samsung devices, turn off **Device care → Battery → Background usage limits** for Cast Operations On-Call.
...
- Add Cast Operations On-Call to any active Focus mode's allow list.
...
- Ensure the Cast Operations instance is served over HTTPS.
...
- Open an issue on the [Cast Operations GitHub repository](https://github.com/autonomy-cloud/operations).
- Contact support through your Cast Operations dashboard.
