...
Cast Operations connects to the tools your team already uses — Zabbix, Jira, PagerDuty, Slack, and many more — through **[Workflows](/docs/workflows/index)**, the built-in automation engine. There's no separate plugin to install. You wire an integration together on a drag-and-drop canvas, and it runs whenever something happens.

This page explains the two patterns every integration uses. Once you understand them, you can connect Cast Operations to almost anything, even tools that don't have their own page here.

## The two patterns

Every integration moves data in one of two directions (and many use both).

### Inbound — another tool sends data into Cast Operations

Use this when an external system needs to _create or update something in Cast Operations_ — usually open an incident or an alert when it detects a problem.

1. Build a workflow that starts with a **[Webhook trigger](/docs/workflows/triggers#webhook)**. Cast Operations gives you a unique URL.
...
### Outbound — Cast Operations sends data to another tool

Use this when _something in Cast Operations should show up in another tool_ — open a Jira ticket, page someone in PagerDuty, post to Slack.

1. Build a workflow that starts with a **[Cast Operations event trigger](/docs/workflows/triggers#cast-operations-event-triggers)** — for example **Incident → On Create**.
...
| --------------------------------------------------------------------- | -------------------- | ----------------------------------------------------------------------------- |
| [Zabbix](/docs/integrations/zabbix)                                   | Inbound              | Turn Zabbix problems into Cast Operations incidents (and resolve them on recovery). |
| [Jira](/docs/integrations/jira)                                       | Outbound (+ inbound) | Open a Jira issue for every incident; sync status back.                       |
| [PagerDuty](/docs/integrations/pagerduty)                             | Outbound (+ inbound) | Trigger and resolve PagerDuty events from Cast Operations incidents.                |
| [Opsgenie](/docs/integrations/opsgenie)                               | Outbound (+ inbound) | Create and close Opsgenie alerts.                                             |
| [ServiceNow](/docs/integrations/servicenow)                           | Outbound (+ inbound) | Open ServiceNow incidents from Cast Operations.                                     |
...
- If the tool can **send a webhook** when something happens, use the **inbound** pattern — point its webhook at a Cast Operations Webhook trigger.
...
- [Triggers](/docs/workflows/triggers) — Webhook and Cast Operations event triggers in detail.
