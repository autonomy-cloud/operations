...
Create an [Opsgenie](https://www.atlassian.com/software/opsgenie) alert whenever a Cast Operations incident is created, and close it when Cast Operations resolves.

This integration is **outbound**: Cast Operations calls the [Opsgenie Alert API](https://docs.opsgenie.com/docs/alert-api). It uses a Cast Operations **[Workflow](/docs/workflows/index)** with an **Incident → On Create** trigger and an **API component**.
...
3. Add an **API** block connected to the trigger:
   ...
   The **`alias`** ties this Opsgenie alert to the Cast Operations incident so you can close it later by alias. Note the Opsgenie auth scheme is the literal word `GenieKey` followed by a space and your key.

4. **Save**, enable, and create a test incident. A `202 Accepted` response in the workflow logs means Opsgenie queued the alert.

## Step 3 — Close on Cast Operations resolve (recommended)
...
Opsgenie priorities run `P1`–`P5`. Map from Cast Operations severities with **Conditions** branches on `{{Incident.incidentSeverity.name}}` before the API block.
