...
Each call is recorded in **Project Settings → AI → AI Logs**, including provider, model, status, tokens, and estimated cost. Prompt and response previews and raw provider error details are not stored in the AI log. Workflow AI also counts toward the project's daily autonomous AI token budget; when the budget is exhausted, the component takes its **Error** path without contacting the model. Project AI must be enabled.
