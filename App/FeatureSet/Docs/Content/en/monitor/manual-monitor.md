...
Manual monitoring allows you to create monitors whose status is managed entirely by hand or through the API. Cast Operations does not perform any automated checks — you control the monitor status directly.

## Overview

Manual monitors are placeholders that you update yourself. This is useful for:

- Integrating with external monitoring tools that update status via the Cast Operations API
...
- **Dashboard** — Change the monitor status directly from the Cast Operations Dashboard
- **API** — Update the monitor status programmatically using the Cast Operations API
...
| API-driven status        | Let external tools update monitor status via the Cast Operations API                  |
| Status page placeholders | Show components on your status page that are managed outside Cast Operations          |
