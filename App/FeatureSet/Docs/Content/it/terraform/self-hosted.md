# Guida alla Configurazione Terraform Cast Operations Self-Hosted

Questa guida è specifica per i clienti che eseguono istanze self-hosted di Cast Operations. Copre la gestione delle versioni, la configurazione e le buone pratiche per l'uso del provider Terraform con la propria distribuzione Cast Operations.

## Note Importanti

⚠️ **I progetti non possono essere creati tramite Terraform** - I progetti devono essere creati manualmente nel dashboard Cast Operations. Usare l'ID del progetto nelle proprie configurazioni Terraform.

⚠️ **La regola più importante per i clienti self-hosted**: Bloccare sempre la versione del provider Terraform per corrispondere esattamente alla versione dell'installazione Cast Operations.

## Struttura delle Risorse

Tutte le risorse Terraform Cast Operations seguono una struttura semplificata:

- `name` (obbligatorio) - Nome della risorsa
- `description` (opzionale) - Descrizione della risorsa
- `data` (opzionale) - Configurazione complessa come JSON

## Critico: Compatibilità delle Versioni

⚠️ **La regola più importante per i clienti self-hosted**: Bloccare sempre la versione del provider Terraform per corrispondere esattamente alla versione dell'installazione Cast Operations.

### Perché il Blocco della Versione è Critico

- Il provider Terraform viene generato automaticamente dall'API di Cast Operations
- Ogni versione di Cast Operations può avere endpoint API e schemi diversi
- L'uso di una versione del provider non corrispondente può causare errori o comportamenti imprevisti
- Il blocco della versione garantisce compatibilità e comportamento prevedibile

## Trovare la Propria Versione Cast Operations

### Metodo 1: Dashboard

1. Effettuare il login nel proprio dashboard Cast Operations
2. Accedere a **Impostazioni** → **Informazioni**
3. Cercare il numero di versione (ad es. "7.0.123")

### Metodo 2: Endpoint API

```bash
curl https://vostra-istanza-latticeruntime.com/api/status
```

### Metodo 3: Immagini Docker

Se si esegue Cast Operations con Docker:

```bash
docker images | grep cast-operations
# Cercare il tag, ad es. cast-operations/dashboard:7.0.123
```

### Metodo 4: Chart Helm

Se si usa Helm:

```bash
helm list -n cast-operations
# Controllare la versione del chart
```

### Metodo 5: Variabili d'Ambiente

Controllare i propri file di configurazione per le variabili di versione:

```bash
grep -r "APP_VERSION\|IMAGE_TAG" /percorso/alla/propria/configurazione/cast-operations
```

## Template di Configurazione Provider

### Template per Versione 7.0.x

```hcl
terraform {
  required_providers {
    cast-operations = {
      source  = "autonomy-cloud/operations"
      version = "= 7.0.123"  # Sostituire 123 con il proprio numero di build esatto
    }
  }
  required_version = ">= 1.0"
}

provider "cast-operations" {
  cast_operations_url = "https://operations.vostracompany.com"  # Il proprio URL self-hosted
  api_key       = var.cast_operations_api_key
}
```

### Template per Versione 7.1.x

```hcl
terraform {
  required_providers {
    cast-operations = {
      source  = "autonomy-cloud/operations"
      version = "= 7.1.45"  # Sostituire con la propria versione esatta
    }
  }
  required_version = ">= 1.0"
}

provider "cast-operations" {
  cast_operations_url = "https://operations.vostracompany.com"
  api_key       = var.cast_operations_api_key
}
```

## Esempio Completo di Configurazione Self-Hosted

Ecco un esempio completo per un'istanza Cast Operations self-hosted:

```hcl
# versions.tf
terraform {
  required_providers {
    cast-operations = {
      source  = "autonomy-cloud/operations"
      version = "= 7.0.123"  # Deve corrispondere alla versione Cast Operations
    }
  }
  required_version = ">= 1.0"

  # Opzionale: Usare lo stato remoto per la collaborazione del team
  backend "s3" {
    bucket = "vostro-bucket-stato-terraform"
    key    = "cast-operations/terraform.tfstate"
    region = "us-west-2"
  }
}

# variables.tf
variable "cast_operations_url" {
  description = "URL istanza Cast Operations"
  type        = string
  default     = "https://operations.vostracompany.com"
}

variable "cast_operations_api_key" {
  description = "Chiave API Cast Operations"
  type        = string
  sensitive   = true
}

variable "environment" {
  description = "Nome dell'ambiente"
  type        = string
  default     = "production"
}

# providers.tf
provider "cast-operations" {
  cast_operations_url = var.cast_operations_url
  api_key       = var.cast_operations_api_key
}

# variables.tf
variable "project_id" {
  description = "ID progetto Cast Operations (creare manualmente nel dashboard)"
  type        = string
}

# main.tf
# Creare team
resource "cast_operations_team" "infrastructure" {
  name        = "Team Infrastruttura"
  description = "Team di infrastruttura e operazioni"
}

resource "cast_operations_team" "development" {
  name        = "Team Sviluppo"
  description = "Team di sviluppo applicazioni"
  project_id = cast_operations_project.main.id
}

# Monitor infrastruttura
resource "cast_operations_monitor" "database" {
  name       = "${var.environment}-database"
  project_id = cast_operations_project.main.id

  monitor_type = "port"
  hostname     = "db.internal.vostracompany.com"
  port         = 5432
  interval     = "2m"
  timeout      = "10s"

  tags = {
    team        = "infrastructure"
    service     = "database"
    environment = var.environment
    criticality = "critical"
  }
}

resource "cast_operations_monitor" "application" {
  name       = "${var.environment}-application"
  project_id = cast_operations_project.main.id

  monitor_type = "website"
  url          = "https://app.vostracompany.com/health"
  interval     = "1m"
  timeout      = "30s"

  expected_status_codes = [200]

  tags = {
    team        = "development"
    service     = "application"
    environment = var.environment
    criticality = "high"
  }
}

# Policy on-call
resource "cast_operations_on_call_policy" "infrastructure_oncall" {
  name       = "On-Call Infrastruttura"
  project_id = cast_operations_project.main.id
  team_id    = cast_operations_team.infrastructure.id

  schedules {
    name     = "Infrastruttura 24x7"
    timezone = "Europe/Rome"

    layers {
      name          = "Primario"
      users         = ["infra1@vostracompany.com", "infra2@vostracompany.com"]
      rotation_type = "weekly"
      start_time    = "00:00"
      end_time      = "23:59"
      days          = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]
    }
  }
}

# Policy avvisi
resource "cast_operations_alert_policy" "critical_infrastructure" {
  name       = "Avvisi Infrastruttura Critici"
  project_id = cast_operations_project.main.id

  conditions {
    monitor_id = cast_operations_monitor.database.id
    threshold  = "down"
  }

  actions {
    type = "email"
    recipients = ["infrastruttura@vostracompany.com"]
  }

  actions {
    type             = "oncall_escalation"
    oncall_policy_id = cast_operations_on_call_policy.infrastructure_oncall.id
  }
}

# Pagina di stato interna
resource "cast_operations_status_page" "internal" {
  name       = "Stato Servizi Interni"
  project_id = cast_operations_project.main.id

  domain = "status.internal.vostracompany.com"

  components {
    name       = "Database"
    monitor_id = cast_operations_monitor.database.id
  }

  components {
    name       = "Applicazione"
    monitor_id = cast_operations_monitor.application.id
  }
}

# outputs.tf
output "project_id" {
  description = "ID Progetto"
  value       = cast_operations_project.main.id
}

output "status_page_url" {
  description = "URL pagina di stato"
  value       = "https://${cast_operations_status_page.internal.domain}"
}
```

## Configurazione Specifica per Ambiente

### Ambiente di Sviluppo

```hcl
# dev.tfvars
cast_operations_url = "https://operations-dev.vostracompany.com"
environment = "development"
```

### Ambiente di Staging

```hcl
# staging.tfvars
cast_operations_url = "https://operations-staging.vostracompany.com"
environment = "staging"
```

### Ambiente di Produzione

```hcl
# prod.tfvars
cast_operations_url = "https://operations.vostracompany.com"
environment = "production"
```

## Processo di Aggiornamento per Self-Hosted

Quando si aggiorna l'istanza Cast Operations:

### 1. Checklist Pre-Aggiornamento

```bash
# Backup dello stato Terraform corrente
terraform state pull > backup-$(date +%Y%m%d).tfstate

# Annotare la versione Cast Operations corrente
curl https://operations.vostracompany.com/api/status | jq '.version'

# Annotare la versione del provider corrente
terraform providers | grep cast-operations
```

### 2. Aggiornare l'Istanza Cast Operations

Seguire il proprio processo standard di aggiornamento Cast Operations (Docker, Helm, ecc.)

### 3. Aggiornare il Provider Terraform

```hcl
# Aggiornare la versione nel blocco terraform
terraform {
  required_providers {
    cast-operations = {
      source  = "autonomy-cloud/operations"
      version = "= 7.0.124"  # Nuova versione dopo l'aggiornamento
    }
  }
}
```

### 4. Testare e Applicare

```bash
# Aggiornare il provider
terraform init -upgrade

# Pianificare per vedere le modifiche
terraform plan

# Applicare se tutto sembra corretto
terraform apply
```

## Configurazione di Rete

### Regole Firewall

Assicurarsi che il proprio runner Terraform possa accedere a:

- Endpoint API Cast Operations (solitamente porta 443/HTTPS)
- Qualsiasi risorsa interna monitorata

### VPN/Reti Private

Se Cast Operations si trova in una rete privata:

```hcl
provider "cast-operations" {
  cast_operations_url = "https://10.0.1.100:443"  # IP interno
  api_key       = var.cast_operations_api_key
}
```

## Buone Pratiche di Sicurezza

### 1. Gestione Chiavi API

```bash
# Usare variabili d'ambiente
export CAST_OPERATIONS_API_KEY="vostra-api-key"

# O usare un sistema di gestione dei segreti
export CAST_OPERATIONS_API_KEY=$(vault kv get -field=api_key secret/cast-operations)
```

### 2. Chiavi API con Privilegi Minimi

Creare chiavi API con i permessi minimi richiesti:

- Gestione monitor
- Gestione policy avvisi
- Gestione team (se necessario)

### 3. Sicurezza di Rete

```hcl
# Esempio con verifica TLS
provider "cast-operations" {
  cast_operations_url = "https://operations.vostracompany.com"
  api_key       = var.cast_operations_api_key

  # Opzioni di sicurezza aggiuntive se supportate
  verify_ssl = true
  timeout    = "30s"
}
```

## Monitorare la Propria Automazione Terraform

Creare monitor per la propria automazione Terraform:

```hcl
resource "cast_operations_monitor" "terraform_runner" {
  name       = "Salute Runner Terraform"
  project_id = cast_operations_project.main.id

  monitor_type = "heartbeat"
  interval     = "15m"

  tags = {
    automation = "terraform"
    criticality = "medium"
  }
}
```

## Risoluzione dei Problemi Self-Hosted

### Problema: Connessione Rifiutata

```
Error: connection refused
```

**Soluzioni**:

1. Controllare che l'istanza Cast Operations sia in esecuzione
2. Verificare che l'URL API sia corretto
3. Controllare la connettività firewall/rete
4. Verificare che i certificati TLS siano validi

### Problema: Mancata Corrispondenza Versione API

```
Error: API version incompatible
```

**Soluzioni**:

1. Controllare la versione Cast Operations: `curl https://vostra-istanza/api/status`
2. Aggiornare la versione del provider per corrispondere
3. Eseguire `terraform init -upgrade`

### Problema: Certificati Autofirmati

Se si usano certificati autofirmati:

```bash
# Saltare temporaneamente la verifica TLS (non consigliato per la produzione)
export CAST_OPERATIONS_SKIP_TLS_VERIFY=true
```

Soluzione migliore: Aggiungere il proprio certificato CA all'archivio di attendibilità del sistema.

## Backup e Disaster Recovery

### Backup dello Stato

```bash
# Backup periodici dello stato
terraform state pull > backup-$(date +%Y%m%d-%H%M%S).tfstate

# Script di backup automatizzato
#!/bin/bash
DATE=$(date +%Y%m%d-%H%M%S)
terraform state pull > "backups/terraform-state-${DATE}.tfstate"
find backups/ -name "terraform-state-*.tfstate" -mtime +30 -delete
```

### Backup della Configurazione

```bash
# Backup della configurazione Terraform
tar -czf terraform-config-$(date +%Y%m%d).tar.gz *.tf *.tfvars
```

## Gestione Multi-Ambiente

### Usando i Workspace

```bash
# Creare ambienti
terraform workspace new dev
terraform workspace new staging
terraform workspace new prod

# Passare tra gli ambienti
terraform workspace select prod
terraform apply -var-file="prod.tfvars"
```

### Usando Directory Separate

```
terraform/
├── environments/
│   ├── dev/
│   │   ├── main.tf
│   │   └── terraform.tfvars
│   ├── staging/
│   │   ├── main.tf
│   │   └── terraform.tfvars
│   └── prod/
│       ├── main.tf
│       └── terraform.tfvars
└── modules/
    └── cast-operations/
        ├── main.tf
        ├── variables.tf
        └── outputs.tf
```

Questo approccio fornisce un migliore isolamento e una gestione più facile della versione per ambiente.
