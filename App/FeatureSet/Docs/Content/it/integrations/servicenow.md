...
Apri automaticamente un incidente [ServiceNow](https://www.servicenow.com) ogni volta che viene creato un incidente Cast Operations — in modo che ITSM e monitoraggio rimangano allineati.

Questa integrazione è **in uscita**: Cast Operations chiama la [Table API](https://docs.servicenow.com/bundle/utah-application-development/page/integrate/inbound-rest/concept/c_TableAPI.html) di ServiceNow. Utilizza un **[Workflow](/docs/workflows/index)** di Cast Operations con un trigger **Incident → On Create** e un **componente API**.
...
2. In Cast Operations, vai su **Workflows → Global Variables → Create**, chiamala `SERVICENOW_AUTH`, incolla la stringa base64 e attiva **Is Secret**.
...
3. Aggiungi un blocco **API** collegato al trigger:
   ...
   Il `correlation_id` mantiene un collegamento all'incidente Cast Operations — utile se in seguito vuoi aggiungere un passaggio di risoluzione. ServiceNow usa `1` (alta), `2` (media), `3` (bassa) per `urgency`/`impact`.
...
## Passaggio 3 — Risolvi alla risoluzione in Cast Operations (opzionale)
...
2. Per aggiornare il record ServiceNow corretto hai bisogno del suo `sys_id`. Puoi salvarlo sull'incidente Cast Operations nel Passaggio 2 (leggi `{{CreateRecord.response-body.result.sys_id}}` e scrivilo in un'etichetta con **Update Incident**), oppure cerca il record con una `GET` su `/api/now/table/incident?sysparm_query=correlation_id=cast-operations-{{Incident._id}}`.
3. Aggiungi un blocco **API**: **Method** `PATCH`, **URL** `https://your-instance.service-now.com/api/now/table/incident/<sys_id>`, body `{ "state": "6", "close_code": "Resolved by monitoring", "close_notes": "Resolved in Cast Operations" }` (`state` `6` = Risolto nel workflow ITIL predefinito).
