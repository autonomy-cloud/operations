...
- Un **Cast Operations Telemetry Ingestion Token** — creane uno da _Project Settings → Telemetry Ingestion Keys_ e copia il valore `x-oneuptime-token`
...
2. Conferma che l'endpoint dell'exporter sia `https://visca.ai/otlp` (o il tuo `…/otlp` self-hosted) e che l'header `x-oneuptime-token` contenga un token valido.
