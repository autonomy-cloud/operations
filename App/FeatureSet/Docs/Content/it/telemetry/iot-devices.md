...
- **MQTT** — connettiti direttamente all'endpoint MQTT integrato di Cast Operations (MQTT su WebSocket all'indirizzo `wss://<your-host>/mqtt`, oppure MQTT TCP grezzo nei deployment self-hosted) e pubblica letture JSON. Nessun collector richiesto, e il supporto Last Will offre un rilevamento offline immediato.
...
- Un dispositivo, gateway o collector in grado di inviare OTLP/HTTP a Cast Operations
- Raggiungibilità di rete dal dispositivo/gateway verso la tua istanza di Cast Operations
- Un **Cast Operations Telemetry Ingestion Token** — creane uno da _Project Settings → Telemetry Ingestion Keys_ e copia il valore `x-cast-operations-token`

## Come Cast Operations Modella l'IoT

Cast Operations mappa i tuoi dispositivi su due concetti utilizzando gli attributi di risorsa di OpenTelemetry:

- **Flotta** — un gruppo logico di dispositivi (per esempio `building-a-sensors` o `field-gateways`). La flotta è derivata dall'attributo di risorsa `iot.fleet.name` e appare in Cast Operations come il servizio di telemetria `iot/<fleet>`. Imposta `service.name=iot/<fleet>` in modo che log e metriche si allineino sotto lo stesso servizio.
- **Dispositivo** — un singolo dispositivo all'interno di una flotta, identificato dall'attributo `device.id`. Cast Operations costruisce e mantiene un inventario di dispositivi per flotta indicizzato su `device.id`.

Gli attributi opzionali affinano il modo in cui ciascun dispositivo è classificato e delimitato nei monitor:

| Attributo            | Obbligatorio | Descrizione                                                                      |
| -------------------- | -------- | -------------------------------------------------------------------------------- |
| `iot.fleet.name`     | Sì       | La flotta a cui appartiene questo dispositivo. Diventa il servizio Cast Operations `iot/<fleet>`    |
...
Se il tuo dispositivo esegue direttamente un SDK OpenTelemetry, puntalo a Cast Operations e applica gli attributi di risorsa IoT tramite le variabili d'ambiente standard `OTEL_*`. Sostituisci il token, l'endpoint, il nome della flotta e il device id con i valori del tuo ambiente.
...
| ----------------------------- | -------- | ---------------------------------------------------------------------------------------------------- |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | Sì       | Endpoint OTLP di Cast Operations (`https://latticeruntime.com/otlp`, oppure `http(s)://YOUR-OPERATIONS-HOST/otlp` in self-hosted) |
| `OTEL_EXPORTER_OTLP_HEADERS`  | Sì       | `x-cast-operations-token=YOUR_TELEMETRY_INGESTION_TOKEN`                                                    |
...
Emetti le tue letture come metriche utilizzando i nomi `iot_*` riportati di seguito (vedi [Convenzioni sulle Metriche](#convenzioni-sulle-metriche)). Entro circa un minuto il dispositivo appare nella sezione **IoT** della dashboard di Cast Operations.

## Invio delle Metriche tramite un OpenTelemetry Collector

Quando molti dispositivi riportano attraverso un gateway, esegui un OpenTelemetry Collector sul gateway ed esporta verso Cast Operations. Il processore `resource` applica gli attributi della flotta; ricevi le letture dai tuoi dispositivi (OTLP, bridge MQTT, file di log, ecc.) e inoltrale:
...
- Mantieni `device.id` (e facoltativamente `iot.device.kind` / `iot.device.type` / `iot.device.firmware`) su ciascun datapoint affinché Cast Operations possa risolvere il singolo dispositivo all'interno della flotta.
- **`otlphttp`** invia a Cast Operations tramite HTTPS con il token di acquisizione allegato. Sono accettate sia la codifica protobuf predefinita sia `encoding: json`.

## Invio delle Metriche tramite MQTT

Cast Operations include un endpoint MQTT integrato, così i dispositivi che parlano già MQTT possono inviare le letture direttamente — senza alcun SDK OpenTelemetry, collector o bridge. Tutto ciò che viene pubblicato tramite MQTT finisce nella stessa pipeline di OTLP: le flotte vengono create automaticamente, l'inventario dei dispositivi si aggiorna e ogni monitor e template di avviso IoT funziona senza modifiche.

**Endpoint**

| Trasporto             | Indirizzo                              | Note                                                                                     |
| --------------------- | -------------------------------------- | ----------------------------------------------------------------------------------------- |
| MQTT su WebSocket     | `wss://<your-host>/mqtt`               | Funziona su qualsiasi deployment — viaggia sulla normale porta HTTPS attraverso l'ingress di Cast Operations |
...
**Topic** — pubblica sotto il prefisso fisso `cast-operations/`. I segmenti di flotta e dispositivo non devono contenere `/`, `+` o `#` e sono limitati a 100 caratteri:

| Topic                                            | Payload                                                                                              |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
| `cast-operations/<fleet>/<device>/telemetry`           | Oggetto JSON di letture — `{ "metrics": { "iot_temperature_celsius": 21.5 } }`, oppure un oggetto piatto i cui campi numerici sono le metriche |
| `cast-operations/<fleet>/<device>/metrics/<metricName>`| Un singolo valore — un numero semplice (`23.4`) oppure `{ "value": 23.4 }`                            |
| `cast-operations/<fleet>/<device>/status`              | `"online"` oppure `"offline"` (anche `1`/`0`, `true`/`false`, `up`/`down`) — mappato su `iot_device_up` |
...
Esempio con `mqtt` per Node.js su WebSocket (funziona con latticeruntime.com e con qualsiasi istanza self-hosted):

```javascript
const mqtt = require("mqtt");

const client = mqtt.connect("wss://latticeruntime.com/mqtt", {
  username: "cast-operations", // ignorato — è il token qui sotto che autentica
...
- Le pubblicazioni al di fuori del contratto dei topic o con payload malformati vengono accettate e **scartate** (MQTT 3.1.1 non prevede una risposta di errore per messaggio) — il server registra un warning con il motivo, quindi controlla i log dell'app Cast Operations se i dati non arrivano.
- Sull'endpoint WebSocket, mantieni il keepalive MQTT **sotto i 5 minuti** — l'ingress di Cast Operations chiude le connessioni WebSocket inattive dopo 300 secondi, il che farebbe scattare il tuo Last Will e un falso avviso Device Offline. I valori predefiniti delle librerie client (60 s per `mqtt` e `paho-mqtt`) vanno bene. L'endpoint TCP grezzo non ha questo limite.
...
| `iot_cpu_usage_ratio`       | Utilizzo della CPU come rapporto da `0` a `1` (Cast Operations lo memorizza come percentuale)        |
...
2. Nella dashboard di Cast Operations, apri la sezione **IoT** — la tua flotta dovrebbe apparire come `iot/<fleet>` entro circa un minuto.
...
2. Conferma che l'endpoint dell'exporter sia `https://latticeruntime.com/otlp` (o il tuo `…/otlp` self-hosted) e che l'header `x-cast-operations-token` contenga un token valido.
3. Se usi MQTT, conferma che il topic segua esattamente `cast-operations/<fleet>/<device>/…` — è il segmento di flotta del topic a creare la flotta.
...
Il token di acquisizione non è valido, è stato revocato o è mancante. Generane uno nuovo da _Project Settings → Telemetry Ingestion Keys_ e aggiorna l'header `x-cast-operations-token`.
...
2. Ricorda che `iot_cpu_usage_ratio` è un rapporto da `0` a `1`; invia il rapporto grezzo e Cast Operations lo rappresenta come percentuale.
...
Se stai eseguendo Cast Operations in self-hosting, punta l'endpoint alla tua istanza:
...
Per MQTT, connettiti a `wss://your-operations-host.example.com/mqtt`, oppure esponi la porta MQTT TCP grezza del servizio app (`MQTT_INGEST_PORT`, predefinita `1883`) se i tuoi dispositivi non sono in grado di parlare WebSocket. Imposta `MQTT_INGEST_ENABLED=false` sul servizio app per disattivare completamente i listener MQTT.
...
- Per approfondire l'integrazione OTLP sottostante, consulta [Integrare OpenTelemetry con Cast Operations](/docs/telemetry/open-telemetry).
