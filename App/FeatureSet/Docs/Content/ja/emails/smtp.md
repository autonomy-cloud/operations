...
Cast Operations は、3 つの認証方式によるカスタム SMTP サーバー経由のメール送信をサポートしています。
...
OAuth 2.0 は、特に基本認証を無効にしているエンタープライズ環境において、メールサーバーへのより安全な認証方法を提供します。Cast Operations は 2 種類の OAuth グラントタイプをサポートしています。
...
Cast Operations で OAuth 認証を使用して SMTP を設定する際に必要な情報は以下の通りです。
...
3. **新規登録** をクリックします
4. アプリケーションの名前を入力します（例: "Cast Operations SMTP"）
...
### ステップ 5: Cast Operations で設定する

Cast Operations で以下の設定を使用して SMTP 設定を作成または編集します。
...
- **設定のテスト**: Cast Operations の「テストメールを送信」ボタンを使用してセットアップを確認します
- **ログの確認**: 詳細なエラーメッセージについては Cast Operations のログを確認します
- **トークンキャッシュ**: Cast Operations は OAuth トークンをキャッシュし、有効期限前に自動的に更新します
...
2. **専用サービスアカウントを使用する**: 他のアプリケーションと共有するのではなく、Cast Operations 専用の認証情報を作成します
