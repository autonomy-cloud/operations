...
Cast Operations は、`faas.name` リソース属性でタグ付けされた OpenTelemetry データを受信した瞬間に、**サーバーレス関数**を自動的に認識します。手動で作成するものは何もありません。お使いのランタイム用の OpenTelemetry SDK で関数をインストルメント化し、その OTLP エクスポーターを Cast Operations に向けるだけで、その関数はトレース、ログ、メトリクスとともに**サーバーレス関数**の下に表示されます。
...
Cast Operations は各関数を `faas.name` リソース属性をキーとして識別します。
...
Cast Operations をセルフホストしている場合は、エンドポイントを `https://YOUR-OPERATIONS-HOST/otlp` に置き換えます。
