...
Los workflows te permiten automatizar tareas en Cast Operations sin escribir código. Arrastra y suelta algunos bloques en un lienzo, conéctalos entre sí y tendrás una automatización que se ejecuta cuando ocurre algo: se abre un incidente, se dispara una programación o alguna otra herramienta envía datos a Cast Operations.
...
- **Conectar Cast Operations con tus otras herramientas** — enviar incidentes a Slack, crear tickets en Jira, publicar en un webhook dentro de tu stack.
- **Reaccionar a lo que ocurre en Cast Operations** — cuando se crea un incidente crítico, notificar al equipo de guardia y abrir un ticket automáticamente.
- **Ejecutar tareas según una programación** — cada cinco minutos, cada noche, cada lunes por la mañana.
- **Recibir datos desde el exterior** — permitir que otros sistemas envíen datos a Cast Operations mediante una URL única.
...
1. **Un disparador** — qué inicia el workflow. Puede ser un botón manual, una programación, un webhook entrante o un evento de Cast Operations (como un nuevo incidente).
...
## Dónde encontrar los workflows en Cast Operations
...
2. **Elegir un disparador** — manual, programado, webhook o un evento de Cast Operations.
...
## Cómo encajan los workflows con el resto de Cast Operations
