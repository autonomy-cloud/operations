...
[Zabbix](https://www.zabbix.com) vigila tus servidores y red; Cast Operations gestiona la respuesta a incidentes, la guardia y las páginas de estado. Conecta ambos y cada problema de Zabbix se convierte automáticamente en un incidente de Cast Operations — para que se notifique a las personas correctas y tu página de estado permanezca actualizada.

Esta integración es **entrante**: Zabbix envía problemas a Cast Operations. Usa un **tipo de medio webhook** de Zabbix en un lado y un **[Workflow](/docs/workflows/index)** de Cast Operations en el otro. Sin plugins, sin servicios adicionales.
...
2. Una **acción** de Zabbix le indica al tipo de medio **Cast Operations** que envíe el evento.
3. El script del tipo de medio hace un POST de una pequeña carga útil JSON a una URL del workflow de Cast Operations.
...
- Tu servidor Zabbix debe poder llegar a tu instancia de Cast Operations por HTTPS.
- Un proyecto de Cast Operations donde puedas crear workflows.

## Parte 1 — Construir el workflow de Cast Operations
...
4. Arrastra un bloque **Create Incident** y conéctalo a la salida **Yes** del bloque Conditions. Rellena:
   ...
   - **Severity**: elige la gravedad de incidente de Cast Operations que prefieras (puedes refinarlo más adelante con más ramas de Conditions que mapeen las gravedades de Zabbix).
...
### Paso 1: Crear el tipo de medio de Cast Operations
...
1. Ve a **Users → Users → Create user**. Nómbralo `Cast Operations Webhook`, dale un rol que pueda recibir notificaciones (p. ej. **User role**) y añádelo a un grupo de usuarios.
...
### Paso 3: Enviar problemas a Cast Operations con una acción
...
4. En la pestaña **Operations**, añade una operación que envíe a **User: Cast Operations Webhook** mediante el tipo de medio **Cast Operations**.
...
1. De vuelta en el workflow de Cast Operations, activa **Enabled**.
...
4. Comprueba **Incidents** en Cast Operations — tu problema de Zabbix es ahora un incidente.
...
Las gravedades de Zabbix (`Not classified`, `Information`, `Warning`, `Average`, `High`, `Disaster`) llegan como `{{Zabbix.Request Body.severity}}`. Para mapearlas a las gravedades de incidente de Cast Operations, añade ramas **Conditions** antes de **Create Incident** — por ejemplo, redirige `Disaster` y `High` a un incidente "Critical" y todo lo demás a "Major". Construye un bloque **Create Incident** por rama.
...
- Una respuesta no 2xx de Cast Operations se muestra mediante el `throw` en el script — comprueba que la URL del workflow sea exactamente correcta.
...
- Si ejecutas Cast Operations en modo autohospedado detrás de un firewall, permite que la IP de salida de tu servidor Zabbix lo alcance por HTTPS.
