...
Cast Operations se conecta con las herramientas que tu equipo ya utiliza —Zabbix, Jira, PagerDuty, Slack y muchas más— a través de **[Workflows](/docs/workflows/index)**, el motor de automatización integrado. No hay ningún plugin adicional que instalar. Conectas una integración en un lienzo de arrastrar y soltar, y se ejecuta cada vez que ocurre algo.

Esta página explica los dos patrones que usa cada integración. Una vez que los entiendas, podrás conectar Cast Operations con casi cualquier cosa, incluso con herramientas que no tienen su propia página aquí.

## Los dos patrones

Cada integración mueve datos en una de dos direcciones (y muchas usan ambas).

### Entrante — otra herramienta envía datos a Cast Operations

Úsalo cuando un sistema externo necesita _crear o actualizar algo en Cast Operations_ — normalmente abrir un incidente o una alerta cuando detecta un problema.

1. Construye un workflow que comience con un **[disparador Webhook](/docs/workflows/triggers#webhook)**. Cast Operations te proporciona una URL única.
...
### Saliente — Cast Operations envía datos a otra herramienta

Úsalo cuando _algo en Cast Operations deba aparecer en otra herramienta_ — abrir un ticket en Jira, notificar a alguien en PagerDuty, publicar en Slack.

1. Construye un workflow que comience con un **[disparador de evento de Cast Operations](/docs/workflows/triggers#cast-operations-event-triggers)** — por ejemplo **Incident → On Create**.
...
| --------------------------------------------------------------------- | --------------------- | --------------------------------------------------------------------------------------------- |
| [Zabbix](/docs/integrations/zabbix)                                   | Entrante              | Convierte los problemas de Zabbix en incidentes de Cast Operations (y los resuelve al recuperarse). |
| [Jira](/docs/integrations/jira)                                       | Saliente (+ entrante) | Abre un issue de Jira por cada incidente; sincroniza el estado de vuelta.                     |
| [PagerDuty](/docs/integrations/pagerduty)                             | Saliente (+ entrante) | Activa y resuelve eventos de PagerDuty desde incidentes de Cast Operations.                         |
| [Opsgenie](/docs/integrations/opsgenie)                               | Saliente (+ entrante) | Crea y cierra alertas de Opsgenie.                                                            |
| [ServiceNow](/docs/integrations/servicenow)                           | Saliente (+ entrante) | Abre incidentes de ServiceNow desde Cast Operations.                                                |
...
- Si la herramienta puede **enviar un webhook** cuando ocurre algo, usa el patrón **entrante** — apunta su webhook a un disparador Webhook de Cast Operations.
...
- [Disparadores](/docs/workflows/triggers) — disparadores Webhook y de eventos de Cast Operations en detalle.
