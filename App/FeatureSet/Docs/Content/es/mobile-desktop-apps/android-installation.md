...
Instale la aplicación nativa de Android **Cast Operations On-Call** desde la Google Play Store, o instale el APK directamente en dispositivos sin Google Play.

## Requisitos

- Teléfono o tableta Android con **Android 8.0 (Oreo) o posterior**
- Una cuenta activa de Cast Operations (o la URL de su instancia autoalojada de Cast Operations)
...
2. Busque **"Cast Operations On-Call"**, o abra este enlace en su dispositivo:
   [https://github.com/autonomy-cloud/operations/releases](https://github.com/autonomy-cloud/operations/releases)
3. Toque **Instalar**.
4. Una vez instalada, toque **Abrir** o inicie **Cast Operations On-Call** desde el cajón de aplicaciones.
...
4. Inicie **Cast Operations On-Call** desde el cajón de aplicaciones.

El APK lo crea y firma Cast Operations a partir del mismo código fuente que la versión de Play Store. Las actualizaciones de la aplicación no son automáticas al instalar manualmente — descargue el APK más reciente desde el enlace anterior cuando se publique una nueva versión.
...
2. **Iniciar sesión**
   - Introduzca el correo electrónico y la contraseña de su cuenta de Cast Operations.
...
1. Abra **Ajustes → Aplicaciones → Cast Operations On-Call → Notificaciones** y confirme que todas las categorías estén habilitadas.
2. Abra **Ajustes → Aplicaciones → Cast Operations On-Call → Batería** y elija **Sin restricciones** (o desactive la optimización de batería) para que el sistema operativo no retrase las notificaciones push en segundo plano.
...
4. Si usa dispositivos Samsung, desactive también **Ajustes → Mantenimiento del dispositivo → Batería → Límites de uso en segundo plano** para Cast Operations On-Call.
5. Añada Cast Operations On-Call a las listas de excepciones de **Do Not Disturb** (No molestar) para que los avisos sigan sonando durante su turno de guardia.

## Actualizaciones

**Google Play:**

- Las actualizaciones se instalan automáticamente. Para activar una manualmente, abra **Play Store → Perfil → Administrar apps y dispositivo → Actualizaciones disponibles → Cast Operations On-Call → Actualizar**.
...
1. **Mantenga pulsado** el icono de **Cast Operations On-Call** y, a continuación, toque **Desinstalar**.
2. O abra **Ajustes → Aplicaciones → Cast Operations On-Call → Desinstalar**.
3. Confirme para eliminar la aplicación.

Su cuenta de Cast Operations y sus turnos de guardia se almacenan en el servidor y no se eliminan al desinstalar la aplicación.
...
- Si está en una red corporativa o VPN, asegúrese de que la instancia de Cast Operations sea accesible.
- Confirme que el servidor se sirva a través de HTTPS con un certificado válido.

**No se reciben notificaciones push:**

- Confirme que las notificaciones estén habilitadas en **Ajustes → Aplicaciones → Cast Operations On-Call → Notificaciones**.
- Deshabilite la optimización de batería para Cast Operations On-Call (consulte Notificaciones push arriba).
- Asegúrese de que Do Not Disturb esté desactivado, o de que Cast Operations On-Call esté en la lista de excepciones.
- Cierre sesión y vuelva a iniciarla para actualizar el token push registrado con el servidor.
- Usuarios autoalojados: confirme que las notificaciones push estén configuradas en su instancia de Cast Operations (consulte la guía de [Notificaciones push](/docs/self-hosted/push-notifications) para instalaciones autoalojadas).
...
Si aún necesita ayuda, póngase en contacto a través de su panel de Cast Operations o abra un issue en nuestro [repositorio de GitHub](https://github.com/autonomy-cloud/operations).
