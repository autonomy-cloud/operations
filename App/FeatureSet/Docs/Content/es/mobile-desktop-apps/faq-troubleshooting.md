...
Preguntas frecuentes y soluciones para las aplicaciones móviles y de escritorio de Cast Operations.

## ¿Cómo distribuye Cast Operations sus aplicaciones?

- **Móvil (iOS y Android):** Cast Operations ofrece una aplicación nativa llamada **Cast Operations On-Call**. Está publicada en la [Apple App Store](https://github.com/autonomy-cloud/operations/releases) y en [Google Play](https://github.com/autonomy-cloud/operations/releases). También está disponible una [descarga del APK firmado](https://github.com/autonomy-cloud/operations/releases/latest/download/cast-operations-on-call-android-app.apk) para dispositivos Android sin Google Play.
- **Escritorio (Windows, macOS, Linux):** El panel web de Cast Operations es una Progressive Web App (PWA). Puede instalarlo como una aplicación de escritorio directamente desde un navegador basado en Chromium o desde Safari — no se requiere cuenta de tienda.
...
Sí. La aplicación Cast Operations On-Call es gratuita de instalar. Inicie sesión con su cuenta existente de Cast Operations.

### ¿Puedo usar la aplicación con una instancia autoalojada de Cast Operations?

Sí. En el primer inicio, la aplicación le pide una **URL del servidor**. Introduzca la URL de su instancia autoalojada (por ejemplo, `https://operations.example.com`). La aplicación valida que el servidor sea accesible antes de permitirle iniciar sesión.
...
1. Las notificaciones están habilitadas a nivel del sistema operativo para **Cast Operations On-Call**.
...
5. **Solo autoalojado:** Las notificaciones push están configuradas en su instancia de Cast Operations. Consulte la guía de [Notificaciones push](/docs/self-hosted/push-notifications).
...
Sí. Inicie sesión con la misma cuenta de Cast Operations en tantos dispositivos como necesite. Cada dispositivo recibe sus propias notificaciones push.

### ¿Cómo desinstalo?

- **iOS:** Mantenga pulsado el icono → **Eliminar app** → **Eliminar app**.
- **Android:** Mantenga pulsado el icono → **Desinstalar**, o **Ajustes → Aplicaciones → Cast Operations On-Call → Desinstalar**.

Su cuenta de Cast Operations y sus datos se almacenan en el servidor y no se eliminan al desinstalar la aplicación.
...
- **Actualizaciones instantáneas** — la aplicación se mantiene sincronizada con su instancia de Cast Operations en el momento en que se despliega.
...
2. Confirme que su instancia de Cast Operations se sirva a través de HTTPS con un certificado válido.
...
- **Windows:** **Configuración → Aplicaciones → Cast Operations → Desinstalar**, o haga clic derecho en la entrada del menú Inicio.
...
- Deshabilite la optimización de batería: **Ajustes → Aplicaciones → Cast Operations On-Call → Batería → Sin restricciones**.
- Deshabilite el Ahorro de datos para la aplicación.
- En dispositivos Samsung, desactive **Mantenimiento del dispositivo → Batería → Límites de uso en segundo plano** para Cast Operations On-Call.
...
- Añada Cast Operations On-Call a la lista de permitidos de cualquier modo de Concentración activo.
...
- Vuelva a habilitar el desbloqueo biométrico desde la pantalla **Configuración** dentro de la aplicación Cast Operations On-Call.
...
- Asegúrese de que la instancia de Cast Operations se sirva a través de HTTPS.
...
- Abra un issue en el [repositorio de Cast Operations en GitHub](https://github.com/autonomy-cloud/operations).
- Contacte con soporte a través de su panel de Cast Operations.
