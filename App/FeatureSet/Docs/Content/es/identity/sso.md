...
Cast Operations admite el inicio de sesión único (SSO) basado en SAML 2.0 para la autenticación empresarial. SSO permite que los miembros de tu equipo inicien sesión en Cast Operations usando las credenciales de la organización, lo que proporciona una gestión de acceso centralizada y mayor seguridad.
...
- **Reducción de la fatiga de contraseñas**: Los usuarios no necesitan recordar una contraseña separada de Cast Operations

## Configuración de SSO

1. **Navegar a la configuración del proyecto**

   - Ve a tu proyecto de Cast Operations
...
3. **Obtener los metadatos SSO de Cast Operations**
...
Keycloak es una popular solución de código abierto para gestión de identidades y accesos. Sigue estos pasos para configurar Keycloak como proveedor de identidad SAML para Cast Operations.

### Prerrequisitos

- Una instancia de Keycloak en ejecución con un dominio configurado
- Acceso de administrador tanto a Keycloak como a Cast Operations
- Cuenta de Cast Operations con soporte SSO

### Paso 1: Configurar SSO en Cast Operations
...
4. Copia el valor del certificado y pégalo en Cast Operations en el siguiente formato:
...
4. Establece el **ID de cliente** en el valor del **Identificador (ID de entidad)** de la **Vista de configuración SSO** de Cast Operations
5. Establece las **URIs de redirección válidas** en tu URL de Cast Operations
6. Establece la **URL raíz** en la URL base de Cast Operations
7. Pega la **URL de respuesta (URL del Servicio de consumidor de aserciones)** de Cast Operations en el campo **URL de enlace POST del Servicio de consumidor de aserciones**
...
1. Guarda todos los ajustes tanto en Keycloak como en Cast Operations
2. Intenta iniciar sesión en Cast Operations usando SSO
3. Deberías ser redirigido a la página de inicio de sesión de Keycloak y de vuelta a Cast Operations tras la autenticación exitosa
...
Microsoft Entra ID es el servicio de gestión de identidades y accesos en la nube de Microsoft. Sigue estos pasos para configurar Entra ID como proveedor de identidad SAML para Cast Operations.

### Prerrequisitos

- Inquilino de Microsoft Entra ID (cualquier nivel que admita aplicaciones empresariales con SSO SAML)
- Acceso de administrador tanto a Microsoft Entra ID como a Cast Operations
- Cuenta de Cast Operations con soporte SSO
...
5. Ingresa un nombre (por ejemplo, "Cast Operations")
...
3. En **Configuración básica de SAML**, haz clic en **Editar** y establece:
   - **Identificador (ID de entidad)**: Pega el **Identificador (ID de entidad)** de la **Vista de configuración SSO** de Cast Operations
   - **URL de respuesta (URL del Servicio de consumidor de aserciones)**: Pega la **URL de respuesta** de la **Vista de configuración SSO** de Cast Operations
...
6. En la sección **Configurar Cast Operations**, copia:
   - **URL de inicio de sesión**: pégala como la **URL de inicio de sesión** en Cast Operations
   - **Identificador de Azure AD**: pégalo como el **Emisor** en Cast Operations
7. Vuelve a Cast Operations y pega el certificado y las URL, luego guarda
...
1. Guarda todos los ajustes tanto en Entra ID como en Cast Operations
2. Intenta iniciar sesión en Cast Operations usando SSO
3. Deberías ser redirigido a la página de inicio de sesión de Microsoft y de vuelta a Cast Operations tras la autenticación exitosa

### Solución de problemas con Microsoft Entra ID

- **Error AADSTS700016**: El Identificador (ID de entidad) en Entra ID no coincide con Cast Operations: verifica que ambos valores sean idénticos
...
- **Error de ID de nombre**: Asegúrate de que la notificación del ID de nombre esté establecida en una dirección de correo electrónico que coincida con el correo del usuario en Cast Operations

---

## Configuración SAML de Okta

Okta es una plataforma de identidad ampliamente utilizada que proporciona sólidas capacidades de SSO SAML. Sigue estos pasos para configurar Okta como proveedor de identidad SAML para Cast Operations.
...
5. Ingresa "Cast Operations" como el **Nombre de la aplicación** y haz clic en **Siguiente**
6. En la sección **Configuración de SAML**, configura:
   - **URL de inicio de sesión único**: Pega la **URL de respuesta (URL del Servicio de consumidor de aserciones)** de la **Vista de configuración SSO** de Cast Operations
   - **URI de audiencia (ID de entidad del SP)**: Pega el **Identificador (ID de entidad)** de la **Vista de configuración SSO** de Cast Operations
...
### Paso 3: Copiar los metadatos SAML de Okta a Cast Operations
...
3. Desde los metadatos XML o desde los detalles de la pestaña **Inicio de sesión**:
   - Copia la **URL de inicio de sesión** (también llamada **URL de inicio de sesión único del proveedor de identidad**): pégala como la **URL de inicio de sesión** en Cast Operations
   - Copia el **Emisor** (también llamado **Emisor del proveedor de identidad**): pégalo como el **Emisor** en Cast Operations
4. Descarga el certificado de firma:
   ...
   - Pega el certificado en Cast Operations (incluyendo las líneas `BEGIN CERTIFICATE` y `END CERTIFICATE`)
5. Guarda la configuración SSO de Cast Operations
...
1. Guarda todos los ajustes tanto en Okta como en Cast Operations
2. Intenta iniciar sesión en Cast Operations usando SSO
3. Deberías ser redirigido a la página de inicio de sesión de Okta y de vuelta a Cast Operations tras la autenticación exitosa

### Solución de problemas con Okta

- **Error 404 o URL SSO no válida**: Verifica que la **URL de inicio de sesión único** en Okta coincida exactamente con la **URL de respuesta** de Cast Operations
- **Error de audiencia**: Asegúrate de que el **URI de audiencia** en Okta coincida exactamente con el **Identificador (ID de entidad)** de Cast Operations
...
La implementación SSO de Cast Operations usa el protocolo SAML 2.0 y debería funcionar con cualquier proveedor de identidad compatible. Los pasos generales de configuración son:

1. En Cast Operations, crea una configuración SSO y anota el **Identificador (ID de entidad)** y la **URL de respuesta (URL del Servicio de consumidor de aserciones)** del botón **Ver configuración SSO**
2. En tu proveedor de identidad, crea una aplicación SAML usando:
   - **URL del Servicio de consumidor de aserciones / URL de respuesta**: De la configuración SSO de Cast Operations
   - **ID de entidad / URI de audiencia**: De la configuración SSO de Cast Operations
   - **Formato del ID de nombre**: Dirección de correo electrónico
3. De tu proveedor de identidad, copia lo siguiente en Cast Operations:
...
Cast Operations actualmente no admite la asignación de roles SAML desde tu proveedor de identidad. El control de acceso basado en roles debe configurarse por separado dentro de la **Configuración del proyecto** > **SSO** de Cast Operations, donde puedes asignar roles predeterminados para usuarios SSO.
