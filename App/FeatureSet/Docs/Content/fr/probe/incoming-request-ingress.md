...
Lorsque `PROBE_INGRESS_PORT` est défini, la sonde lie un écouteur HTTP supplémentaire sur ce port. L'écouteur accepte les mêmes chemins d'URL `secretkey` que les points d'accès publics Cast Operations :
...
La sonde proxy ensuite la requête vers votre instance Cast Operations, en préservant la méthode, le corps et les en-têtes de requête (moins les en-têtes hop-by-hop tels que `Host`, `Connection`, `Content-Length`, etc.). La sonde attache automatiquement un en-tête `Cast Operations-Probe-Id` afin que la requête soit attribuée à la sonde de transmission.
...
Les services internes peuvent alors envoyer des signaux de vie à `http://cast-operations-probe-ingress.<espace-de-noms>.svc.cluster.local:3875/heartbeat/<clé-secrète>`.
...
- **Réponse synchrone, transmission asynchrone.** La sonde accuse réception de la requête entrante immédiatement avec un `200` et transmet à Cast Operations en arrière-plan. Votre service n'a pas à attendre que la transmission se termine.
- **Les en-têtes sont préservés.** Tous les en-têtes sauf les hop-by-hop (`Host`, `Connection`, `Content-Length`, `Transfer-Encoding`, `Keep-Alive`, `Proxy-Authenticate`, `Proxy-Authorization`, `TE`, `Trailer`, `Upgrade`) sont transmis. La sonde ajoute un en-tête `Cast Operations-Probe-Id` l'identifiant.
...
| `PROBE_INGRESS_FORWARD_TIMEOUT_MS`  | `10000`                  | Délai d'attente (ms) pour chaque tentative de transmission vers Cast Operations. Minimum `1000`.                        |
...
Les variables standard de la sonde (`PROBE_KEY`, `PROBE_ID`, `CAST_OPERATIONS_URL`, variables de proxy) s'appliquent toutes — voir [Sondes personnalisées](/docs/probe/custom-probe) pour la liste complète.
...
- **`Probe ingress: failed to forward to <url> after N attempts`** — la sonde n'a pas pu atteindre Cast Operations. Vérifiez la connectivité sortante de la sonde, les paramètres de proxy et la valeur de `CAST_OPERATIONS_URL`.
...
- **Le signal de vie apparaît dans Cast Operations mais pas via la sonde** — confirmez que votre service atteint `http://<hôte-sonde>:<port>/...` et non l'URL publique. Une entrée DNS ou `/etc/hosts` mal configurée est la cause habituelle.
