...
La surveillance manuelle vous permet de créer des moniteurs dont le statut est géré entièrement à la main ou via l'API. Cast Operations n'effectue aucune vérification automatisée — vous contrôlez directement le statut du moniteur.

## Vue d'ensemble

Les moniteurs manuels sont des espaces réservés que vous mettez à jour vous-même. Cela est utile pour :

- Intégrer avec des outils de surveillance externes qui mettent à jour le statut via l'API Cast Operations
...
- **Tableau de bord** — Changez le statut du moniteur directement depuis le tableau de bord Cast Operations
- **API** — Mettez à jour le statut du moniteur par programmation en utilisant l'API Cast Operations
...
| Statut piloté par API              | Permettre à des outils externes de mettre à jour le statut du moniteur via l'API Cast Operations                  |
| Espaces réservés de page de statut | Afficher des composants sur votre page de statut qui sont gérés en dehors de Cast Operations                      |
