...
L'agent est publié sous le nom `ghcr.io/autonomy-cloud/operations/docker-agent:release` sur Docker Hub. Consultez le [guide d'installation de l'agent Docker](https://github.com/autonomy-cloud/operations/tree/master/DockerAgent) pour les exemples complets `docker run` et `docker compose`.
