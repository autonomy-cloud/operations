...
[Zabbix](https://www.zabbix.com) surveille vos serveurs et votre réseau ; Cast Operations gère votre réponse aux incidents, les astreintes et les pages de statut. Connectez les deux et chaque problème Zabbix devient automatiquement un incident Cast Operations — ainsi les bonnes personnes sont alertées et votre page de statut reste fiable.

Cette intégration est **entrante** : Zabbix envoie des problèmes à Cast Operations. Elle utilise un **type de média webhook** Zabbix d'un côté et un **[Workflow](/docs/workflows/index)** Cast Operations de l'autre. Aucun plugin, aucun service supplémentaire.
...
2. Une **action** Zabbix demande au type de média **Cast Operations** d'envoyer l'événement.
3. Le script du type de média POSTs une petite charge utile JSON vers l'URL du workflow Cast Operations.
...
- Votre serveur Zabbix doit pouvoir atteindre votre instance Cast Operations via HTTPS.
- Un projet Cast Operations où vous pouvez créer des workflows.

## Partie 1 — Créer le workflow Cast Operations
...
4. Faites glisser un bloc **Create Incident** et connectez-le à la sortie **Yes** du bloc Conditions. Remplissez :
   ...
   - **Severity** : choisissez la gravité d'incident Cast Operations souhaitée (vous pourrez l'affiner plus tard avec davantage de branches Conditions qui associent les gravités Zabbix).
...
### Étape 1 : Créer le type de média Cast Operations
...
1. Allez dans **Users → Users → Create user**. Nommez-le `Cast Operations Webhook`, donnez-lui un rôle permettant de recevoir des notifications (par ex. **User role**), et ajoutez-le à un groupe d'utilisateurs.
...
### Étape 3 : Envoyer les problèmes à Cast Operations avec une action
...
4. Dans l'onglet **Operations**, ajoutez une opération qui envoie à **User: Cast Operations Webhook** via le type de média **Cast Operations**.
...
1. De retour dans le workflow Cast Operations, activez **Enabled**.
...
4. Vérifiez **Incidents** dans Cast Operations — votre problème Zabbix est maintenant un incident.
...
Comme la résolution dépend de la façon dont vous modélisez les états d'incident dans votre projet, conservez le chemin **create** comme noyau fiable et superposez le chemin de résolution une fois que vous avez confirmé que les événements circulent correctement. Voir [Composants → Composants de données Cast Operations](/docs/workflows/components#cast-operations-data-components).

## Association des gravités Zabbix (optionnel)

Les gravités Zabbix (`Not classified`, `Information`, `Warning`, `Average`, `High`, `Disaster`) arrivent sous `{{Zabbix.Request Body.severity}}`. Pour les associer aux gravités d'incident Cast Operations, ajoutez des branches **Conditions** avant **Create Incident** — par exemple, routez `Disaster` et `High` vers un incident « Critical » et tout le reste vers « Major ». Créez un bloc **Create Incident** par branche.
...
- Une réponse non 2xx de Cast Operations est signalée par le `throw` dans le script — vérifiez que l'URL du workflow est exactement correcte.
...
- Si vous exécutez Cast Operations en auto-hébergé derrière un pare-feu, autorisez l'IP de sortie de votre serveur Zabbix à l'atteindre via HTTPS.
