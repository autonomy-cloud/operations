...
Un **Agent de runbook** est un petit processus auto-hébergé qui exécute les étapes Bash _et_ JavaScript de vos runbooks **dans votre propre infrastructure**. Le Worker Cast Operations n'exécute jamais vos scripts — il les met en file d'attente, et l'Agent de runbook que l'auteur de l'étape a choisi les réclame, les exécute et renvoie le résultat.
...
Les versions précédentes d'Cast Operations exécutaient les étapes Bash et JavaScript sur le Worker. JavaScript était en bac à sable (`isolated-vm`), Bash non. Les deux posaient problème pour tout ce qui dépasse un déploiement self-hosted single-tenant :
...
- **Portée.** La plupart des étapes utiles veulent agir sur l'infrastructure du _client_ (« redémarrer ce service », « kubectl sur notre cluster », « rechercher un enregistrement dans notre BDD interne ») — pas sur celle d'Cast Operations.
...
1. Vous créez un Agent de runbook dans Cast Operations. Cast Operations génère un ID et une clé secrète.
2. Vous lancez le conteneur de l'agent sur un hôte de votre infrastructure avec cet ID/clé et votre URL Cast Operations.
3. L'agent demande à Cast Operations toutes les quelques secondes : « du travail pour moi ? »
...
L'agent n'a besoin que de **HTTPS sortant** vers votre instance Cast Operations. Il n'accepte aucune connexion entrante.
...
- atteindre votre instance Cast Operations en HTTPS, et
...
Si une clé fuite, ouvrez l'agent dans Cast Operations et réinitialisez sa clé. L'ancienne cesse immédiatement de fonctionner. Mettez à jour le conteneur de l'agent avec la nouvelle clé et redémarrez-le.
