# Utiliser Fluentd pour envoyer des données de télémétrie à Cast Operations

## Vue d'ensemble

Vous pouvez utiliser le plugin [Fluentd](https://www.fluentd.org/) pour collecter des journaux et des données de télémétrie depuis vos applications et services. Le plugin envoie les données de télémétrie à la source HTTP Cast Operations. Vous pouvez utiliser le plugin de sortie http de fluentd pour envoyer les données de télémétrie à la source HTTP Cast Operations. Ce plugin peut être trouvé ici : https://docs.fluentd.org/output/http

## Démarrage

Fluentd prend en charge des centaines de sources de données et vous pouvez ingérer des journaux depuis n'importe laquelle de ces sources dans Cast Operations. Parmi les sources populaires, on trouve :
...
Vous pouvez utiliser la configuration suivante pour envoyer les données de télémétrie à la source HTTP Cast Operations. Vous pouvez ajouter cette configuration au fichier de configuration de fluentd. Le fichier de configuration se trouve généralement à `/etc/fluentd/fluent.conf` ou `/etc/td-agent/td-agent.conf`.

Vous devez remplacer `YOUR_SERVICE_TOKEN` par le jeton que vous avez créé à l'étape précédente. Vous devez également remplacer `YOUR_SERVICE_NAME` par le nom de votre service. Le nom du service peut être n'importe quel nom que vous souhaitez. Si le service n'existe pas dans Cast Operations, il sera créé automatiquement.
...
**Si vous auto-hébergez Cast Operations** : Si vous auto-hébergez Cast Operations, vous pouvez remplacer l'`endpoint_url` par l'URL de votre instance Cast Operations. `http(s)://VOTRE_HÔTE_CAST_OPERATIONS/fluentd/logs`
