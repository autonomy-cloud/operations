# Appareils IoT Cast Operations

## Vue d'ensemble

Cast Operations surveille des flottes d'appareils IoT — capteurs, passerelles, contrôleurs et boîtiers edge — en ingérant un petit ensemble de métriques `iot_*`, étiquetées avec la **flotte** à laquelle chaque relevé appartient et son propre **identifiant d'appareil**. Cast Operations regroupe ces métriques dans une flotte, construit un inventaire d'appareils en temps réel et suit pour chaque appareil la batterie, la connectivité, la température, le CPU, la mémoire et la disponibilité.
...
- **MQTT** — connectez-vous directement à l'endpoint MQTT intégré de Cast Operations (MQTT sur WebSocket à l'adresse `wss://<your-host>/mqtt`, ou MQTT TCP brut sur les déploiements auto-hébergés) et publiez des relevés JSON. Aucun collector requis, et la prise en charge du Last Will offre une détection hors ligne instantanée.
...
- Un appareil, une passerelle ou un collector capable d'envoyer du OTLP/HTTP vers Cast Operations
- Une accessibilité réseau depuis l'appareil/la passerelle vers votre instance Cast Operations
- Un **jeton d'ingestion de télémétrie Cast Operations** — créez-en un depuis _Project Settings → Telemetry Ingestion Keys_ et copiez la valeur `x-cast-operations-token`

## Comment Cast Operations modélise l'IoT

Cast Operations fait correspondre vos appareils à deux concepts à l'aide des attributs de ressource OpenTelemetry :

- **Flotte** — un groupe logique d'appareils (par exemple `building-a-sensors` ou `field-gateways`). La flotte est dérivée de l'attribut de ressource `iot.fleet.name` et apparaît dans Cast Operations comme le service de télémétrie `iot/<fleet>`. Définissez `service.name=iot/<fleet>` pour que les logs et les métriques s'alignent sous le même service.
- **Appareil** — un appareil individuel au sein d'une flotte, identifié par l'attribut `device.id`. Cast Operations construit et maintient un inventaire d'appareils par flotte, indexé sur `device.id`.

Des attributs facultatifs affinent la façon dont chaque appareil est classé et délimité dans les moniteurs :

| Attribut             | Requis   | Description                                                                       |
| -------------------- | -------- | -------------------------------------------------------------------------------- |
| `iot.fleet.name`     | Oui      | La flotte à laquelle cet appareil appartient. Devient le service Cast Operations `iot/<fleet>` |
...
| ----------------------------- | -------- | ---------------------------------------------------------------------------------------------------- |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | Oui      | Endpoint OTLP de Cast Operations (`https://latticeruntime.com/otlp`, ou `http(s)://YOUR-OPERATIONS-HOST/otlp` en auto-hébergé) |
| `OTEL_EXPORTER_OTLP_HEADERS`  | Oui      | `x-cast-operations-token=YOUR_TELEMETRY_INGESTION_TOKEN`                                                    |
...
Émettez vos relevés sous forme de métriques en utilisant les noms `iot_*` ci-dessous (voir [Conventions de métriques](#conventions-de-métriques)). En l'espace d'une minute environ, l'appareil apparaît dans la section **IoT** du tableau de bord Cast Operations.

## Envoyer des métriques via un OpenTelemetry Collector

Lorsque de nombreux appareils rapportent via une passerelle, exécutez un OpenTelemetry Collector sur la passerelle et exportez vers Cast Operations. Le processeur `resource` estampille les attributs de flotte ; recevez les relevés de vos appareils (OTLP, pont MQTT, fichiers de logs, etc.) et transférez-les :
...
- Conservez `device.id` (et éventuellement `iot.device.kind` / `iot.device.type` / `iot.device.firmware`) sur chaque point de données pour que Cast Operations puisse résoudre l'appareil individuel à l'intérieur de la flotte.
- **`otlphttp`** envoie vers Cast Operations en HTTPS avec le jeton d'ingestion attaché. L'encodage protobuf par défaut et `encoding: json` sont tous deux acceptés.

## Envoyer des métriques via MQTT

Cast Operations embarque un endpoint MQTT intégré, si bien que les appareils qui parlent déjà MQTT peuvent envoyer leurs relevés directement — sans SDK OpenTelemetry, sans collector et sans pont. Tout ce qui est publié via MQTT atterrit dans le même pipeline que l'OTLP : les flottes sont créées automatiquement, l'inventaire d'appareils se met à jour, et chaque moniteur et modèle d'alerte IoT fonctionne sans modification.

**Endpoints**

| Transport             | Adresse                                | Notes                                                                                     |
| --------------------- | -------------------------------------- | ----------------------------------------------------------------------------------------- |
| MQTT sur WebSocket    | `wss://<your-host>/mqtt`               | Fonctionne sur tous les déploiements — passe par le port HTTPS habituel via l'ingress Cast Operations |
...
**Topics** — publiez sous le préfixe fixe `cast-operations/`. Les segments de flotte et d'appareil ne doivent pas contenir `/`, `+` ni `#`, et sont limités à 100 caractères :

| Topic                                            | Charge utile                                                                                         |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
| `cast-operations/<fleet>/<device>/telemetry`           | Objet JSON de relevés — `{ "metrics": { "iot_temperature_celsius": 21.5 } }`, ou un objet plat dont les champs numériques sont les métriques |
| `cast-operations/<fleet>/<device>/metrics/<metricName>`| Une valeur unique — un nombre brut (`23.4`) ou `{ "value": 23.4 }`                                    |
| `cast-operations/<fleet>/<device>/status`              | `"online"` ou `"offline"` (également `1`/`0`, `true`/`false`, `up`/`down`) — correspond à `iot_device_up` |
...
Exemple avec le module Node.js `mqtt` sur WebSocket (fonctionne avec latticeruntime.com et toute instance auto-hébergée) :

```javascript
const mqtt = require("mqtt");

const client = mqtt.connect("wss://latticeruntime.com/mqtt", {
  username: "cast-operations", // ignoré — c'est le jeton ci-dessous qui authentifie
...
- Les publications hors du contrat de topics ou avec des charges utiles malformées sont acceptées puis **abandonnées** (MQTT 3.1.1 n'a pas de réponse d'erreur par message) — le serveur journalise un avertissement avec la raison, vérifiez donc les logs de l'application Cast Operations si les données n'arrivent pas.
- Sur l'endpoint WebSocket, gardez le keepalive MQTT **en dessous de 5 minutes** — l'ingress Cast Operations ferme les connexions WebSocket inactives après 300 secondes, ce qui déclencherait votre Last Will et une fausse alerte Device Offline. Les valeurs par défaut des bibliothèques clientes (60 s pour `mqtt` et `paho-mqtt`) conviennent. L'endpoint TCP brut n'a pas ce plafond.
...
Cast Operations reconnaît les noms de métriques `iot_*` suivants. Chaque point de données doit porter le label `device.id` afin que le relevé soit attribué au bon appareil. Vous n'avez besoin d'envoyer que les métriques qui ont du sens pour votre appareil — celles qui sont absentes ne sont tout simplement pas tracées.
...
| `iot_cpu_usage_ratio`       | Utilisation du CPU sous forme de ratio `0`–`1` (Cast Operations le stocke en pourcentage) |
...
2. Dans le tableau de bord Cast Operations, ouvrez la section **IoT** — votre flotte devrait apparaître comme `iot/<fleet>` en l'espace d'une minute environ.
...
2. Confirmez que l'endpoint de l'exportateur est `https://latticeruntime.com/otlp` (ou votre `…/otlp` auto-hébergé) et que l'en-tête `x-cast-operations-token` porte un jeton valide.
3. Si vous utilisez MQTT, confirmez que le topic suit exactement `cast-operations/<fleet>/<device>/…` — c'est le segment de flotte du topic qui crée la flotte.
...
Le jeton d'ingestion est invalide, révoqué ou manquant. Générez-en un nouveau depuis _Project Settings → Telemetry Ingestion Keys_ et mettez à jour l'en-tête `x-cast-operations-token`.
...
2. Rappelez-vous que `iot_cpu_usage_ratio` est un ratio `0`–`1` ; envoyez le ratio brut et Cast Operations l'affiche en pourcentage.
...
Si vous auto-hébergez Cast Operations, pointez l'endpoint vers votre propre instance :
...
Pour MQTT, connectez-vous à `wss://your-operations-host.example.com/mqtt`, ou exposez le port MQTT TCP brut du service app (`MQTT_INGEST_PORT`, `1883` par défaut) si vos appareils ne savent pas parler WebSocket. Définissez `MQTT_INGEST_ENABLED=false` sur le service app pour désactiver entièrement les listeners MQTT.
...
- Pour apprendre en profondeur l'intégration OTLP sous-jacente, consultez [Intégrer OpenTelemetry avec Cast Operations](/docs/telemetry/open-telemetry).
