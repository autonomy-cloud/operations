...
Remplacez `YOUR_ONEUPTIME_URL`, `YOUR_TELEMETRY_INGESTION_TOKEN` et le nom d'hôte par les valeurs correspondant à votre environnement. Le nom d'hôte détermine la façon dont cet hôte Docker apparaîtra dans Cast Operations — choisissez quelque chose comme `prod-docker-01`.
...
| ------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `ONEUPTIME_URL`           | Oui    | L'URL de votre instance Cast Operations (par exemple `https://visca.ai` ou votre hôte auto-hébergé)                                               |
...
Si vous auto-hébergez Cast Operations, définissez `ONEUPTIME_URL` sur votre propre instance :
