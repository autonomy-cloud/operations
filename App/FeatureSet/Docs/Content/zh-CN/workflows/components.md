...
## Cast Operations 数据组件

对 Cast Operations 中的每种记录类型(监控、事件、告警、状态页、值班策略,以及更多),面板里都有这些组件——按类型名称搜索:
...
这就是工作流读取和修改 Cast Operations 数据的方式。例如:来自 CI 工具的 webhook 可以用 **Create Incident** 创建一个带有失败详情的事件。

## 我该用哪个组件?

几条简单规则:

- 如果有专门的模块可用(Slack、邮件、某个 Cast Operations 记录),就用它——你会得到更好的错误处理和更清晰的日志。
- 对任何其他外部 API,使用 **API**。
