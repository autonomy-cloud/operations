...
在您的 iPhone 或 iPad 上从 Apple App Store 安装 **Cast Operations On-Call** 原生 iOS 应用。
...
2. 点按 **搜索** 标签并搜索 **"Cast Operations On-Call"**,或在设备上打开此链接:
...
4. 安装完成后,点按 **打开**,或从主屏幕启动 **Cast Operations On-Call**。
...
1. 前往 **设置 → 通知 → Cast Operations On-Call**。
2. 启用 **允许通知**、**声音**、**徽标** 以及 **锁定屏幕 / 横幅 / 通知中心** 投递。
3. 将 **通知分组** 设置为 **自动**。
4. 如果您正在值班,请在轮班期间禁用 **低电量模式**,并避免强制退出应用 — 如果强制关闭应用,iOS 可能会延迟后台投递。
5. 将 **Cast Operations On-Call** 添加到您仍希望接收呼叫的所有 **专注模式** 中。

## 更新

应用通过 App Store 更新:

- 打开 **App Store**,点按您的头像,滚动至 **Cast Operations On-Call**,然后点按 **更新**。
- 或在 **设置 → App Store → App 更新** 中启用自动安装更新。

## 卸载

1. 在主屏幕上 **长按** **Cast Operations On-Call** 图标。
2. 点按 **移除 App → 删除 App**。
3. 点按 **删除** 进行确认。
...
- 打开 **设置 → 通知 → Cast Operations On-Call**,确认已允许通知。
- 禁用 **勿扰模式**,或将 Cast Operations On-Call 添加到当前激活的专注模式的允许列表中。
