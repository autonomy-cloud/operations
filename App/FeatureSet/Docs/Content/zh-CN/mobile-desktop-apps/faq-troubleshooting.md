...
Cast Operations 移动和桌面应用的常见问题及解决方案。

## Cast Operations 是如何分发其应用的?

- **移动端(iOS 和 Android):** Cast Operations 提供一款名为 **Cast Operations On-Call** 的原生应用。该应用已发布到 [Apple App Store](https://github.com/autonomy-cloud/operations/releases) 和 [Google Play](https://github.com/autonomy-cloud/operations/releases)。对于没有 Google Play 的 Android 设备,也提供经过签名的 [APK 下载](https://github.com/autonomy-cloud/operations/releases/latest/download/cast-operations-on-call-android-app.apk)。
- **桌面端(Windows、macOS、Linux):** Cast Operations Web 控制面板是一个渐进式 Web 应用 (PWA)。您可以直接从基于 Chromium 的浏览器或 Safari 将其安装为桌面应用 — 无需任何商店账户。
...
是的。Cast Operations On-Call 应用可免费安装。您使用现有的 Cast Operations 账户登录。

### 我可以将该应用与自托管的 Cast Operations 实例一起使用吗?

可以。首次启动时,应用会询问 **服务器 URL**。请输入您自托管实例的 URL(例如 `https://operations.example.com`)。应用会先验证服务器可访问,然后才允许您登录。
...
1. 在操作系统级别已为 **Cast Operations On-Call** 启用通知。
2. 已禁用电池优化并允许后台活动(Android)。
3. 勿扰模式或专注模式已关闭,或者该应用已在例外列表中。
4. 您已登录 — 推送令牌仅在登录后才会注册到服务器。
5. **仅限自托管:** 您的 Cast Operations 实例已配置推送通知。请参阅 [推送通知](/docs/self-hosted/push-notifications) 指南。
...
可以。您可以根据需要在任意多台设备上使用同一个 Cast Operations 账户登录。每台设备都会收到自己的推送通知。

### 如何卸载?

- **iOS:** 长按图标 → **移除 App** → **删除 App**。
- **Android:** 长按图标 → **卸载**,或前往 **设置 → 应用 → Cast Operations On-Call → 卸载**。

您的 Cast Operations 账户和数据存储在服务器上,卸载应用时不会被删除。
...
- **即时更新** — 您一旦部署,应用便会立即与您的 Cast Operations 实例保持同步。
...
2. 确认您的 Cast Operations 实例通过 HTTPS 提供服务并使用有效证书。
3. 清除浏览器缓存并重新加载。
...
- **Windows:** **设置 → 应用 → Cast Operations → 卸载**,或右键点击开始菜单条目。
...
- 禁用电池优化:**设置 → 应用 → Cast Operations On-Call → 电池 → 无限制**。
- 为该应用禁用数据节省程序。
- 在 Samsung 设备上,关闭 Cast Operations On-Call 的 **设备维护 → 电池 → 后台使用限制**。

**推送通知延迟或丢失 (iOS):**

- 避免强制退出应用 — iOS 可能会暂停后台投递。
- 在您值班期间禁用低电量模式。
- 将 Cast Operations On-Call 添加到所有激活的专注模式的允许列表中。

**Face ID / Touch ID / 指纹无法正常工作:**

- 确保已在操作系统设置中注册生物识别信息。
- 从 Cast Operations On-Call 应用内的 **设置** 屏幕重新启用生物识别解锁。
...
- 确保 Cast Operations 实例通过 HTTPS 提供服务。
...
- 在 [Cast Operations GitHub 仓库](https://github.com/autonomy-cloud/operations) 上提交 issue。
- 通过您的 Cast Operations 控制面板联系支持。
