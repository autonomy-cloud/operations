...
[Zabbix](https://www.zabbix.com) 监控你的服务器和网络；Cast Operations 负责事件响应、值班和状态页。将两者连接起来，每个 Zabbix 问题都会自动成为 Cast Operations 事件——让合适的人收到告警，状态页保持实时准确。

此集成为**入站**模式：Zabbix 向 Cast Operations 发送问题。一侧使用 Zabbix **webhook 媒体类型**，另一侧使用 Cast Operations **[工作流](/docs/workflows/index)**。无需插件，无需额外服务。
...
2. Zabbix **动作**通知 **Cast Operations** 媒体类型发送事件。
3. 媒体类型的脚本将一小段 JSON 负载 POST 到 Cast Operations 工作流 URL。
4. 工作流读取负载并创建事件（可选地，在 Zabbix 恢复时解决事件）。

## 前提条件

- 一台你管理的 Zabbix 服务器（本指南针对 **Zabbix 6.0 LTS / 7.0 LTS**；webhook 媒体类型在 5.0+ 上同样有效）。
- 你的 Zabbix 服务器必须能通过 HTTPS 访问你的 Cast Operations 实例。
- 一个可以创建工作流的 Cast Operations 项目。

## 第一部分——构建 Cast Operations 工作流
...
4. 拖入 **Create Incident** 模块并连接到 Conditions 模块的 **Yes** 输出。填写：
   ...
   - **Severity**：选择你想要的 Cast Operations 事件严重程度（之后可以通过更多 Conditions 分支来映射 Zabbix 严重程度）。
...
1. 前往 **Users → Users → Create user**，命名为 `Cast Operations Webhook`，赋予其可以接收通知的角色（例如 **User role**），并将其加入用户组。
...
### 步骤 3：通过动作将问题发送到 Cast Operations
...
Zabbix 严重程度（`Not classified`、`Information`、`Warning`、`Average`、`High`、`Disaster`）以 `{{Zabbix.Request Body.severity}}` 形式传入。要将它们映射到 Cast Operations 事件严重程度，在 **Create Incident** 之前添加 **Conditions** 分支——例如，将 `Disaster` 和 `High` 路由到"Critical"事件，将其他所有内容路由到"Major"。每个分支各建一个 **Create Incident** 模块。
...
- Cast Operations 返回非 2xx 响应时会被脚本中的 `throw` 捕获——检查工作流 URL 是否完全正确。
...
- 如果你在防火墙后自托管 Cast Operations，允许你的 Zabbix 服务器的出口 IP 通过 HTTPS 访问它。
