...
每当创建 Cast Operations 事件时，自动创建一个 [Jira](https://www.atlassian.com/software/jira) 工单——让工程工作在开发者已经使用的地方得到追踪，并附带指向事件的链接。

此集成为**出站**模式：Cast Operations 调用 Jira 的 REST API。它使用带有 **Incident → On Create** 触发器和 **API 组件**的 Cast Operations **[工作流](/docs/workflows/index)**。你可以选择添加**入站**路径，以便关闭 Jira 工单时解决 Cast Operations 事件。
...
1. 开启工作流 **Enabled**。
2. 在 Cast Operations 中创建一个测试事件（或从监控器触发一个）。
...
要在有人关闭 Jira 工单时解决 Cast Operations 事件，添加一个**入站**工作流：
...
2. 在 Jira 中，前往 **Project settings → Automation → Create rule**：
   ...
   - **动作**：_Send web request_ → 方法 `POST`，URL = 你的工作流 webhook URL，正文包含工单密钥和 Cast Operations 事件 ID，例如：
