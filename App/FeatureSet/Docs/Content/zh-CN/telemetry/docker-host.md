...
Cast Operations Docker Agent 是一个预构建的容器镜像，内置了经过调优的 OpenTelemetry Collector 配置。将它与你现有的容器一起运行，它会自动发现主机上的每一个容器，采集 CPU / 内存 / 网络 / 块 I/O 指标以及容器日志，并通过 OTLP 将所有数据转发到 Cast Operations。单个镜像，单条命令。
...
- 一个 **Cast Operations 遥测摄取令牌（Telemetry Ingestion Token）** —— 从 _Project Settings → Telemetry Ingestion Keys_ 创建一个并复制其值
...
大约一分钟内，该主机就应当出现在 Cast Operations 仪表板中，并开始持续上报指标和日志。
...
- 如果是 Kubernetes 集群而非独立的 Docker 主机，请使用 [Cast Operations Kubernetes Agent](/docs/telemetry/kubernetes-agent)。
