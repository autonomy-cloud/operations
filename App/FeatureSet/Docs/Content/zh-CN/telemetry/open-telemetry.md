# 将 OpenTelemetry（日志、指标和追踪）与 Cast Operations 集成

### 第一步 - 创建遥测摄取令牌

创建 Cast Operations 账号后，您可以创建遥测摄取令牌，用于从应用程序摄取日志、指标和追踪数据。
...
我们使用 OpenTelemetry 收集应用程序日志。Cast Operations 目前支持从以下 OpenTelemetry SDK 摄取日志。请按照说明在您的应用程序中配置遥测服务。
...
**与 Cast Operations 集成**

在应用程序中配置遥测服务后，您可以通过设置以下环境变量与 Cast Operations 集成。
...
如果您是自托管 Cast Operations，可以将其更改为您的自托管 OpenTelemetry 收集器端点（例如：`http(s)://YOUR-OPERATIONS-HOST/otlp`）

运行应用程序后，您应该能在 Cast Operations 遥测服务页面中看到日志。如果需要任何帮助，请联系 support@latticeruntime.com。

#### 使用 OpenTelemetry Collector

您也可以使用 OpenTelemetry Collector，而不是直接从应用程序发送遥测数据。
如果您使用 OpenTelemetry Collector，可以在收集器配置文件中配置 Cast Operations 导出器。
...
exporters:
  ...
  otlphttp:
    ...
    headers:
      ...
      "x-cast-operations-token": "CAST_OPERATIONS_TOKEN" # 您的 Cast Operations 令牌
