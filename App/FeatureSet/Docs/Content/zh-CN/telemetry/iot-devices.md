# Cast Operations IoT 设备

## 概述

Cast Operations 通过采集标准的 OpenTelemetry (OTLP) 指标来监控 IoT 设备机群——传感器、网关、控制器和边缘盒子。每台设备（或代表它的网关）通过 OTLP HTTP 推送一小组 `iot_*` 指标，并标注它所属的**机群**及其自身的**设备 id**。Cast Operations 将这些指标归入一个机群，构建实时设备清单，并跟踪每台设备的电量、连接性、温度、CPU、内存和可用性。
...
Cast Operations 使用 OpenTelemetry 资源属性将你的设备映射到两个概念上：

- **机群（Fleet）**——一组设备的逻辑分组（例如 `building-a-sensors` 或 `field-gateways`）。机群由 `iot.fleet.name` 资源属性派生，并在 Cast Operations 中显示为遥测服务 `iot/<fleet>`。请设置 `service.name=iot/<fleet>`，以便日志和指标归在同一个服务下。
- **设备（Device）**——机群内的单台设备，由 `device.id` 属性标识。Cast Operations 以 `device.id` 为键构建并维护每个机群的设备清单。

可选属性可以进一步细化每台设备在监控器中的分类和范围划分方式：

| 属性                  | 是否必需 | 描述                                                                              |
| -------------------- | -------- | -------------------------------------------------------------------------------- |
| `iot.fleet.name`     | 是       | 该设备所属的机群。会成为 Cast Operations 服务 `iot/<fleet>`                              |
...
如果你的设备直接运行 OpenTelemetry SDK，请将其指向 Cast Operations，并通过标准的 `OTEL_*` 环境变量打上 IoT 资源属性。请将令牌、端点、机群名称和设备 id 替换为适合你环境的值。
...
| 环境变量                       | 是否必需 | 描述                                                                                                  |
| ----------------------------- | -------- | ---------------------------------------------------------------------------------------------------- |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | 是       | Cast Operations OTLP 端点（`https://latticeruntime.com/otlp`，自托管时为 `http(s)://YOUR-OPERATIONS-HOST/otlp`） |
...
当许多设备通过网关上报时，在网关上运行一个 OpenTelemetry Collector 并导出到 Cast Operations。`resource` 处理器负责打上机群属性；从你的设备接收读数（OTLP、MQTT 桥接、文件日志等）并将其转发出去：
...
exporters:
  otlphttp:
    ...
    # Cast Operations 要求使用 JSON 编码器，而不是默认的 Proto(buf)
    encoding: json
    headers:
...
- 在每个数据点上保留 `device.id`（可选保留 `iot.device.kind` / `iot.device.type` / `iot.device.firmware`），以便 Cast Operations 能在机群内解析出单台设备。
- **`otlphttp`** 通过 HTTPS 携带采集令牌发送到 Cast Operations。请注意 `encoding: json` 和 `Content-Type: application/json` 请求头是必需的。

## 通过 MQTT 发送指标

Cast Operations 内置了 MQTT 端点，因此已经能够使用 MQTT 通信的设备可以直接推送读数——无需 OpenTelemetry SDK、采集器或桥接。通过 MQTT 发布的一切都会进入与 OTLP 相同的管道：机群会被自动创建，设备清单会更新，每个 IoT 监控器和告警模板都无需改动即可工作。

**端点**

| 传输方式               | 地址                                     | 说明                                                                                       |
| --------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------ |
| MQTT over WebSocket   | `wss://<your-host>/mqtt`                | 在每种部署上都可用——通过 Cast Operations ingress 走常规的 HTTPS 端口                              |
...
使用 Node.js `mqtt` over WebSocket 的示例（适用于 latticeruntime.com 和任何自托管实例）：
...
- 在主题约定之外的发布，或负载格式错误的发布，会被接受并**丢弃**（MQTT 3.1.1 没有逐条消息的错误回复）——服务器会记录一条带有原因的警告，因此如果数据没有到达，请检查 Cast Operations 应用日志。
- 在 WebSocket 端点上，请将 MQTT keepalive 保持在 **5 分钟以内**——Cast Operations ingress 会在 300 秒后关闭空闲的 WebSocket 连接，这会触发你的 Last Will 以及一条虚假的设备离线告警。客户端库的默认值（`mqtt` 和 `paho-mqtt` 均为 60 秒）没有问题。原始 TCP 端点没有这一上限。
- 负载上限为 128 KB，且每次发布最多 100 个指标；超大的数据包会导致连接断开。

## 指标约定

Cast Operations 识别以下 `iot_*` 指标名称。每个数据点都应带有 `device.id` 标签，以便将读数归因到正确的设备。你只需发送对你的设备有意义的指标——缺失的指标只是不会被绘制成图表。
...
| `iot_cpu_usage_ratio`       | CPU 利用率，以 `0`–`1` 的比率表示（Cast Operations 将其存储为百分比）                  |
...
2. 在 Cast Operations 仪表板中，打开 **IoT** 区域——你的机群应在大约一分钟内以 `iot/<fleet>` 的形式出现。
...
2. 确认导出器端点为 `https://latticeruntime.com/otlp`（或你自托管的 `…/otlp`），且 `x-cast-operations-token` 请求头携带了有效令牌。
...
2. 请记住 `iot_cpu_usage_ratio` 是一个 `0`–`1` 的比率；发送原始比率，Cast Operations 会将其渲染为百分比。
3. 设备开始上报后，首批数据点最多需要一分钟才会显现。

## 自托管 Cast Operations

如果你正在自托管 Cast Operations，请将端点指向你自己的实例：
...
- 要深入了解底层的 OTLP 集成，请参阅[将 OpenTelemetry 与 Cast Operations 集成](/docs/telemetry/open-telemetry)。
