...
由于查询是从您网络内部的探针运行的，Cast Operations 无需直接连接到您的数据库，完整的结果集也绝不会离开探针——只有结果的一小部分有界投影会被上报回来。
...
在每次检查时，探针会连接到您的数据库，在只读上下文中运行您的查询，最多读回有限数量的行，并将一份紧凑的投影上报给 Cast Operations。随后会根据该投影评估您监控器的判定条件。
...
完整的结果集绝不会发送到 Cast Operations，因此客户数据不会被复制到 Cast Operations 存储中。
...
- 一个能够通过网络访问您数据库主机和端口的**探针**。它可以是 Cast Operations 托管的探针（如果您的数据库可从互联网访问），也可以是运行在您网络内部的自托管探针。有关如何安装自定义探针，请参阅探针文档。
...
1. 前往 Cast Operations 控制台 → 项目设置 → 监控器密钥 → 创建监控器密钥。
2. 创建一个密钥（例如 `dbPassword`），并授予此监控器对它的访问权限。
3. 在监控器的密码字段中，输入 `{{monitorSecrets.dbPassword}}`。

在将配置交给探针之前，Cast Operations 会在服务器端解析该密钥。Cast Operations 绝不会为您创建这些密钥——是否引用密钥由您自行决定。
