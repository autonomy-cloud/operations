...
Cast Operations 支持 SCIM v2.0 协议，用于自动化用户配置和取消配置。SCIM 使身份提供商（IdP）（如 Azure AD、Okta 及其他企业身份系统）能够自动管理用户对 Cast Operations 项目和状态页面的访问权限。

## 概述

SCIM 集成提供以下优势：

- **自动化用户配置**：当用户在 IdP 中被分配时，自动在 Cast Operations 中创建用户
- **自动化用户取消配置**：当用户在 IdP 中被取消分配时，自动从 Cast Operations 中移除用户
- **用户属性同步**：保持 IdP 与 Cast Operations 之间用户信息的同步
- **集中访问管理**：从现有身份管理系统管理 Cast Operations 访问权限

## 项目 SCIM

项目 SCIM 允许身份提供商管理 Cast Operations 项目中的团队成员。
...
1. **在 IdP 中分配用户**：当用户在 IdP 中被分配到 Cast Operations 时
2. **SCIM 配置**：IdP 调用 Cast Operations SCIM API 创建用户
3. **团队成员资格**：用户被自动添加到配置的默认团队
4. **授予访问权限**：用户现在可以访问 Cast Operations 项目
5. **取消分配用户**：当用户在 IdP 中被取消分配时
6. **SCIM 取消配置**：IdP 调用 Cast Operations SCIM API 移除用户
...
1. **在 IdP 中分配用户**：当用户在 IdP 中被分配到 Cast Operations 状态页面时
2. **SCIM 配置**：IdP 调用 Cast Operations SCIM API 创建订阅者
3. **授予访问权限**：用户现在可以访问私有状态页面
4. **取消分配用户**：当用户在 IdP 中被取消分配时
5. **SCIM 取消配置**：IdP 调用 Cast Operations SCIM API 移除订阅者
...
Microsoft Entra ID 提供企业级身份管理，具备强大的 SCIM 配置能力。按照以下详细步骤配置与 Cast Operations 的 SCIM 配置。

#### 前提条件

- 具有 Premium P1 或 P2 许可证的 Microsoft Entra ID 租户（自动配置所必需）
- 具有 Scale 计划或更高版本的 Cast Operations 账号
- 对 Microsoft Entra ID 和 Cast Operations 的管理员访问权限

#### 第一步：从 Cast Operations 获取 SCIM 配置
...
1. 在您的 Cast Operations 企业应用程序中，转到 **预配**
2. 点击 **开始使用**
3. 将 **预配模式** 设置为 **自动**
4. 在 **管理员凭据** 下：
...
| Azure AD 属性                                                 | Cast Operations SCIM 属性            | 是否必填 |
| ------------------------------------------------------------- | ------------------------------ | -------- |
...
| Azure AD 属性 | Cast Operations SCIM 属性 |
| ------------- | ------------------- |
...
1. 在您的 Cast Operations 企业应用程序中，转到 **用户和组**
2. 点击 **+ 添加用户/组**
3. 选择您要配置到 Cast Operations 的用户和/或组
4. 点击 **分配**
...
Okta 提供灵活的身份管理，具有出色的 SCIM 支持。按照以下详细步骤配置与 Cast Operations 的 SCIM 配置。

#### 前提条件

- 具有配置能力的 Okta 租户（生命周期管理功能）
- 具有 Scale 计划或更高版本的 Cast Operations 账号
- 对 Okta 和 Cast Operations 的管理员访问权限
...
1. 登录您的 Okta 管理控制台
2. 导航至 **应用程序** > **应用程序**
3. 找到并选择您现有的 Cast Operations 应用程序
...
5. 输入"Cast Operations"作为应用名称
6. 完成 SAML 配置（参阅 SSO 文档）
7. 点击 **完成**

#### 第三步：启用 SCIM 预配

1. 在您的 Cast Operations 应用程序中，转到 **常规** 选项卡
2. 在 **应用程序设置** 部分，点击 **编辑**
3. 在 **预配** 下，选择 **SCIM**
4. 点击 **保存**
5. 将出现新的 **预配** 选项卡
...
| Okta 属性          | Cast Operations SCIM 属性             | 方向        |
| ------------------ | ------------------------------- | ----------- |
...
2. 筛选与您的 Cast Operations 应用程序相关的事件
3. 验证预配事件是否成功
4. 检查 Cast Operations 以确认用户已被创建
...
- **错误：401 Unauthorized**：在 Cast Operations 中重新生成 Bearer Token 并更新 Okta
...
当用户被取消配置（通过 DELETE 请求或将 `active` 设置为 `false`）时，他们将从 SCIM 设置中配置的团队中被移除。用户账号本身仍保留在 Cast Operations 中，但失去对项目的访问权限。

### 我可以在不使用 SSO 的情况下使用 SCIM 吗？

可以，SCIM 和 SSO 是独立的功能。您可以使用 SCIM 进行用户配置，同时允许用户使用 Cast Operations 密码或任何其他认证方式登录。

### 如何处理 Cast Operations 中已存在的用户？

当 SCIM 尝试创建已存在（通过电子邮件匹配）的用户时，Cast Operations 只会将其添加到配置的默认团队，而不是创建重复用户。
