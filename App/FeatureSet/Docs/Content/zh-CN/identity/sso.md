...
Cast Operations 支持基于 SAML 2.0 的单点登录（SSO）进行企业认证。SSO 允许您的团队成员使用组织的身份提供商（IdP）登录 Cast Operations，从而实现集中访问管理和增强的安全性。
...
- **减少密码疲劳**：用户无需记住单独的 Cast Operations 密码

## 设置 SSO

1. **导航至项目设置**

   - 进入您的 Cast Operations 项目
   - 导航至 **项目设置** > **认证** > **SSO**
...
Keycloak 是一款流行的开源身份和访问管理解决方案。按照以下步骤将 Keycloak 配置为 Cast Operations 的 SAML 身份提供商。

### 前提条件

- 运行中的 Keycloak 实例，且已配置 realm
- 对 Keycloak 和 Cast Operations 的管理员访问权限
- 支持 SSO 的 Cast Operations 账号

### 第一步：配置 Cast Operations SSO
...
4. 复制证书值并以以下格式粘贴到 Cast Operations 中：
...
2. 创建新客户端或编辑现有客户端
3. 将 **客户端协议** 设置为 `saml`
4. 将 **客户端 ID** 设置为 Cast Operations **查看 SSO 配置** 中的 **标识符（实体 ID）** 值
5. 将 **有效重定向 URI** 设置为您的 Cast Operations URL
6. 将 **根 URL** 设置为您的 Cast Operations 基础 URL
7. 将 Cast Operations 中的 **回复 URL（断言使用者服务 URL）** 粘贴到 **断言使用者服务 POST 绑定 URL** 字段中
...
1. 保存 Keycloak 和 Cast Operations 中的所有设置
2. 尝试使用 SSO 登录 Cast Operations
3. 您应该被重定向到 Keycloak 登录页面，成功认证后返回 Cast Operations
...
Microsoft Entra ID 是 Microsoft 基于云的身份和访问管理服务。按照以下步骤将 Entra ID 配置为 Cast Operations 的 SAML 身份提供商。

### 前提条件

- Microsoft Entra ID 租户（任何支持企业应用程序 SAML SSO 的层级）
- 对 Microsoft Entra ID 和 Cast Operations 的管理员访问权限
- 支持 SSO 的 Cast Operations 账号
...
3. 在 **基本 SAML 配置** 中，点击 **编辑** 并设置：
   - **标识符（实体 ID）**：粘贴 Cast Operations **查看 SSO 配置** 中的 **标识符（实体 ID）**
   - **回复 URL（断言使用者服务 URL）**：粘贴 Cast Operations **查看 SSO 配置** 中的 **回复 URL**
4. 点击 **保存**
5. 在 **SAML 证书** 部分：
   - 下载 **证书（Base64）**
   - 在文本编辑器中打开下载的证书文件并复制内容
6. 在 **设置 Cast Operations** 部分，复制：
   - **登录 URL** — 将其粘贴为 Cast Operations 中的 **登录 URL**
   - **Azure AD 标识符** — 将其粘贴为 Cast Operations 中的 **颁发者**
7. 返回 Cast Operations，粘贴证书和 URL，然后保存
...
1. 保存 Entra ID 和 Cast Operations 中的所有设置
2. 尝试使用 SSO 登录 Cast Operations
3. 您应该被重定向到 Microsoft 登录页面，成功认证后返回 Cast Operations

### Microsoft Entra ID 故障排查

- **AADSTS700016 错误**：Entra ID 中的标识符（实体 ID）与 Cast Operations 不匹配 — 验证两个值是否完全相同
- **证书错误**：确保您下载的是 **Base64** 证书（而非原始/二进制格式），并包含 `BEGIN CERTIFICATE` / `END CERTIFICATE` 行
- **用户未分配**：用户必须明确分配到企业应用程序才能通过 SSO 登录
- **Name ID 不匹配**：确保 Name ID 声明设置为与用户在 Cast Operations 中的电子邮件匹配的电子邮件地址

---

## Okta SAML 配置

Okta 是一款广泛使用的身份平台，提供强大的 SAML SSO 能力。按照以下步骤将 Okta 配置为 Cast Operations 的 SAML 身份提供商。
...
5. 输入"Cast Operations"作为 **应用名称** 并点击 **下一步**
6. 在 **SAML 设置** 部分，配置：
   - **单一登录 URL**：粘贴 Cast Operations **查看 SSO 配置** 中的 **回复 URL（断言使用者服务 URL）**
   - **受众 URI（SP 实体 ID）**：粘贴 Cast Operations **查看 SSO 配置** 中的 **标识符（实体 ID）**
   - **Name ID 格式**：选择 `EmailAddress`
   - **应用程序用户名**：选择 `Email`
7. 点击 **下一步**，选择 **我是添加内部应用的 Okta 客户** 并点击 **完成**

### 第三步：将 Okta SAML 元数据复制到 Cast Operations

1. 在您的 Okta 应用程序中，转到 **登录** 选项卡
2. 在 **SAML 签名证书** 部分，找到活动证书，点击 **操作** > **查看 IdP 元数据**
3. 从元数据 XML 或 **登录** 选项卡详情中：
   - 复制 **登录 URL**（也称为 **身份提供商单一登录 URL**）— 将其粘贴为 Cast Operations 中的 **登录 URL**
   - 复制 **颁发者**（也称为 **身份提供商颁发者**）— 将其粘贴为 Cast Operations 中的 **颁发者**
4. 下载签名证书：
   - 在 **SAML 签名证书** 部分，点击活动证书的 **操作** > **下载证书**
   - 在文本编辑器中打开下载的 `.cert` 文件并复制内容
   - 将证书粘贴到 Cast Operations 中（包括 `BEGIN CERTIFICATE` 和 `END CERTIFICATE` 行）
5. 保存 Cast Operations SSO 配置
...
1. 保存 Okta 和 Cast Operations 中的所有设置
2. 尝试使用 SSO 登录 Cast Operations
3. 您应该被重定向到 Okta 登录页面，成功认证后返回 Cast Operations

### Okta 故障排查

- **404 或无效的 SSO URL**：验证 Okta 中的 **单一登录 URL** 是否与 Cast Operations 中的 **回复 URL** 完全匹配
- **受众不匹配**：确保 Okta 中的 **受众 URI** 与 Cast Operations 中的 **标识符（实体 ID）** 完全匹配
...
Cast Operations 的 SSO 实现使用 SAML 2.0 协议，应能与任何合规的身份提供商配合使用。通用配置步骤如下：

1. 在 Cast Operations 中，创建 SSO 配置，并从 **查看 SSO 配置** 按钮记录 **标识符（实体 ID）** 和 **回复 URL（断言使用者服务 URL）**
2. 在您的身份提供商中，使用以下信息创建 SAML 应用程序：
   - **断言使用者服务 URL / 回复 URL**：来自 Cast Operations SSO 配置
   - **实体 ID / 受众 URI**：来自 Cast Operations SSO 配置
   - **Name ID 格式**：电子邮件地址
3. 从您的身份提供商复制以下信息到 Cast Operations：
   - **登录 URL**（SSO 端点）
   - **颁发者**（IdP 的实体 ID）
   - **公共证书**（X.509 签名证书）
...
Cast Operations 目前不支持从身份提供商映射 SAML 角色。基于角色的访问控制必须在 Cast Operations 的 **项目设置** > **SSO** 设置中单独配置，您可以在其中为 SSO 用户分配默认角色。
