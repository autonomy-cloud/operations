...
Cast Operations ansluter till de verktyg ditt team redan använder — Zabbix, Jira, PagerDuty, Slack och många fler — via **[Arbetsflöden](/docs/workflows/index)**, den inbyggda automationsmotorn. Det finns inga separata plugins att installera. Du kopplar ihop en integration på en dra-och-släpp-arbetsyta, och den körs när något händer.

Den här sidan förklarar de två mönster som varje integration använder. När du förstår dem kan du ansluta Cast Operations till nästan vad som helst, även verktyg som inte har en egen sida här.

## De två mönstren

Varje integration flyttar data i en av två riktningar (och många använder båda).

### Inkommande — ett annat verktyg skickar data till Cast Operations

Använd detta när ett externt system behöver _skapa eller uppdatera något i Cast Operations_ — vanligtvis öppna en incident eller ett larm när det upptäcker ett problem.

1. Bygg ett arbetsflöde som börjar med en **[Webhook-utlösare](/docs/workflows/triggers#webhook)**. Cast Operations ger dig en unik URL.
...
### Utgående — Cast Operations skickar data till ett annat verktyg

Använd detta när _något i Cast Operations ska visas i ett annat verktyg_ — öppna ett Jira-ärende, larma någon i PagerDuty, posta till Slack.

1. Bygg ett arbetsflöde som börjar med en **[Cast Operations-händelseutlösare](/docs/workflows/triggers#cast-operations-event-triggers)** — till exempel **Incident → On Create**.
...
| --------------------------------------------------------------------- | ----------------------- | --------------------------------------------------------------------------------- |
| [Zabbix](/docs/integrations/zabbix)                                   | Inkommande              | Omvandla Zabbix-problem till Cast Operations-incidenter (och lös dem vid återhämtning). |
| [Jira](/docs/integrations/jira)                                       | Utgående (+ inkommande) | Öppna ett Jira-ärende för varje incident; synkronisera status tillbaka.           |
| [PagerDuty](/docs/integrations/pagerduty)                             | Utgående (+ inkommande) | Utlös och lös PagerDuty-händelser från Cast Operations-incidenter.                      |
| [Opsgenie](/docs/integrations/opsgenie)                               | Utgående (+ inkommande) | Skapa och stäng Opsgenie-larm.                                                    |
| [ServiceNow](/docs/integrations/servicenow)                           | Utgående (+ inkommande) | Öppna ServiceNow-incidenter från Cast Operations.                                       |
...
- Om verktyget kan **skicka en webhook** när något händer, använd det **inkommande** mönstret — peka dess webhook mot en Cast Operations Webhook-utlösare.
...
- [Utlösare](/docs/workflows/triggers) — Webhook- och Cast Operations-händelseutlösare i detalj.
