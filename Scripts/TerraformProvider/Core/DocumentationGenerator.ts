...
export class DocumentationGenerator {
  ...
  private async generateProviderDoc(): Promise<void> {
    const terraformLocalName: string = this.getTerraformLocalName();
...
`;
  ...
  private generateResourceDoc(resource: any): string {
    ...
    return `---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "${terraformLocalName}_${resource.name} Resource - ${this.config.providerName}"
subcategory: ""
description: |-
  ${resourceName} resource
---

# ${terraformLocalName}_${resource.name} (Resource)
...
\`\`\`terraform
resource "${terraformLocalName}_${resource.name}" "example" {
${exampleFields}
}
\`\`\`
...
\`\`\`shell
terraform import ${terraformLocalName}_${resource.name}.example <id>
\`\`\`
`;
  }
  ...
  private generateDataSourceDoc(dataSource: any): string {
    ...
    return `---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "${terraformLocalName}_${dataSource.name} Data Source - ${this.config.providerName}"
subcategory: ""
description: |-
  ${dataSourceName} data source
---

# ${terraformLocalName}_${dataSource.name} (Data Source)
...
\`\`\`terraform
data "${terraformLocalName}_${dataSource.name}" "example" {
...
# Configure variables
variable "${terraformLocalName}_api_key" {
...
`;
    ...
    if (resources.length > 0) {
      ...
      if (firstResource) {
        const resourceExample: string = `# Example usage of ${terraformLocalName}_${firstResource.name} resource
resource "${terraformLocalName}_${firstResource.name}" "example" {
...
output "${firstResource.name}_id" {
  ...
  value       = ${terraformLocalName}_${firstResource.name}.example.id
}
`;
    ...
    if (dataSources.length > 0) {
      ...
      if (firstDataSource) {
        const dataSourceExample: string = `# Example usage of ${terraformLocalName}_${firstDataSource.name} data source
data "${terraformLocalName}_${firstDataSource.name}" "example" {
...
output "${firstDataSource.name}_result" {
  ...
  value       = data.${terraformLocalName}_${firstDataSource.name}.example
}
`;
...
`;
  ...
  /**
   * Registry source names may contain hyphens, while local provider names and
   * resource type prefixes must be valid Terraform identifiers.
   */
  private getTerraformLocalName(): string {
    return StringUtils.toSnakeCase(this.config.providerName);
  }
