...
resource "cast_operations_monitor" "manual" {
...
resource "cast_operations_monitor" "incoming_request" {
...
resource "cast_operations_monitor" "server" {
...
resource "cast_operations_monitor" "manual_2" {
  name         = "TF Manual Type 2 ${random_id.suffix.hex}"
  description  = "Second manual monitor"
  monitor_type = "Manual"
}

resource "cast_operations_monitor" "manual_3" {
...
# Outputs
output "manual_id" {
  value       = cast_operations_monitor.manual.id
  description = "Manual monitor ID"
}

output "manual_type" {
  value       = cast_operations_monitor.manual.monitor_type
  description = "Manual monitor type"
}

output "incoming_request_id" {
  value       = cast_operations_monitor.incoming_request.id
  description = "IncomingRequest monitor ID"
}

output "incoming_request_type" {
  value       = cast_operations_monitor.incoming_request.monitor_type
  description = "IncomingRequest monitor type"
}

output "incoming_request_incoming_request_secret_key" {
  value       = cast_operations_monitor.incoming_request.incoming_request_secret_key
  description = "Server-generated secret key for incoming requests"
  sensitive   = true
}

output "server_id" {
  value       = cast_operations_monitor.server.id
  description = "Server monitor ID"
}

output "server_type" {
  value       = cast_operations_monitor.server.monitor_type
  description = "Server monitor type"
}

output "server_server_monitor_secret_key" {
  value       = cast_operations_monitor.server.server_monitor_secret_key
  description = "Server-generated secret key for server monitor"
  sensitive   = true
}

output "manual_2_id" {
  value       = cast_operations_monitor.manual_2.id
  description = "Second manual monitor ID"
}

output "manual_3_id" {
  value       = cast_operations_monitor.manual_3.id
