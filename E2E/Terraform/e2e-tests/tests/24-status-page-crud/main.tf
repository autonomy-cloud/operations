...
#
# This test validates complete CRUD operations for the cast_operations_status_page resource:
...
resource "cast_operations_status_page" "private" {
...
resource "cast_operations_status_page" "with_email" {
...
resource "cast_operations_status_page" "branded" {
  ...
  enable_email_subscribers            = false
  enable_sms_subscribers              = false
  hide_powered_by_operations_branding = true
}

# Test Case 5: Status Page with Labels
resource "cast_operations_label" "status_page_label" {
  name        = "TF E2E Status Page Label ${random_id.suffix.hex}"
  description = "Label for status page testing"
  color       = "#e74c3c"
}

resource "cast_operations_status_page" "with_labels" {
...
output "public_downtime_monitor_statuses" {
  value       = cast_operations_status_page.public.downtime_monitor_statuses
