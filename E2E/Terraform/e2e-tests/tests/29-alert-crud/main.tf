...
resource "cast_operations_alert_severity" "test_severity" {
...
resource "cast_operations_alert_state" "test_state" {
...
resource "cast_operations_monitor" "for_alert" {
...
resource "cast_operations_alert" "basic" {
  title             = "TF Basic Alert ${random_id.suffix.hex}"
  description       = "Basic alert created by Terraform E2E tests"
  alert_severity_id = cast_operations_alert_severity.test_severity.id
  monitor_id        = cast_operations_monitor.for_alert.id
}

# Test Case 2: Alert with root cause
resource "cast_operations_alert" "with_root_cause" {
...
resource "cast_operations_label" "alert_label" {
  name        = "TF Alert Label ${random_id.suffix.hex}"
  description = "Label for alert testing"
  color       = "#1abc9c"
}

resource "cast_operations_alert" "with_labels" {
  title             = "TF Labeled Alert ${random_id.suffix.hex}"
  description       = "Alert with labels attached"
  alert_severity_id = cast_operations_alert_severity.test_severity.id
  monitor_id        = cast_operations_monitor.for_alert.id
  labels            = [cast_operations_label.alert_label.id]
}

# Outputs
output "basic_alert_id" {
  value       = cast_operations_alert.basic.id
  description = "Basic alert ID"
}

output "basic_alert_title" {
  value       = cast_operations_alert.basic.title
  description = "Basic alert title"
}

output "root_cause_alert_id" {
  value       = cast_operations_alert.with_root_cause.id
  description = "Alert with root cause ID"
}

output "root_cause_value" {
  value       = cast_operations_alert.with_root_cause.root_cause
  description = "Root cause value"
}

output "labeled_alert_id" {
  value       = cast_operations_alert.with_labels.id
  description = "Labeled alert ID"
}

output "severity_id" {
  value       = cast_operations_alert_severity.test_severity.id
  description = "Alert severity ID"
}

output "state_id" {
  value       = cast_operations_alert_state.test_state.id
  description = "Alert state ID"
}

output "monitor_id" {
  value       = cast_operations_monitor.for_alert.id
  description = "Monitor ID"
}

output "label_id" {
  value       = cast_operations_label.alert_label.id
  description = "Alert label ID"
}

# Server-computed fields
output "basic_alert_created_at" {
  value       = cast_operations_alert.basic.created_at
