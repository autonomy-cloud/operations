...
Usage:
{{ include "cast-operations.renderTemplate" ( dict "value" .Values.path.to.the.Value "context" $) }}
*/}}
{{- define "cast-operations.renderTemplate" -}}
...
*/}}
{{- define "cast-operations.memoryToBytes" -}}
{{- $q := . | toString -}}
...
{{- define "cast-operations.image.tag" -}}
  {{- $values := .Values -}}
  {{- default "release" $values.image.tag -}}
{{- end -}}

{{- define "cast-operations.image" -}}
  {{- $values := .Values -}}
  {{- $service := .ServiceName -}}
  {{- $imageName := default $service .ImageName -}}
  {{- printf "%s/%s/%s:%s" $values.image.registry $values.image.repository $imageName (include "cast-operations.image.tag" (dict "Values" $values)) -}}
{{- end -}}
...
*/}}
{{- define "cast-operations.postgres.backendHost" -}}
...
{{- define "cast-operations.postgres.backendPort" -}}
{{- if .Values.postgresOperator.cnpg.enabled -}}
5432
{{- else if .Values.postgresql.enabled -}}
...
{{- define "cast-operations.postgres.backendUser" -}}
...
*/}}
{{- define "cast-operations.postgres.backendPasswordEnv" -}}
...
{{- define "cast-operations.env.common" }}
{{- $provisionSSL := false -}}
...
{{- include "cast-operations.env.globalLlmProvider" $ }}
{{- end }}
...
*/}}
{{- define "cast-operations.env.globalLlmProvider" }}
...
{{- define "cast-operations.env.castOperationsSecret" }}
- name: CAST_OPERATIONS_SECRET
  {{- if $.Values.castOperationsSecret }}
  value: {{ $.Values.castOperationsSecret }}
  {{- else }}
  {{- if $.Values.externalSecrets.castOperationsSecret.existingSecret.name }}
  valueFrom:
    secretKeyRef:
        name: {{ $.Values.externalSecrets.castOperationsSecret.existingSecret.name }}
        key: {{ $.Values.externalSecrets.castOperationsSecret.existingSecret.passwordKey }}
  {{- else }}
  valueFrom:
    secretKeyRef:
      name: {{ printf "%s-%s" $.Release.Name "secrets"  }}
      key: cast-operations-secret
  {{- end }}
  {{- end }}
{{- end }}

{{- define "cast-operations.env.registerProbeKey" }}
...
{{- define "cast-operations.env.runtime" }}
...
- name: CLICKHOUSE_USER
  ...
  value: {{ $chAltinity.auth.username | default "cast-operations" }}
...
- name: CLICKHOUSE_DATABASE
  {{- if $chAltinity.enabled }}
  value: {{ $chAltinity.database | default "cast-operations" | squote }}
  {{- else if $.Values.clickhouse.enabled }}
  value: {{ printf "cast-operations" | squote}}
  {{- else }}
...
#   - built-in StatefulSet: the "cast-operations" 1-node cluster + embedded Keeper in
...
- name: CLICKHOUSE_CLUSTER_NAME
  {{- if $chAltinity.enabled }}
  value: {{ $chAltinity.cluster.name | default "cast-operations" | squote }}
  {{- else if $.Values.clickhouse.enabled }}
  value: "cast-operations"
  {{- else }}
  value: {{ $.Values.externalClickhouse.clusterName | default "cast-operations" | squote }}
  {{- end }}
{{- if $chAltinity.enabled }}
...
- name: DATABASE_NAME
  {{- if $cnpg.enabled }}
  value: {{ $cnpg.database | default "castoperationsdb" }}
  {{- else if $.Values.postgresql.enabled }}
...
{{- include "cast-operations.env.castOperationsSecret" . }}
{{- end }}

{{- define "cast-operations.env.pod" }}
- name: NODE_NAME
  valueFrom:
    fieldRef:
...
{{- define "cast-operations.service" }}
apiVersion: v1
kind: Service
metadata:
  labels:
...
{{- define "cast-operations.deployment" }}
apiVersion: apps/v1
kind: Deployment
metadata:
  name: {{ printf "%s-%s" $.Release.Name $.ServiceName  }}
...
spec:
  ...
  {{- else }}
  {{- if or (not (eq (include "cast-operations.autoscalingEnabled" (dict "Values" $.Values "Override" $.AutoscalingOverride)) "true")) ($.DisableAutoscaler) }}
  ...
  template:
    ...
    spec:
      ...
      containers:
        {{- if $.ImageName }}
        - image: {{ include "cast-operations.image" (dict "Values" .Values "ServiceName" $.ServiceName "ImageName" $.ImageName) }}
        {{- else }}
        - image: {{ include "cast-operations.image" (dict "Values" .Values "ServiceName" $.ServiceName) }}
        {{- end}}
          name: {{ printf "%s-%s" $.Release.Name $.ServiceName  }}
          ...
          env:
            {{- include "cast-operations.env.common" . | nindent 12 }}
            {{- include "cast-operations.env.runtime" . | nindent 12 }}
            {{- if $.Env }}
            {{- range $key, $val := $.Env }}
            - name: {{ $key }}
              value: {{ $val | squote }}
...
{{/*
cast-operations.autoscalingEnabled returns "true" when autoscaling is effectively
...
Usage: {{- if eq (include "cast-operations.autoscalingEnabled" (dict "Values" $.Values "Override" $.Values.nginx.autoscaling)) "true" }}
*/}}
{{- define "cast-operations.autoscalingEnabled" -}}
...
{{/*
cast-operations.autoscaler renders a HorizontalPodAutoscaler. Each field falls back to
...
not set" on upgrade. Mirrors the leading newline that cast-operations.service relies on.
*/}}
{{- define "cast-operations.autoscaler" }}
{{- $g := .Values.autoscaling | default dict -}}
{{- $o := .Override | default dict -}}
{{- $enabled := $g.enabled -}}
{{- if hasKey $o "enabled" -}}{{- $enabled = $o.enabled -}}{{- end -}}
...
Usage:
  include "cast-operations.pdb" (dict
    "ServiceName" "app"
    "Override" $.Values.app.podDisruptionBudget
    "Release" $.Release
    "Values" $.Values)
*/}}
{{- define "cast-operations.pdb" -}}
...
Usage: include "cast-operations.kedaScaledObject" (dict "ServiceName" "service-name" "Release" .Release "Values" .Values "MetricsConfig" {...})
*/}}
{{- define "cast-operations.kedaScaledObject" }}
