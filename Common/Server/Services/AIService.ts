 ...
 * — the in-house coding agent's tool loop routes every LLM call
...
export interface AILogRequest {
  ...
  /** Provider retry count. A workflow uses zero to avoid duplicate executions. */
...
export class Service extends BaseService {
  ...
  /** Assert that AI is enabled for this project. */
  @CaptureSpan()
  ...
  public async getAutonomousDailyBudgetStatus(
    ...
    const usedTokensToday: number = await LlmLogService.getTotalTokensUsedSince(
      {
        projectId,
        since: OperationsDate.getStartOfDay(
  ...
  public async executeWithLogging(
    ...
    /*
     * G4 daily budget enforcement — autonomous features only. Autonomous runs fail closed on budget
