 ...
 *
 * When an incident moves to a resolved state, AI drafts a postmortem from
  ...
  public static async draftPostmortemOnResolve(data: {
    ...
    try {
      ...
      await IncidentFeedService.createIncidentFeedItem({
        ...
        feedInfoInMarkdown: `## 🧠 AI — Draft Postmortem\n\nOperations AI drafted a postmortem for incident #${incident.incidentNumber} from the incident timeline and telemetry. It has been saved on the incident for you to review and edit.`,
      ...
      logger.debug(
        `AI: drafted postmortem for incident ${incidentId.toString()}.`,
      );
    } catch (error) {
      logger.error(
        `AI: failed to draft postmortem for incident ${incidentId.toString()}: ${error}`,
      );
    }
  }
}
